...
// 🔴 2026-09-26 加固（老现象在语文/教辅侧的复现路径）：题干内"要求/提示"分条若被模型写成「1./2./3.」
//   会被 2k 当成独立小题 → 每条各补一处作答行（"作文题的要求下每条都长横线"）。
//   判据（保守双条件）：以「1.」开头且上一非空兄弟是以"要求/提示…："结尾的段 → 判分条，续项一并排除。
describe('⑦ 要求/提示分条的作答位：整题一处、分条之后（不逐条补）', () => {
  it('「要求：1./2./3.」不被当作三道小题——整题只补一处作答位', () => {
    const html = '<h2>三、综合练习（10分）</h2>'
      + '<p>要求：</p>'
      + '<p>1. 写清楚事情经过。</p>'
      + '<p>2. 语句通顺，标点正确。</p>'
      + '<p>3. 不少于三句话。</p>';
    const r = auditExamPaper(html, { subject: '语文', stage: 'primary_low', genType: 'exam' });
    const lines = (r.html.match(/blank-line/g) || []).length;
    // 整题一处（按分值 10×0.5=5 → 封顶 4 行）；若分条被当三题则会是 3 块 × 2 行 = 6 条
    expect(lines, `应为整题一处（4 行），实际 ${lines} 条（>4 说明分条被逐条补了）`).toBe(4);
  });
});
