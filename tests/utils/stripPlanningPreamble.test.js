...
describe('stripPlanningPreamble（过程自述剥离）', () => {
  it('开头"我已获取教材原文…现在编写正文"自述段 → 整段剥除', () => {
    const html = '<p>我已获取教材原文与知识点。现在依据2022年版课标与教材内容，围绕"小数乘法和除法（二）"的核心知识（小数乘法计算方法、循环小数等）命制课时练。现在编写正文。</p><h2>一、基础建构</h2><p>1. 题目。</p>';
    expect(stripPlanningPreamble(html)).toBe('<h2>一、基础建构</h2><p>1. 题目。</p>');
  });
  ...
  it('题号开头的真内容段 → 不剥（防误删正文）', () => {
    const html = '<p>1. 班级义卖准备了一批手工香皂。</p><h2>一、基础建构</h2>';
  ...
  it('真实产物样本（16:31 随堂巩固开头自述）→ 剥除', () => {
    const html = '<p>我已获取教材原文与知识点。现在依据课标与教材，编写课时练习正文。本课核心知识覆盖：小数乘法（含小数乘小数竖式、积的近似值、乘大于/小于1的数的规律）、小数除法（除数是小数的除法、商的近似值、实际问题中商的近似取值、除以大于/小于1的数的规律）、算理（小数化分数理解、小数点移动）、循环小数、估算近似应用。</p><h2>一、基础建构</h2><p>1. 题目。</p>';
    expect(stripPlanningPreamble(html)).toBe('<h2>一、基础建构</h2><p>1. 题目。</p>');
  });
});
