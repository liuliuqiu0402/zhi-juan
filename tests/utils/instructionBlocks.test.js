...
const TEMPLATE_BODY = '你是资深命题专家。请为{grade}{subject}命制一份正式试卷。\n'
  + '· 作答空间形态按答案类型匹配，不自行发明：\n'
  + '· 空位具体形态（圆括号空／横线空／圈作答）由你按题干措辞与作答需要选定——所填为字母/序号/√×等符号的题多用圆括号空位（　）或圈作答，填词/句/数等短答与"列举归类"空位多用横线空位；形态真实可书写、同卷同性质空位风格统一即可，填符号类的括号内宽只须容纳所填符号（1~2 字位）；\n'
...
const BLANK_LINE = '· 短答空位宽度按"恰好容纳该空答案"换算：先在心里给出该空答案并数清字符数（数字/汉字/小数点各算 1 个），写等量的全角空格（1 字位≈1 个全角空格≈1 em 书写宽），连列空位全带、不得遗漏；';
