...
describe('各块文本口径（防漂移的逐字锚点）', () => {
  it('组织方式：exam 逐字不变（守卷面结构）；教辅整类不再注入本块（形式全交模型）', () => {
    // 🔒 2026-09-17 用户裁定（课标要求须在每个栏目行上可见、并作为"要求"生效）：本块补
    //    "结构行【要求·须逐项落实】= 课标/正规卷形态要求 + 成稿前逐栏目对照自查"（实证：蓝本注"短文，选择与判断"，
    //    卷面只出"判断"一种设问——注解被当背景描述读过去）。除此之外逐字不动。
    expect(buildOrganizeBlock('exam')).toBe('【组织方式】输出一律以委托书【卷面结构】的大题序列组织（大题名、顺序、题量以委托书为准）；开头【锚点清单】只声明要练到的范围，不是组织方式，不得据此替代委托书结构。每行"【要求·须逐项落实】"后是该大题按**课标与正规卷**定的形态要求（设问方式、材料形态、读写方式、每空词数、材料读几遍等），**必须逐项落实**；成稿前逐栏目对照自查：凡该行写了而卷面没有照做的（如要求两种设问形态却只出一种），改题面或改内容，二者取一——把它当"背景描述"读过去，即属未落实。\n\n');
  ...
  it('尾约束×2：块内自带 \\n\\n 前缀，关键句逐字', () => {
    ...
    // 2026-09-17 用户裁定（模型侧承接、全类型通用）：由"使本题…仅凭正文自身即可完成"改为类型中性的
    // "使每处内容…即可完成或读懂"（内容型资料没有"本题"，原措辞对新类型不成立），三域结构见其上方注释。
    expect(self).toContain('使每处内容**仅凭正文自身即可完成或读懂**');
