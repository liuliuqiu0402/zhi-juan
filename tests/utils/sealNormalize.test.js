...
<h2>一、看拼音写词语</h2><p>正文段落，不应被合并。</p>`;
  ...
  it('旧结构：信息栏与提示横向 p 并入 sealed-line，重建为 [sl-dash][sl-text]…交替新结构', () => {
    ...
    // 新结构：字段整组横排（sl-text），虚线单元（sl-dash）穿插
    expect(out).toContain('<span class="sl-text">学校：＿＿＿</span>');
    expect(out).toContain('<span class="sl-text">密封线内不要答题</span>');
    // 密/封/线 逐字拆为独立字段（字间虚线相连）
    expect(out).toContain('<span class="sl-text">密</span>');
    expect(out).toContain('<span class="sl-text">线</span>');
    // 字段顺序：信息 → 提示 → 密封线三字；首尾均为虚线单元
    const pos = (s) => out.indexOf(s);
    expect(pos('<span class="sl-text">密封线内不要答题</span>')).toBeGreaterThan(pos('<span class="sl-text">学号：＿＿＿</span>'));
    expect(pos('<span class="sl-text">密</span>')).toBeGreaterThan(pos('<span class="sl-text">密封线内不要答题</span>'));
  ...
  it('主题包装 sealed_exam：仅密封特征 p 并入字段序列，正文不被吞并', () => {
    ...
    expect(wrapped).toContain('<span class="sl-text">学校：＿＿＿</span>');
    expect(wrapped).toContain('<span class="sl-text">密</span>');
    ...
    expect(unwrapped).toContain('<span class="sl-text">姓名：＿＿＿</span>');
