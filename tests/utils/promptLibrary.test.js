...
describe('试卷大类层 / 部分标题 / 自洽判据域（2026-09-26 用户定版）', () => {
  ...
  it('大类层下三层展开：大类（居中无编号）→ 大题标题（自拟、一、、分值）→ 小题（1.2.3. 连续）', () => {
    ...
    expect(t, '大题标题仍自拟、不照抄调研分类名（2026-09-19 口径不放松）').toContain('不是照抄它行首的调研分类名');
    expect(t, '小题序号跨大类/部分继续递增、全卷连续').toContain('跨大类/部分继续递增、全卷连续');
    expect(t, '大类下分值账目闭合').toContain('该大类下各题分值合计必须等于该大类在【卷面结构】里的分值');
  });

  it('自洽（层级↔内容）：大类名 ↔ 其下各题内容相符；大类名与大题标题不重复、不顶替', () => {
    const t = examTpl();
    expect(t).toContain('大类名须与其下各题实际考查的内容相符');
    expect(t).toContain('不得挂"积累与运用"之名而行阅读之实');
