...
describe('书写载体注入矩阵：内容型（summary / preview，2 类）', () => {
  ...
  // 🔴 2026-09-27 用户追问"内容型里也会含有例题的吧？"——例题属**讲解示范型栏目**，
  //    解答随例题即时展示（**不算需隐藏的学生答案**）、且保留例题自身作答形态（答案回填在其作答位上）。
  it('内容型含"例题/示例讲解"条款（讲解示范随例题即时展示 + 例题保留原作答形态）', () => {
    for (const g of CONTENT_TYPES) {
      const t = tplOf(g);
      expect(t, `${g} 缺例题条款`).toContain('典型例题');
      expect(t, `${g} 缺"讲解示范型栏目"定性`).toContain('讲解示范型');
      expect(t, `${g} 缺"例题保留其原有作答形态"`).toContain('保留其原有作答形态');
    }
  });
});
