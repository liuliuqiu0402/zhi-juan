...
describe('buildAnswerSpaceInstruction（通用六行：形态按作答需要选择+硬约束，宽度/一致性并入通用层）', () => {
  ...
  it('通用（无学科）逐字完整：总句+语感选定(含1~2字位)+题干末尾括号位+短答宽度(含可执行换算)+同题同形态/一空一载体+禁文字占位+题面空位类不另附长横线', () => {
    expect(generic).toBe(
      ...
      // 🔴 2026-09-17 用户实证新增（同一大题内跨题的位置统一；全卷范围的括号形态统一）
  ...
  it('带选项题作答位定死：题首（题干前）+ 圆括号空位、选项行内/末尾/之后不加（2026-09 位置实证 / 2026-09-15 形态实证 / 2026-09-16 段落实证）', () => {
    ...
    //    且纯形态描述、不含题型名（防题型诱导）。
    const CLAUSE_COMMON = '其作答位**形态一律圆括号空位**';
    const HEAD = '**位置在题号之前的题首**、用**半角**圆括号';
    const TAILPOS = '**位置在题干末尾**、用**全角**圆括号';
