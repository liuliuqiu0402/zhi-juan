...
describe('buildAnswerSpaceInstruction（通用六行：形态按语感选择+硬约束，宽度/一致性并入通用层）', () => {
  const generic = buildAnswerSpaceInstruction();
  it('通用（无学科）逐字完整：总句+语感选定(含1~2字位)+题首括号位+短答宽度(含可执行换算)+同题同形态/一空一载体+禁文字占位+判断类不另附长横线', () => {
    expect(generic).toBe(
      '· 作答空间形态按答案类型匹配，不自行发明：\n' +
  ...
  it('选择/判断/圈选类作答括号位置定死：题首（题干前）、选项行内/末尾不加（2026-09 用户实证·选项末尾挂空位）', () => {
    // 实证：英语卷选择题答案空位被模型挂到选项末尾（C. are; am＿）——作答位位置属排版硬约束，
    // 不交模型语感；本条放通用段（单一事实源），全学科随行（含数学判断√×位）。
    const CLAUSE = '的作答括号在题干前（题首，如（　）1. 题干…）';
  ...
  it('判断/选择/圈选类不另附长横线条款恒在（2026-09 源头条款·阅读判断题实证）：通用段注入、全学科随行', () => {
    // 实证：英语卷阅读判断题被模型在短文后额外出 2 行长横线——作答位本就在小题括号空位内；
    // 本条放在通用段（单一事实源），与补差端 2k 结构性题型排除（判断/选择/圈选不补差）同口径。
    const CLAUSE = '判断/选择/圈选类题（含阅读判断题）的作答位即题面空位（括号空位/圈选位），不另附长横线作答区';
    expect(generic).toContain(CLAUSE);
    for (const [s, st] of [['英语', 'primary_high'], ['语文', 'primary_low'], ['数学', 'primary_mid'], ['物理', 'middle']]) {
      expect(buildAnswerSpaceInstruction(s, st), `${s}·${st} 缺判断类不另附长横线条款`).toContain(CLAUSE);
    }
  });
