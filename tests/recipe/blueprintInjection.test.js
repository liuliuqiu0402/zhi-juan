...
describe('buildStructureText（exam 卷面结构注入段，单一事实源）', () => {
  ...
  it('每个栏目行都带「要求」标注，注内容原样保留（不是行尾注解）', () => {
    const bp = getExamBlueprint('英语', 'primary_high');
    const inject = buildStructureText(bp);
    expect(inject.match(/——【要求·须逐项落实】/g) || [], '逐行都要带标注').toHaveLength(bp.sections.length);
    // 注内容就是课标/正规卷要求，原样保留、不得改写或截断
    expect(inject).toContain('短文，选择与判断');
  ...
  it('8 类非 exam 三维度模板均含【输出格式】与正文边界要求', () => {
    for (const g of NON_EXAM_TYPES) {
      ...
      if (CONTENT_TYPES.includes(g)) {
        ...
        expect(t.template, `类型 ${g} 缺作答空间语义`).toContain('作答空位形态与所填内容相称');
...
describe('作答空间形态语义全模板覆盖（按答案类型匹配；形态词带锚不诱导）', () => {
  ...
  it('题为主 7 类模板作答空间语义六行逐字完整（不漏一字：总句+符号位倾向+题干末尾括号位+短答换算+同题同形态+禁文字占位+题面空位类不另附长横线）', () => {
    ...
    const FULL_LINE =
      ...
      '· 题面带选项（A./B./C. 等）的题，其作答位**形态一律圆括号空位**（不用下划线空/横线空——圆括号对应所填字母/序号/√×等符号，下划线空对应填词/句/数等短答，两者不得混用）：**位置在题干末尾**、用**全角**圆括号，写成"1. ……的是（　）"（中文科目正式卷惯例）；选项行内、选项末尾，以及选项行之后，一律不加作答位：不给整行横线、不给空白作答行、不另设作答区，作答位只有上述那一处（题干内指示所填内容的那处空位不是作答位——它是题面的一部分、由选项作答，照常保留，不在本句限制之内）；🔴 本条是**硬约束**，题面若声明了与本条不符的形态，**改题面、不改本条**——与【题目自洽①】冲突时以本条为准；\n' +
  ...
  it('形态语义按答案类型绑定（换算锚随 BLANK 动态注入；内容型无作答空间规则）', () => {
    for (const g of ALL_TYPES) {
      ...
      if (CONTENT_TYPES.includes(g)) {
        ...
        expect(t.template, `类型 ${g} 缺换算锚`).toContain('1 字位≈1 个全角空格≈1 em 书写宽');
        expect(t.template, `类型 ${g} 缺形态选择权句`).toContain('由你按题干措辞与作答需要选定');
        expect(t.template, `类型 ${g} 缺短答横线倾向句`).toContain('多用横线空位');
        expect(t.template, `类型 ${g} 缺硬约束句`).toContain('同性质空位的形态一致');
