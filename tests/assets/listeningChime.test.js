 ...
 *
 * 音色口径（✅ 2026-09-20 用户试听后**定版**："这个对了，系统里就按这个来"；详见生成脚本文首参数表）：
 *   · 形态＝两音（高考听力"打点声"即"叮咚"两音，非单声）
 *   · 音程＝下行大三度，音区取**高八度 E6(1318.5Hz) → C6(1046.5Hz)**（用户选定"第三组对"）
 *   · 音色＝金属钟铃分音 [0.5×(嗡音托底), 1×, 2×, 2.76×, 5.4×]——
 *     高次分音刻意削弱且先衰（用户反馈"音太尖了，感觉捏着嗓子在吊音"）；
 *     实测 3kHz 以上能量占比由 6.5% 降到 3.1%（5kHz 以上 1.0%→0.3%），厚度增加而尖度下降
 *   · 节奏与响度＝按用户"太快了、要再响亮些"调优：两音间隔 0.42s、衰减放缓（总长≈1.46s），
 *     峰值 -0.4 dBFS；实测综合响度 -13.3 LUFS，明显高于被读语音（-19.5 ~ -20.9 LUFS）
 */
