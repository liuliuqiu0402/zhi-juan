...
// ==================== 🔧 整卷生成（DeepSeek 云端主路径）====================
  ...
  const _runPaperOrder = async (params = {}) => {
    ...
    const anchorMissingNote = anchorReport.missingList?.length
      ? `⚠️ 覆盖缺料：${anchorReport.missingList.length} 项核心知识所在章节无教材片段（该考点已列入【锚点清单】，但缺少教材原文依据，只能凭课标与通识把握）：${anchorReport.missingList.map((m) => `${m.chapter}·${m.name}`).join('、')}。请检查勾选章节的原文解析/粘贴是否完整（补齐原文即恢复依据）。`
      : '';
    ...
    //    🔴 2026-09-13（同日第二版·下限非上限）：清单语义由"命题范围边界"改为"覆盖下限"——
    //       清单考点须**全部覆盖**（保证本单元必学点不漏），但清单**不是命题上限**，可依课标学段学业要求补充清单外考点；
    //       理由：生成后覆盖对账已整体废除（P2b，2026-09），"范围即边界"的原始用途消失，而"锚点外考点考不到 +
    //       教材分析不全则漏考"成为净损失；放开上限后由"下限"继续守住不漏，"上限"交给课标把握。
    if (compressedText || anchorListText) {
      const refMode = contractOf(genType).mode;
      prompt += '【素材使用约定】\n'
        + '· 开头【锚点清单】的考点**至少要全部覆盖到**（这是覆盖**下限**，保证本单元必学知识不漏）；但它**不是命题上限**——可依本学段课标学业要求，适当补充清单未涉及的知识点或考查角度（不超出本学段学业要求）；本条只约束"覆盖哪些考点"，**不是素材来源限制**；中段【压缩原文】是理解教材内容与难度的**参考之一**——情境、素材、人名、数据与句式可取自教材，也可取自课外真实生活（主题相关、难度适切），**来源不限、不作指定**；\n';
      prompt += refMode === 'full'
