...
- 第2层 coreKnowledge = **可独立成题 / 可独立教学组织的考点**，每条即一个锚；第1层 bigConcept **不取为锚**
- **最小单位强制下沉第3层**：单个字、单个词条、单个符号、单个数值、术语碎片 → 一律放进对应考点的 specificConcepts，**不得提为第2层条目**
...
- 🔴 **完整性自查（提取完成后必做，逐条核对，缺即补）**：以本课/单元的**课标要求的学业内容**为参照逐条自查——课标要求掌握的概念、规则、方法、技能、语言材料（字词句篇）、语篇要点等，是否都已有对应的第2层考点或第3层具体概念落在树中；发现缺失即补入对应层级（宁可细、不可漏）；禁止用"概括成一个上位词"的方式把具体可考点整体吞掉
- 🔴 **防"合并吞点"**：做同义归并前，先确认没有被归并的内容是**可独立成题/可独立教学组织**的；归并只在不丢具体可考点的前提下进行，归并后仍须能在第3层逐项列出被并入的具体内容
...
- 📝 生字/生词：逐个提取（不合并、不遗漏），但**下沉到第3层 specificConcepts**（归属第2层「识字与书写」等考点）；单字/单词条**不得提为第2层条目**
...
- 🔧 第3层收录边界（防重复、防混入题面）：① **同一考点内去重** —— 同一单位只收一条（同一内容的不同写法/重复出现合并为一条，不得因原文多处出现就重复罗列）；② **只收"单位本身"**，不收题干/选项/例句/上下文里的整句或整段（如"下列词语中加点字的读音""读一读，记一记"这类题面、指令语、提示语不属最小单位）；③ 第2层考点名本身不再下沉重复收录；④ **整句类考点豁免** —— 若考点内容**只能以整句/情境形式表达**（数学结论、科学现象、情境描述、口诀、算式实例等，天然压不成字词碎片），则**保留该整句为单位收进第3层，不得丢弃，也不得硬拆成无意义的碎字词**；语义保真 > 形式最小化（防"为了符合'只收单位'而丢考点或拆坏语义"）
- 🔧 主题词按原文篇幅匹配：短文（<5段）2-3个主题词，长文3-6个，以能概括全文核心内容为准
- 🔧 JSON 字段值尽量简短，不要写长句子（**例外**：收录边界④的整句类考点为保语义可保留完整表述，不受此限）
...
// ==================== 🔧 整卷生成（DeepSeek 云端主路径）====================
  ...
  const _runPaperOrder = async (params = {}) => {
    ...
    // 🔴 缺料诊断上抛（2026-09 P0；2026-09-13 措辞与实现对齐）：missing 考点**仍会进【锚点清单】**（
    ...
    const anchorMissingNote = anchorReport.missingList?.length
      ? `⚠️ 覆盖缺料：${anchorReport.missingList.length} 项核心知识所在章节无教材片段（该考点已列入【锚点清单】，但缺少教材原文依据，只能依课标要求与学科规范把握，须守【质量底线】事实可信口径——无把握不写具体事实）：${anchorReport.missingList.map((m) => `${m.chapter}·${m.name}`).join('、')}。请检查勾选章节的原文解析/粘贴是否完整（补齐原文即恢复依据）。`
      : '';
    ...
    //    🔴 2026-09-13（用户定版·双向开放）：区分"考点范围"与"素材来源"——范围（覆盖哪些考点）以【锚点清单】为准，
    ...
    if (compressedText || anchorListText) {
      ...
      const coverageExtent = extentKey === 'expand'
        ...
        : extentKey === 'integrate'
          ? '清单**不是范围围墙**——可联系**已学**旧知与同类知识做**结构化整合**（分类整理、前后勾连、形成知识网络），边界为"已学 + 本学段课标要求"，不引入未学的超纲内容；'
          : '只按清单（本课/本单元）呈现，不做清单外的补充与整合（默写类须严格对应教材要求）；';
      prompt += '【素材使用约定】\n'
        + `· ${coverageFloor}${coverageExtent}本条只约束"覆盖哪些知识点"，**不是素材来源限制**；中段【压缩原文】是理解教材内容与难度的**参考之一**——情境、素材、人名、数据与句式可取自教材，也可取自课外真实生活（主题相关、难度适切），**来源不限、不作指定**；\n`;
      prompt += refMode === 'full'
        ? '· 本资料为知识归纳型：可引用、可归纳【压缩原文】，但须转写为教辅表述，不得整段照录；正文不得出现任何出处标注（"选自/单元/章节/课题/课文名/位置式指引/原文出处"等溯源字样一律不写）。\n'
        : '· 本资料为命题/练习型：【压缩原文】供你理解题型结构、知识梯度与难度；题干、情境、人名、数据与句式由你拟定，**来源不限**（教材或课外真实生活皆可，主题相关、难度适切）。\n';
      prompt += '· 【压缩原文】中的练习/习题段仅供理解题型与难度，**不得照搬题目**。\n\n';
      // ✅ A5-1/A12-2（2026-09-11）：输出组织一律以委托书【教辅结构】栏目序列为准——
      //    【锚点清单】只声明"写什么范围"（覆盖范围），**不是组织方式**（它是考点维度，与栏目结构是两套组织）。
      //    ✅ §4.1 极短指针（非重复）：覆盖范围（下限）以开头【锚点清单】为准、素材来源不作指定（不复述清单内容）。
      prompt += '【组织方式】输出一律以委托书【教辅结构】的栏目序列组织（栏目名、顺序、题量以委托书为准）；开头【锚点清单】声明覆盖**下限**（非命题上限），不得据此替代栏目结构；素材来源不限（教材或课外真实生活皆可，主题相关、难度适切）。\n\n';
    }
    ...
    prompt += `\n\n【尾约束·全文自洽】
题干所声明的、本题作答所必需的一切内容（不论其形态），都必须在正文中真实、足量、形式吻合地存在——使本题**仅凭正文自身即可完成**；凡题干提到而正文未给出、或给出但不完整、不足以支撑该设问的，即属无效内容，须于定稿前修正。素材依课标可取自教材之外的真实情境、不限所选教材，但题面引用或呈现的素材必须与本题实际给出的内容完全一致；正文不得先现待作答结论；正文对知识、要点、示例、数据、结论的归纳与转述须准确、完整、不遗漏、条理清晰，各题、各要点、各结论及其答案之间前后一致、互不矛盾。凡题干与正文不符，或内容漏错、自相矛盾，即为无效内容，须于定稿前修正。`;
