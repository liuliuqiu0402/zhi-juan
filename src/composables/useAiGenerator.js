...
// ==================== 🔧 整卷生成（DeepSeek 云端主路径）====================
  ...
  const _runPaperOrder = async (params = {}) => {
    ...
    if (compressedText || anchorListText) {
      ...
      prompt += '【素材使用约定】\n'
        + '· 开头【锚点清单】的考点**至少要全部覆盖到**（这是覆盖**下限**，保证本单元必学知识不漏）；但它**不是命题上限**——可依本课标学段学业要求，适当补充清单未涉及的考点或考查角度（不超出本学段学业要求）；本条只约束"覆盖哪些考点"，**不是素材来源限制**；中段【压缩原文】是理解教材内容与难度的**参考之一**——情境、素材、人名、数据与句式可取自教材，也可取自课外真实生活（主题相关、难度适切），**来源不限、不作指定**；\n';
      prompt += refMode === 'full'
