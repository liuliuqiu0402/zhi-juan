...
// ==================== 🔧 整卷生成（DeepSeek 云端主路径）====================
  ...
  const _runPaperOrder = async (params = {}) => {
    ...
    if (compressedText || anchorListText) {
      ...
      prompt += '【素材使用约定】\n'
        + '· 命题的**考点范围**以开头【锚点清单】为准（这只约束"覆盖哪些考点"，**不是素材来源限制**）；中段【压缩原文】是理解教材内容与难度的**参考之一**——情境、素材、人名、数据与句式可取自教材，也可取自课外真实生活（主题相关、难度适切），**来源不限、不作指定**；\n';
      prompt += refMode === 'full'
      ...
      //    ✅ §4.1 极短指针（非重复）：覆盖范围以开头【锚点清单】为准、素材来源不作指定（不复述清单内容）。
      prompt += '【组织方式】输出一律以委托书【教辅结构】的栏目序列组织（栏目名、顺序、题量以委托书为准）；开头【锚点清单】仅声明覆盖范围，不得据此替代栏目结构；素材来源不限（教材或课外真实生活皆可，主题相关、难度适切）。\n\n';
    }
