...
// ==================== 🔧 整卷生成（DeepSeek 云端主路径）====================
  ...
  const _runPaperOrder = async (params = {}) => {
    ...
    if (compressedText || anchorListText) {
      ...
      prompt += '【素材使用约定】\n'
        + '· 编写/命题范围以开头【锚点清单】为准；可依据的教材原文以中段【压缩原文】为准，不得凭训练记忆补写范围外内容；\n';
      prompt += refMode === 'full'
        ? '· 本资料为知识归纳型：可引用、可归纳【压缩原文】，但须转写为教辅表述，不得整段照录；正文不得出现任何出处标注（"选自/单元/章节/课题/课文名/位置式指引/原文出处"等溯源字样一律不写）。\n'
        : '· 本资料为命题/练习型：【压缩原文】仅供参考——仅供理解题型结构、知识梯度与难度；题干、情境、人名、数据与句式一律自拟，禁止沿用原文连续字面。\n';
      ...
      //    ✅ §4.1 极短指针（非重复）：覆盖范围以开头【锚点清单】为准、素材以中段【压缩原文】为准（不复述清单内容）。
      prompt += '【组织方式】输出一律以委托书【教辅结构】的栏目序列组织（栏目名、顺序、题量以委托书为准）；开头【锚点清单】仅声明覆盖范围，不得据此替代栏目结构；素材以中段【压缩原文】为准。\n\n';
    }
    ...
    //    内容是否在正文真实足量存在、本题能否仅凭卷面自足完成」，**不得改回形态/内容名称清单**
    //    （清单必不完备、且构成诱导；实测曾因列举漏项而失守）。
    prompt += `\n\n【尾约束·全文自洽】
题干所声明的、本题作答所必需的一切内容（不论其形态），都必须在正文中真实、足量、形式吻合地存在——使本题**仅凭卷面自身即可完成**；凡题干提到而卷面未给出、或给出但不完整、不足以支撑该设问的，即属无效内容，须于定稿前修正。素材依课标可取自教材之外的真实情境、不限所选教材，但题面引用或呈现的素材必须与本题实际给出的内容完全一致；正文不得先现待作答结论；正文对知识、要点、示例、数据、结论的归纳与转述须准确、完整、不遗漏、条理清晰，各题、各要点、各结论及其答案之间前后一致、互不矛盾。凡题干与正文不符，或内容漏错、自相矛盾，即为无效内容，须于定稿前修正。`;
...
同一份资料内各栏目呈现形式与组织顺序应有所差异，不得全份同类版式照搬；同一呈现方式与同一组织顺序不可逐栏、逐单元反复套用。`;
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        ...
        content = normalizeIndents(normalizeLeadingMarkers(normalizeMatchQuestions(stripRedundantInlineCarrierRows(normalizeMathCircleBlanks(normalizeBlankMarkers(cleanSectionHtml(respObj.content || '')))))));
