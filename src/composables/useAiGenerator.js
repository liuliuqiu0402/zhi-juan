...
// ==================== 🔧 整卷生成（DeepSeek 云端主路径）====================
  ...
  const buildBrowseSystem = (contentMode, anchorList = [], genType = '') => {
    ...
    contentMode
      ? '· 本资料为知识归纳型：browse 取回的原文须先归纳转写后再呈现（可用自己的话重组要点），不得整段照录、不得把原文整句誊进正文；若保留溯源，出处只用“位置式指引”——写所在 单元/章节/课题/栏目名（如 三上·第1单元·小数乘法、课文《…》），只给位置、不写页码（正文为手工粘贴无页码可依，页数是猜测不准确），且严禁把出处写成对原文的复述（出处行不得与正文要点重复）；'
