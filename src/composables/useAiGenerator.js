...
export function useAiGenerator() {
  ...
  const GEN_TYPE_LABEL_POOLS = {
    exam: ['综合检测', '测试卷', '阶段测评'],
    practice: ['同步练习', '同步训练', '巩固练习'],
    special: ['专项突破', '专项训练', '强化练习'],
    preview: ['预习导学', '预习导航', '预习单'],
...
条目性质 kind（每条 coreKnowledge 必标，取值二选一：knowledge / material）。判据只看一条：**这条能不能直接变成一道题的考查点？** 能 → knowledge；它本身只是材料或背景 → material。自检两类误判：把"本身就能成题的考查点"标成 material（覆盖不足）；把"只是材料或背景"当考查点（逼着重述材料）。标 material **不减少任何提取内容**，仍逐条提取，只影响生成期的覆盖口径；kind 缺失时一律按 knowledge 处理。
...
// ==================== 🔧 整卷生成（DeepSeek 云端主路径）====================
  ...
  const _runPaperOrder = async (params = {}) => {
    ...
    //    'once'  一次成型：正文+答案一次输出（上下文全程一致，知识型/错题/默写推荐）
    //    'auto'  自动按资料类型（两条路都可用）：纯题型 → split；知识型/默写/错题 → once
    ...
    //    任务式教辅（同步练习/专项/复习）模型仍可能把《参考答案》写进正文末尾——
