...
const extractContentCards = async (selectedBooks, callAI, robustJsonParse, updateStatus) => {
  ...
  //    目录（章节标题 + 子标题）本身就是知识点线索，确保"仅勾选目录"也能生成：
  //    Step2 知识图谱基于目录标题 + 学科课标知识补全知识点，生成端按指令分配知识点，AI 无素材凭空生成。
    ...
    for (const chapter of chapters) {
      ...
      if (chapter.analyzed && chapter.knowledgeHierarchy?.length > 0 && !textChangedSinceAnalysis) {
        ...
        contentCards.push({ chapterTitle: chapter.title, summary: chapter.coreTopics || displayKps.slice(0, 5).join('、'),
          ...
          // 🔧 覆盖锚（2026-09）：章级 knowledgeHierarchy 完整树随卡附带——知识点→章归属由树结构天然成立，
...
1. 知识点清单（去重，不超过30个）
2. 重难点判断（不超过8个）
3. 层级知识图谱：单元→大概念(≤5)→核心知识点[knowledge|material](≤6)→具体概念(≤4)
4. 跨章节关联（不超过10条）
...
/**
 * 非思考单次对话助手：单次非流式对话，显式关闭引擎思考模式（原文压缩等底层调用复用）。
 * 复用应用内用户填写的生成引擎配置（P1：不外部接管凭据）；调用失败由调用方降级处理。
 * @param {Array<{role:string, content:string}>} messages
 * @returns {Promise<string>}
 */
export async function chatNonThinkingOnce(messages = [], { maxTokens = 8000, temperature = 0.3, timeoutMs = 0 } = {}) {
  ...
  if (!content) throw new Error('非思考对话返回为空（引擎无内容，可能思考配额耗尽或模型不支持）');
  return content;
}
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (config.engine === 'ollama') {
          ...
          // 🔧 会话前缀：调用方注入的历史消息（如有）随请求携带，
          //    写作/答案页请求与前置请求同源同序（零注入、只追加）
...
条目性质 kind（每条必标，二选一）：material＝单元主题、学习目标、语篇/板块类**概述性条目**（生成时只作理解与难度依据，不列入命题覆盖单位）；knowledge＝可独立成题的学科知识点、技能或方法。
...
- 🔬 **条目性质 kind（2026-09-14 新增，供生成期分流）**：每条 coreKnowledge 须标 kind（取值二选一）：
  · knowledge＝**知识性条目**：词汇、句型与语法、语音、概念、规则、方法、技能等**可独立命题**的知识单位；
  · material＝**语言材料·语篇条目**：课文/语篇要点、故事情节、板块主题、活动话题等"材料性"内容——
    它们照旧逐条提取、信息量不减，但生成时**只作理解与难度依据，不列入命题覆盖单位**。
  · 判据（一句话）：**这条能不能直接变成一道题的考查点？** 能 → knowledge；只是"这段材料讲了什么/属于哪个板块" → material。
  · 非语言学科（数学/物理/化学等）通常全为 knowledge；kind 缺失时一律按 knowledge 处理（向后兼容）。
- **最小单位强制下沉第3层**：单个字、单个词条、单个符号、单个数值、术语碎片 → 一律放进对应知识点的 specificConcepts，**不得提为第2层条目**
- 反例自检：第2层出现"人 / 口 / 手"这类单字名 → 违规，须改为下沉（正确形态：第2层「识字与书写」，第3层 ["人","口","手"]）
...
- 📝 生字/生词：逐个提取（不合并、不遗漏），但**下沉到第3层 specificConcepts**（归属第2层「识字与书写」等知识点）；单字/单词条**不得提为第2层条目**
...
- 🔒 必须逐条标注，绝不将多个知识点合并为一条（如"生字5个"→逐个提取5条）；但**最小单位一律落第3层**——如生字"人""口""手"→ 第2层「识字与书写」+ 第3层 ["人","口","手"]（既不合并不遗漏，也不提为第2层条目）
${isLowerGrade ? '- 🔧 低段(1-2)：识字与写字（汉语拼音、基本笔画、常见偏旁部首与汉字）、阅读浅近童话寓言并诵读优秀诗文、写话与口语交际、围绕身边事物提问与梳理\n' : ''}${isMidGrade ? '- 🔧 中段(3-4)：独立识字与书写、把握叙事性作品主要内容与思想感情、不拘形式写下见闻感受的习作、口语交际与有目的地搜集资料\n' : ''}${isUpperGrade ? '- 🔧 高段(5-6)：默读与浏览、阅读叙事性作品与说明性文章、诵读积累优秀诗文、写记实与想象作文并学写读书笔记和常见应用文\n' : ''}${isJunior ? '- 🔧 初中：阅读浅易文言文与古诗文、欣赏文学类作品并领悟内涵、写作（真情实感、讲求文体）、名著阅读与梳理探究\n' : ''}${isSenior ? '- 🔧 高中：语言建构与运用、思辨性阅读与表达、文学阅读与写作、实用性阅读与交流、文化传承与理解（必修学习任务群）\n' : ''}`;
  } else if (isMath) {
...
// ==================== 🔧 整卷生成（DeepSeek 云端主路径）====================
   ...
   * - 章节目录（前部）：告知覆盖范围
   * - 原文片段（中部）：每知识点保底 top1 片段（保证每个知识点都有原文依据，防相关度排序
   *   把后段知识点的片段挤掉）；未命中片段的知识点自动拆词补检（覆盖度检查，静默 debug）；
   *   剩余预算按相关度增量补充；总量受 maxChars 上限兜底（丢的是无关片段，不是知识点原文）
  ...
  const _runPaperOrder = async (params = {}) => {
    ...
    const anchorMissingNote = anchorReport.missingList?.length
      ? `⚠️ 覆盖缺料：${anchorReport.missingList.length} 项核心知识所在章节无教材片段（该知识点已列入【锚点清单】，但缺少教材原文依据：只能依**课标学业要求**把握（课标为跨版本统一依据）；涉及本套教材特有的术语/编排/学段位置等说法，不得凭记忆断言，守【质量底线】教材版本口径）：${anchorReport.missingList.map((m) => `${m.chapter}·${m.name}`).join('、')}。请检查勾选章节的原文解析/粘贴是否完整（补齐原文即恢复依据）。`
      : '';

    // ── 材料来源（A15/A4 · 2026-09 定稿）：程序直读勾选章节整章原文 → 压缩（Map→Reduce）──
    //    取消 browse：原文不再"模型按需逐点取片段"，而是"程序直读整章原文再压缩"。
    //    由此没有工具轮、没有上下文累积（单次请求）；委托书不再拼接 basis/ref 素材区。
    //  📏 本量的**取值口径**（2026-09-14 用户问「预算用的是章节名还是真原文」，此处留档避免再猜）：
    //    它 = 各内容卡片 `segments[].text` 的字数之和，而 segments 的来源按**卡片类型**分三种——
    //      ① 已分析章（主流）：`splitTextIntoSegments(cleanRawText, 500)` 切分合并而来
    //         → **真原文**（含 [FORMULA]/[TABLE]/[HEADING] 标记的少量膨胀）；与送压缩的那份文本同源；
    //      ② 有原文但未分析（降级目录模式）：`buildTocCard(..., rawText)` → segments = [目录文本, **真原文**]
    //         → 仍是真原文（另加目录文本）；
    //      ③ 纯目录章（压根没有原文）：`buildTocCard(chapter, 'toc')` → segments = [目录文本]
    //         → **章节名 + 子标题**（此时不存在原文，预算只能按目录量估，会落到预算地板）。
    //    结论：**只要该章有原文，预算用的就是真原文字数**——锚清单通道不注入原文，但原文量照算（A19 口径解耦）；
    //    **锚清单自身的字数不参与预算**（清单只用于注入）。纯目录章是唯一"以标题代替原文量"的情形，两通道一致。
    const selectedRawChars = (contentCards || []).reduce(
      (s, c) => s + (c.segments || []).reduce((ss, p) => ss + (p && p.text ? p.text.length : 0), 0), 0);

    // ── 锚点清单（A1-4）+ 压缩原文（A4）：写作期前缀「锚点清单 → 压缩原文 → 委托书」的两件素材 ──
    //    锚点清单按章分组（章序=勾选章序=原文章序），只在清单出现一次；
    ...
    if (materialChannel === 'anchor') {
      console.log(`📌 [素材通道] ${genType} → 锚清单注入（锚点清单 ${anchors.length} 条（含第3层具体概念）+ 难度要求；不注入整章原文）`);
    }
    ...
    if (directInject) {
      ...
      try {
        ...
        compressedText = rawSections.map((sec) => [sec.title, sec.text].filter(Boolean).join('\n')).join('\n\n');
        console.warn(`⚠️ [原文压缩] 异常（${e?.message || e}）→ 回退整章原文`);
      }
    } else {
      console.log('📚 [写作通道] 无勾选章节原文（材料为空），本次仅凭锚点清单+委托书写作');
    }
    } else if (rawSections.length) {
      // ✅ A19（2026-09-14 素材通道）：锚清单通道**可观测性**——不注入整章原文时无压缩等待，
      //    进度/状态照写（否则进度条看不出本次走了哪个通道、通道是否生效）。
      progress.value = 14;
      statusText.value = '素材通道=锚清单注入：不注入教材原文，按【锚点清单】（含各知识点具体概念）把握范围与难度...';
      console.log('📚 [写作通道] 素材通道=锚清单注入：不注入整章原文（仅【锚点清单】（含第3层具体概念）+ 难度要求）');
    } else {
      console.log('📚 [写作通道] 无勾选章节原文（材料为空），本次仅凭锚点清单+委托书写作');
    }
    ...
    //  ✅ A19（2026-09-14 用户定·口径解耦，**改前必读**）：**预算口径与素材通道无关**——
    //    锚清单通道虽不注入整章原文，但仍**一律按"勾选原文量"**推算（`selectedRawChars` 在上方
    //    通道分支**之外**计算，取的是内容卡段文本总字数，与是否注入/是否压缩无关；
    //    `collectChapterRawText` 同样始终执行）。理由：原文量是"本次覆盖工作量"的客观代理量，
    //    通道只决定"素材进不进指令、省不省一次压缩调用"，不改变工作量。故两条通道的预算一致，
    //    便于跨通道对比与费用预估；**不得**因通道改名/改实现而改口径（[每类型预算] 日志两通道都打印
    //    「勾选原文=N字」，即为该不变量的可核对证据）。
    ...
    //    等只在 console 的补救事件，成功交付后同样透出到生成报告【问题列表】
    let bodyPathNotes = [];
    ...
    if (skipAnswerPage) {
      ...
      try {
        ...
        const selfContainedAnsNote = isSelfContainedTeaching
          ...
          : '';
        // 🔧 组织口径按类型分流：题类（exam/课时练/专项等）按"大题与题号"层级；自包含教辅（summary/review/
    ...
    // ✅ A9（2026-09-11 清理）：覆盖对账器废除后，正文覆盖是否呈现由正文生成期决定，
    //    报告不再做事后"字面撮合"的覆盖点→题映射（见下 P2b 说明）。
    ...
    //    旧口径只取"锚绑定片段 + 返回性段类型过滤"，会把练习段与未标注段漏出比对范围；
  ...
  const generateFullPaperNatural = async (params = {}) => _runPaperOrder(params || {});
...
`;
            ...
            //    卷内主题集中由卷级守门词频检测兜底（命中进【问题列表】交编辑核对），程序不再生成任何情境内容。
            contextFramework = '';
          }
        } catch (e) {
