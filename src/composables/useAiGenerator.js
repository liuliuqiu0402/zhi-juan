...
// ==================== 🔧 整卷生成（DeepSeek 云端主路径）====================
  ...
  const _runPaperOrder = async (params = {}) => {
    ...
    const anchorMissingNote = anchorReport.missingList?.length
      ? `⚠️ 覆盖缺料：${anchorReport.missingList.length} 项核心知识所在章节无教材片段（该考点已列入【锚点清单】，但缺少教材原文依据：只能依**课标学业要求**把握（课标为跨版本统一依据）；涉及本套教材特有的术语/编排/学段位置等说法，不得凭记忆断言，守【质量底线】教材版本口径）：${anchorReport.missingList.map((m) => `${m.chapter}·${m.name}`).join('、')}。请检查勾选章节的原文解析/粘贴是否完整（补齐原文即恢复依据）。`
      : '';
