...
export const INSTRUCTION_VERSION = 35;
...
export const builtinInstructions = [
  ...
  },
  {
    ...
    subject: '', stage: 'high',
    content: '高中试卷补充：按高考题型结构设计；前80%为基础和中档题确保区分度，后20%为提高和压轴题；融入少量高考真题改编（占比不超过20%）和学科核心素养考查',
    builtin: true
  },
  ...
  // ── 课时练 ──
  {
    ...
    subject: '', stage: 'primary',
    content: '小学课时练补充：基础部分以模仿教材原题为主，能力部分对原题变式（变换数据/问法/场景），拓展部分设计游戏化或生活化任务；图文结合，题干表述口语化',
    builtin: true
  },
  {
 ...
 // ── 知识点总结 ──
  ...
  // ── 专项突破 ──
  {
    ...
    subject: '', stage: 'primary',
    content: '围绕某一专项能力进行深度训练。结构：趣味导入→方法讲解→典例剖析→变式训练→挑战自我。方法讲解配合图示或口诀，每类题型配3-5道阶梯训练。',
    builtin: true
  },
  {
    id: 'full_special_middle', name: '🎯 初中专项突破', category: '专项突破', type: 'full',
    subject: '', stage: 'middle',
    content: '围绕某一专项能力进行深度训练。结构：方法指导→典例剖析→变式训练→真题实战。重点关注中考高频考点和常见失分点，每类题型设置3-5道变式。',
    builtin: true
  },
  {
  ...
  // ── 课前预习 ──
  {
    ...
    subject: '', stage: 'primary',
    content: '小学课前预习补充：语言亲切，配上小提示和鼓励语；用生活化语言引出课题，激发学习兴趣；预习检测题量3-5题，以基础判断和简答为主，不设过难题目',
    builtin: true
  },
  // 语文学科专属：生字预习规范
  {
    ...
    content: '小学语文预习生字专项：\n\n【生字格式——必须遵守】\n每个生字独立用<span class="tian-zi-ge">字</span>包裹，附带完整信息：<span class="tian-zi-ge">蝌</span>(部首：虫，15画，左右结构)。禁止只写字和拼音不写部首/笔画/结构！所有括号一律用英文半角 ( )，严禁中文全角括号！',
    builtin: true
  },
  {
...
- 🚫 禁止用"XX学业测评""XX综合测试""第一单元测试"等非正式标题充当卷首大标题；标题居中，标题下一行居中写"（考试时间：X分钟　满分：X分）"。`,
  ...
  },
  {
    ...
    content: '【生成要求】\n1. ⚠️ 严格遵守"典型错题→精准归因→正确解法→变式巩固→方法归纳"五步流程，每个错误类型必须完整走完整个流程，不可跳过任何步骤。\n2. 错题覆盖要全面：涵盖概念混淆、计算失误、审题偏差、方法误用等各类典型错误，每个错误类型至少配一道变式巩固练习。\n3. 归因分析要具体深入：明确指向错误的知识点根源或思维偏差（如"未理解进位加法中\'满十进一\'的规则"），严禁笼统归因为"粗心""不认真""不会做"。归因须显式归入四类之一并标注：【知识漏洞】【思维误区】【审题偏差】【习惯问题】。\n4. ⚠️ 填空格式见【输出格式-填空留空】规范（句内横线<u class="blank-N">&emsp;</u>、独立括号<span class="blank-N">&emsp;</span>，互斥不叠加，严禁空标签与下划线）。5. 🚫 这不是试卷，不要在标题或题目中标注分值（不要出现"(共X分)""(X分)""每题X分""满分X分"等）。\n6. 如需配图（如错题原题截图示意等），按【EduRender模板-配图】的完整 [IMAGE] 格式输出（TYPE:SD，详见该节模板）；⚠️ 配图以必要为原则，能用文字描述清楚的不要配图。\n7. ⚠️ 文末必须包含完整的参考答案区域（<div class="answer-section">），所有错题的正确解法、变式练习答案和方法归纳都需要提供，不得省略。\n8. 每道错题标注来源（教材页码/课时练习/阶段检测）。\n9. 🔴 错题回访区：文末须附"错题回访区"，将各题变式巩固题编号列出，提示学生 3 天后、1 周后各复做一次并自我评分（√/△/×），形成间隔复习闭环（错题本的核心价值）。\n10. 直接返回完整HTML片段，严禁用```html或<html>/<body>包裹。',
    builtin: true,
    guarantee: true
  },
  {
    ...
    content: '【生成要求】\n1. ⚠️ 大题结构以【真题卷结构蓝本】为唯一依据：大题序列、名称、分值、顺序完全一致，不得增删大题、不得另行包装为"基础/能力/综合"板块（蓝本即真题卷通行结构）；卷首"（考试时间：X分钟　满分：X分）"必须与蓝本一致，严禁更改时长或满分。知识点与认知层级仅用于内部组卷设计，🚫 卷面严禁输出任何知识点/层级标注。\n2. 难度分布：{diffRatio}，题目从易到难排列。\n3. ⚠️ 大题内部小题形式保持多样（填空/选择/判断/连线/简答/补全对话/仿写/造句轮换），严禁全部或绝大多数使用选择题。\n4. ⚠️ 填空格式见【输出格式-填空留空】规范（句内横线<u class="blank-N">&emsp;</u>、独立括号<span class="blank-N">&emsp;</span>，互斥不可叠加，严禁空标签与下划线）。书写类填空（看拼音写词语/选字填空/量词填空/按课文内容填空/结构分类等）一律用横线 <u class="blank-N">&emsp;</u>；括号 span.blank-N 仅限选择/判断题答案空。标签内必须填 &emsp;（严禁真空或只放一个空格）。\n5. 禁止"下列说法正确的是""以下哪个选项是正确的""以上都是/都不对"等低质量设问。\n6. 题干必须置于真实情境或明确任务中（生活/社会/科学场景，或与整卷主题情境衔接），情境须支撑作答；禁止空洞抽象、"戴帽子"假情境（题干前加无关导语）、与其他题雷同。\n7. ⚠️ 文末必须含完整 <div class="answer-section"> 答案与解析区，所有题目（含选择题）都有答案和解析。\n8. 直接返回完整HTML片段，严禁```html或<html>/<body>包裹。\n9. 🔴 分数标注（正规考试卷规范，分散标注、不堆砌）：大题标题统一格式"一、×××（本大题共X小题，每小题X分，共X分）"（如"一、单项选择题（本大题共10小题，每小题4分，共40分）"）；客观题/同分大题用此式；主观题大题（各小题分值不同）标题写"（本大题共X题，共X分）"（如"三、阅读与鉴赏（本大题共5题，共14分）"），严禁把各小题分值说明堆在大题标题；单题大题写"（本大题共1题，共X分）"。小题有题号/标题的：题号后标"（X分）"（如"1.口语交际（10分）"）；小题无标题（题干型）的：题干末尾标"（X分）"（如"……的感情是（____）（2分）"）。🔴 所有小题（不分题型：选择/判断/填空/阅读/简答/写作等一律标注）都必须在题号后或题干末尾标注"（X分）"，严禁任何小题漏标——大题标题的"每题X分"与小题标注并存、互为印证，二者都不可省略。各小题分值合计必须=大题总分，各大题合计=卷面满分；分值一律为整数（严禁0.5分等小数）。\n10. 🔴 多空小题分值标注（正规试卷规范，两级并存）：一题含多空（如默写/看拼音写词语/选字填空）时，小题题干后必须注明"（每空X分）"（如"默写。（每空1分，共10分）"）——与大题标题的"每小题X分"并存、互不替代：大题标题说明小题分值，小题题干说明每空分值，两级都标才清晰；每空分值不同时逐空标注（尽量设计为每空同分）；每空分值×空数必须=该小题分值；🔴 小题分值必须能被空数整除（每空整数分）：3 空题分值只能是 3/6/9…分，严禁出现"3 空 4 分"式无法整除的分值——若设计分值不能整除空数，必须调整每空分或题分值（如 3 空 4 分应改为每空 1 分共 3 分，或每空 2 分共 6 分）。\n11. 🔴 答案分布随机性强化（按学段/题型通用）：单项选择题正确答案全卷随机均匀——n 选项时每选项出现次数尽量相等（3 选项约各 1/3、4 选项约各 1/4），严禁单一选项占比≥50%（如 8 题中 4 题同选项）；同一选项连续出现不超过 3 次；学段选项数惯例：低段（1-2年级）2-3 选项、中高段及以上 4 选项，分布规则不受选项数影响。',
    builtin: true,
    guarantee: true
  },
  {
    ...
    content: '【生成要求】\n1. ⚠️ 任务形式多样化：每个学习任务采用不同的活动形式（读/写/说/做/演/画/讨论/探究等），避免全部是纸笔答题。任务之间体现多样化学科实践。\n2. ⚠️ 任务数充足：全卷至少8-12个学习任务，根据选中章节的知识点密度灵活决定。每个核心知识点至少体现为一个独立任务或任务环节，确保全面覆盖。\n3. 难度递进：{diffRatio}，任务从易到难排列（基础建构→合作探究→迁移创新）。\n4. ⚠️ 填空格式见【输出格式-填空留空】规范（句内横线<u class="blank-N">&emsp;</u>、独立括号<span class="blank-N">&emsp;</span>，互斥不叠加，严禁空标签与下划线）。5. 禁止使用"下列说法正确的是""以下哪个选项是正确的""以上都是""以上都不对"等低质量设问。\n6. 每个学习任务的题干必须有具体情境（生活场景/学科情境/真实问题），不得空洞、抽象或与其他任务雷同。\n7. 如需配图（看图选词/看图连线/看图写话等），按【EduRender模板-配图】的完整 [IMAGE] 格式输出（TYPE:SD，详见该节模板），不要只写"看图"而不配标记；⚠️ 配图不宜过多——仅在看图确实为任务必需时才配图。\n8. 🚫 严禁在任何位置出现分值信息：任务不标分数、成果不标注得分点/分值比重、解析不给评分维度。这不是试卷，是学习任务单，不需要任何分值体系。\n9. ⛔ 成果与任务严格分离：每个任务的成果展示/答案只出现在文末 <div class="answer-section"> 区域内，严禁在任务后直接附答案。任务区只出现任务本身，不要出现任何答案内容。\n10. ⚠️ 文末必须包含完整的答案与成果区（<div class="answer-section">），所有任务都要有参考答案或成果示例。\n11. 直接返回完整HTML片段，严禁用```html或<html>/<body>包裹。',
    builtin: true,
    guarantee: true
  },
  ...
  // ═══════════════════════════════════════
  {
    id: 'topconst_review', name: '【顶层约束】复习', category: '生成-顶层约束', type: 'fragment',
    subject: '', stage: '', genType: 'review', prompt_order: 4,
    content: '【生成要求】\n1. ⚠️ 考点覆盖完整：覆盖所选范围全部核心考点和易错点，不得遗漏也不得超纲。\n2. 结构化组织：按上方【结构大纲】组织内容，知识梳理→典型例题→易错辨析→综合自测四段递进。⚠️ 各板块标题须直接含检测字样：知识体系板块标题须含“知识框架/思维导图/知识树/知识地图”等字样（不得用“知识回顾”“重点闯关”等替代词），题目板块标题须含“典型题”或“例题”字样（低段可用“第一关·典型题”等趣味包装，但字面必须保留）。\n3. ⚠️ 典型例题充分：每个核心考点至少配1道典型例题，含完整解答和解析。\n4. ⚠️ 综合自测覆盖全面：综合自测题覆盖所选范围全部考点，按考点权重合理分配题量。\n5. 🚫 不要在题目或标题中标注任何分值（不要出现"(共X分)""(X分)""每题X分""满分X分"等），复习资料不是考试卷。\n6. 如需配图，使用标准 [IMAGE] 标记格式。⚠️ 配图以"必要"为原则，能用文字描述清楚的不要配图。\n7. ⚠️ 文末必须包含完整的答案与解析区域（<div class="answer-section">），所有题目都要有答案，易错辨析要有正误对比。\n8. 直接返回完整HTML片段，严禁用```html或<html>/<body>包裹。',
    builtin: true,
    guarantee: true
  },
...
4. 情境化示例：裸题"看拼音写词语：nǎ lǐ → _____"→情境化"小蝌蚪迷路了，请帮它把拼音转成汉字：'妈妈，你在 nǎ lǐ _____？'"；裸题"花( )里开满了鲜花"→情境化"秋游来到花园，请帮小朋友选字：花( )里开满了鲜花。备选字：园 圆 丛 从"`, 
  ...
  // ═══════════════════════════════════════
  // 课标骨架对齐（R1/R2：骨架锚定新课标学业要求 + 骨架下内容不交叉）
  // 生成端强制注入，按「学段 × 资料类型」三维度精准匹配（学科维度由"生成-学科核心素养"块承载，不重复注入）：
  // 骨架板块必须锚定对应学段的课标学业要求，板块内容与骨架一一对应、考查维度不交叉
  // ═══════════════════════════════════════
  {
    id: 'skeleton_align_primary_low', name: '课标骨架对齐-小学低段', category: '生成-课标骨架', type: 'fragment',
    prompt_order: 34,
    subject: '', stage: 'primary_low', genType: 'exam,practice,special,reading,errorbook,preview,dictation,review,summary',
    content: `【课标骨架对齐-小学低段】
1. 骨架锚定课标学业要求：低段课标强调基础性、趣味性与生活化——板块目标显式对应基础读写算的达成（如"基础巩固"对应基础掌握、"拓展"对应生活化简单运用），严禁为凑结构而生造与课标无关的板块。
2. 板块内容与骨架一一对应：每个板块只承载该板块的课标目标内容，禁止板块间内容交叉——"基础巩固"不出现方法迁移题、"能力提升"不重复基础考点。
3. 相邻板块的考查维度不得重叠（低段以识记与简单理解为界，不越入抽象迁移）。`,
    builtin: true,
    guarantee: true
  },
  {
    id: 'skeleton_align_primary_mid', name: '课标骨架对齐-小学中段', category: '生成-课标骨架', type: 'fragment',
    prompt_order: 34,
    subject: '', stage: 'primary_mid', genType: 'exam,practice,special,reading,errorbook,preview,dictation,review,summary',
    content: `【课标骨架对齐-小学中段】
1. 骨架锚定课标学业要求：中段课标强调从识记走向理解与初步运用——板块目标显式对应（"基础巩固"对应基础达成、"能力提升"对应理解运用、"拓展"对应情境化综合），严禁为凑结构而生造与课标无关的板块。
2. 板块内容与骨架一一对应：每个板块只承载该板块的课标目标内容，禁止板块间内容交叉——"基础巩固"不出现方法迁移题、"能力提升"不重复基础考点。
3. 相邻板块的考查维度不得重叠（中段以理解、运用为界，不越入抽象迁移）。`,
    builtin: true,
    guarantee: true
  },
  {
    id: 'skeleton_align_primary_high', name: '课标骨架对齐-小学高段', category: '生成-课标骨架', type: 'fragment',
    prompt_order: 34,
    subject: '', stage: 'primary_high', genType: 'exam,practice,special,reading,errorbook,preview,dictation,review,summary',
    content: `【课标骨架对齐-小学高段】
1. 骨架锚定课标学业要求：高段课标强调综合运用与初小衔接——板块目标显式对应（"基础巩固"对应基础达成、"能力提升"对应方法迁移、"拓展"对应综合情境任务），严禁为凑结构而生造与课标无关的板块。
2. 板块内容与骨架一一对应：每个板块只承载该板块的课标目标内容，禁止板块间内容交叉——"基础巩固"不出现方法迁移题、"能力提升"不重复基础考点。
3. 相邻板块的考查维度不得重叠（高段可至综合运用，为初中衔接预留边界）。`,
    builtin: true,
    guarantee: true
  },
  {
    id: 'skeleton_align_middle', name: '课标骨架对齐-初中', category: '生成-课标骨架', type: 'fragment',
    prompt_order: 34,
    subject: '', stage: 'middle', genType: 'exam,practice,special,reading,errorbook,preview,dictation,review,summary',
    content: `【课标骨架对齐-初中】
1. 骨架锚定课标学业质量要求：初中课标以核心素养为纲（语言运用/科学思维/实践探究等学科素养）——板块目标显式对应学业质量水平描述（如"基础巩固"对应水平一达成、"探究拓展"对应水平二应用），严禁为凑结构而生造与课标无关的板块。
2. 板块内容与骨架一一对应：每个板块只承载该板块的课标目标内容，禁止板块间内容交叉——"基础巩固"不出现方法迁移题、"能力提升"不重复基础考点。
3. 相邻板块的考查维度不得重叠（初中以理解、应用、分析为界，不越入纯学术迁移）。`,
    builtin: true,
    guarantee: true
  },
  {
    id: 'skeleton_align_high', name: '课标骨架对齐-高中', category: '生成-课标骨架', type: 'fragment',
    prompt_order: 34,
    subject: '', stage: 'high', genType: 'exam,practice,special,reading,errorbook,preview,dictation,review,summary',
    content: `【课标骨架对齐-高中】
1. 骨架锚定课标学业质量水平要求：高中课标学业质量分四级——板块目标显式对标对应水平（如"基础巩固"对应水平1-2达成、"综合应用"对应水平3-4），严禁为凑结构而生造与课标无关的板块。
2. 板块内容与骨架一一对应：每个板块只承载该板块的课标目标内容，禁止板块间内容交叉——"基础巩固"不出现方法迁移题、"能力提升"不重复基础考点。
3. 相邻板块的考查维度不得重叠（高中允许迁移、综合、创新，但各板块仍守住自身维度边界，不越界混考）。`,
    builtin: true,
    guarantee: true
  },
  {
    id: 'skeleton_align_exam', name: '课标骨架对齐-试卷', category: '生成-课标骨架', type: 'fragment',
    prompt_order: 34,
    subject: '', stage: '', genType: 'exam',
    content: `【课标骨架对齐-试卷】
1. 大题骨架以【真题卷结构蓝本】为唯一依据，每个大题须锚定课标学业质量要求（如"积累与运用"对应语言积累与梳理的学业要求、"阅读"对应阅读与鉴赏能力），不得无依据自创大题或改换大题名称。
2. 小题考查维度与所属大题目标一致：积累类大题只出积累与运用题、阅读类大题只出基于文本的理解与鉴赏题，禁止跨大题混搭（如阅读大题内嵌入脱离文本的字词填空）。
3. 同一知识点不得跨大题重复考查（如"近义词"已在积累题考查，阅读题不再重复设问）；全卷考查维度不交叉、不遗漏。`,
    builtin: true,
    guarantee: true
  },
...
4. 结构完整：标题层级清晰、板块命名与结构大纲一致、答案与解析完整且与题目一一对应。`,
    builtin: true,
    guarantee: true
  },
  ...
  // ═══════════════════════════════════════
  {
    ...
    content: `【题目充实度要求——小学】
1. 阅读短文最低长度：课外短文120-180字（低段课内选文以教材原文为准，不受此限）
...
4. 主观题引导：开放题给思考方向提示，写话题提供词语支架`, 
  ...
  },
  {
    ...
    content: `【题目充实度要求——初中】
1. 阅读短文最低长度：≥400字；材料分析题需提供完整背景文本（如历史材料题至少2段史料）。
2. 大题（综合题/实验探究/材料解析/完形填空等）题干须包含充分的背景信息：
   - 理科计算题：完整的已知条件和情景描述（2-4句话），不要"已知a=1求b"
   - 文科材料题：至少提供1-2段真实文献/数据/图表，问题需基于材料设问
   - 实验探究题：完整的实验目的→器材→步骤→数据→结论框架
3. 选择题：选项应有区分度，干扰项为常见错误思路而非随意编造
4. 主观题引导：给思考角度提示（如"从XX和XX两个角度分析"），开放性试题需有评价量表说明`, 
    builtin: true
  },
  {
    ...
    content: `【题目充实度要求——高中】
1. 阅读/材料文本最低长度：≥500字；论述类文本需完整呈现论证逻辑链。
2. 大题题干必须充分、对标高考真题的文本量：
   - 数理化解答题：完整的真实情景描述（3-5句背景）+ 所有已知条件 + 明确求解目标，对标高考答题规范
   - 语文现代文/文言文阅读：选文长度不得低于高考真题标准，问题需覆盖信息提取→分析鉴赏→探究评价三层
   - 英语阅读理解：每篇250-400词，选项长度均衡
   - 政史地材料题：提供2-3段真实文献/图表/数据，问题须设从"概括"到"评述"的梯度
   - 生化实验探究：完整的科研情景→实验设计→数据呈现→分析论证框架
3. 选择题：4个选项均为合理干扰（非凑数），错误选项反映典型思维误区
4. 主观题：开放性试题须有评价等级描述（如"观点明确3分+论证充分4分+语言表达3分"）`, 
    builtin: true
  },
  ...
  { id: 'subject_history', name: '[历史] 史料实证+时序梳理', category: '生成-学科特色', prompt_order: 18, type: 'fragment', subject: '历史', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review', content: '历史资料应包含史料分析题（文字/图片/表格史料）、时序梳理题（时间轴/大事年表）；注重论从史出、史论结合。\n- 禁止历史事件的时间、地点、人物张冠李戴\n- 禁止用现代价值观简单评判历史事件\n- 禁止史料分析题使用虚构史料\n- 注重论从史出、史论结合\n', builtin: true },
  { id: 'subject_geography', name: '[地理] 地图技能+区域认知', category: '生成-学科特色', prompt_order: 18, type: 'fragment', subject: '地理', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review', content: '地理资料应包含地图判读题（等高线/气候图/政区图）、区域比较分析题；注重人地协调观和空间思维能力。\n- 禁止编造地理数据（经纬度、海拔、面积等须真实可查）\n- 禁止混淆地理概念（如天气与气候、地形与地势）\n- 禁止地图缺少比例尺、图例、方向标\n', builtin: true },
  ...
  // ═══════════════════════════════════════
  {
    ...
    content: '请为每道题目提供答案要点和简要解析（正式考试答案册样式：客观题附简析、主观题附评分要点，禁止"易错提示""命题思路"等教辅栏目）。',
    builtin: true
  },
  ...
  // ═══════════════════════════════════════
  {
    ...
    prompt_order: 38,
    subject: '', stage: 'primary', genType: 'exam,practice,special,reading,summary',
  ...
  },
  {
    ...
    prompt_order: 38,
    subject: '', stage: 'middle', genType: 'exam,practice,special,reading,summary',
  ...
  },
  {
    ...
    prompt_order: 38,
    subject: '', stage: 'high', genType: 'exam,practice,special,reading,summary',
  ...
  },
  {
    ...
    content: '融入2-3处中华优秀传统文化元素，如经典诗文、传统节日、历史典故、书法艺术、传统科技成就。',
    builtin: true
  },
  ...
  },
  {
    ...
    prompt_order: 25,
    subject: '', stage: 'primary_low', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
    content: '小学低段：选择题≤3选项，题干简洁口语化。',
    builtin: true
  },
  {
    id: 'block_quality_primary_mid', name: '【题目质量标准】小学中段3-4年级', category: '生成-题目质量标准', type: 'fragment',
    prompt_order: 25,
    subject: '', stage: 'primary_mid', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
    content: '小学中段：选择题≤4选项。',
    builtin: true
  },
  {
    id: 'block_quality_primary_high', name: '【题目质量标准】小学高段5-6年级', category: '生成-题目质量标准', type: 'fragment',
    prompt_order: 25,
    subject: '', stage: 'primary_high', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
    content: '小学高段：选择题≤4选项，为初中衔接做准备。',
    builtin: true
  },
  {
    id: 'block_quality_middle', name: '【题目质量标准】初中', category: '生成-题目质量标准', type: 'fragment',
    prompt_order: 25,
    subject: '', stage: 'middle', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
    content: '初中：选择题≤4选项，简答题需完整解题过程。',
    builtin: true
  },
  {
    id: 'block_quality_high', name: '【题目质量标准】高中', category: '生成-题目质量标准', type: 'fragment',
    prompt_order: 25,
    subject: '', stage: 'high', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
    content: '高中：对标高考规范，答案严谨完整。',
    builtin: true
  },
  // 学科专属质量标准
  ...
  { id: 'block_quality_geography', name: '【题目质量标准】地理', category: '生成-题目质量标准', prompt_order: 25, type: 'fragment', subject: '地理', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review', content: '地理：地名/数据准确；读图题标注图例和比例尺。', builtin: true },
  { id: 'block_quality_politics', name: '【题目质量标准】道法/政治', category: '生成-题目质量标准', prompt_order: 25, type: 'fragment', subject: '道德与法治,政治,思想政治', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review', content: '道法/政治：法律条文引用准确；案例分析结合具体法条。', builtin: true },
  ...
  // 通用格式风格约束
  { id: 'block_quality_formatting', name: '【格式标准】黑白印刷风格', category: '生成-题目质量标准', prompt_order: 25, type: 'fragment', subject: '', stage: '', genType: 'exam,practice,summary,special,errorbook,preview,dictation,reading,review', content: '教辅/试卷素材必须采用黑白印刷友好风格：正文默认黑色，标题层级用h1~h3+加粗区分，禁止使用装饰性CSS文字颜色（color属性）和border-left装饰竖条。重点内容用★标注或<strong>加粗突出，不用彩色文字。数学几何图形中的线条颜色标注除外——图形内可以用不同颜色区分线段、辅助线、标记点。整体风格对标正式试卷/纸质出版物黑白印刷标准，不使用网页式彩色装饰。', builtin: true },
  ...
  // ═══════════════════════════════════════
  {
    ...
    content: '低段语文素养立意命题范式（对标新课标真题风格，范例示范的是命题思路，不是让你直接复制这些题）：\n① 情境识字："校园标语牌上写着：\u201c十年树木，百年树人。\u201d这里的\u2018树\u2019是（ ）A.树木 B.培养、种植" — 字词放在真实语境中考查理解运用。\n② 字理归类："松、柏、桂、枫都是木字旁，它们都和什么有关？你还认识哪些木字旁的字？" — 发现规律，考查思维能力。\n③ 语境填空："菜市场里，妈妈买了（ ）豆角，又买了（ ）葡萄。A.一把 B.一串" — 量词在生活情境中运用。\n④ 生活联结："你最喜欢哪种小动物？它有什么特点？用一两句话写下来（不会写的字用拼音）。" — 联结生活经验+开放表达。\n⑤ 探究发现："读了《拍手歌》，说说你从课文中学到了什么，为什么说\u2018人和动物是朋友\u2019。" — 说说理由，思维可见。\n🚫 禁止回忆式设问：不要出"杨树的特点是（ ）""小蝌蚪先长出了什么""XX的反义词是（ ）"这类直接从课文挖原句的单点回忆题——新课标考查素养运用，不是背课文。',
    builtin: true
  },
  {
    ...
    content: '素养立意命题范式（新课标真题风格，范例示范命题思路，不是让你复制这些题）：\n① 语境运用："秋天到了，果园里（果实累累／春华秋实）。选词填空，并说说你为什么这样选。" — 词语在语境中运用+说理由。\n② 探究发现："下面句子中加点的词语可以互换吗？为什么？" — 比较辨析+说理。\n③ 生活联结："学校要举行读书节，请你为班级设计一条读书标语。" — 生活实践+创意表达。\n④ 阅读理解：课内以课文原文节选为准，课外短文200-350字；题目从信息提取→关键词句理解→段落大意概括，设问不直接复述选文原句。\n🚫 禁止书本挖空：不要出"课文《秋天的雨》中，\u2018五彩缤纷\u2019的意思是（）"这类从课文中直接挖原句/原词的回忆题；多音字、近义词必须放进语境考查。',
    builtin: true
  },
  {
    ...
    content: '素养立意命题范式（新课标真题风格，范例示范命题思路，不是让你复制这些题）：\n① 语境运用："读句子，体会加点词语的表达效果，并仿写一句。" — 词句在语境中品味+迁移运用。\n② 探究发现："《白鹭》一文为什么说\u2018白鹭是一首精巧的诗\u2019？从文中找依据，说说你的理解。" — 有理有据+深度思维。\n③ 生活联结："为学校广播站拟写一则招募通知。" — 生活实践+应用文写作。\n④ 非连续性文本：提供图表/说明书/广告，考查信息提取与综合运用。\n⑤ 阅读理解：课内以课文原文节选为准，课外短文350-500字；设问从信息提取→词句理解→篇章结构→写作手法→思想感情，禁止设问直接复述选文原句。\n🚫 禁止书本挖空：古诗文默写必须理解性默写（给情境默诗句），禁止机械单句默写；课内阅读禁止挖原句填空。',
    builtin: true
  },
  {
    ...
    content: '基础知识题应覆盖：字音字形、词语运用、句子练习、古诗文积累，分值按【真题卷结构蓝本】大题分值体系拆分：各大题标题统一标注"（共X题，每小题X分，共X分）"，分值相同的小题不必逐题标注，独立主观题（口语交际/看图写话）小标题后标"（X分）"。\n阅读理解题：选文长度与年级匹配，题目覆盖信息提取→词句理解→主旨概括三个层次。\n习作题：给出明确的写作要求（主题/文体/字数），配简要的写作提示。\n🔴 禁止书本挖空：课文原句/原词不得直接挖空考查，古诗文默写以理解性默写为主，知识点放进语境考查运用。',
    builtin: true
  },
  {
    ...
    content: '素养立意命题范式（新课标真题风格，范例示范命题思路，不是让你复制这些题）：\n① 情境计算："小明买3支铅笔，每支2元，付给售货员10元，应找回几元？" — 购物情境+两步思维。\n② 说理题："15+8，你是怎么算的？先算什么，再算什么？把你的想法写下来。" — 说算理，思维可见。\n③ 探究发现："观察：1+2=3，2+3=5，3+4=7，你发现了什么？照样子再写两组。" — 找规律+迁移。\n④ 生活联结："数一数，你家里有几扇门？几个窗户？门多还是窗户多？多几个？" — 真实生活+统计意识。\n🚫 禁止书本挖空：不要出"长方形的面积公式是（）""加法的含义是（）"这类定义/公式直接挖空题；计算题放进情境（购物、分物、游戏），禁止纯算式罗列。',
    builtin: true
  },
  {
    ...
    content: '素养立意命题范式（新课标真题风格，范例示范命题思路，不是让你复制这些题）：\n① 真实情境多步应用："学校图书室有360本书，分给6个年级，每个年级3个班，平均每班分到几本？" — 两步连除，数据真实。\n② 估算与验证："妈妈带了200元，买3件衣服，每件58元，够吗？先估一估，再算一算。" — 估算意识+验证。\n③ 说理题："两位数乘一位数，积可能是几位数？举例说明你的判断。" — 说理+举例，思维可见。\n④ 探究发现："用12个边长1厘米的小正方形拼长方形，有几种拼法？它们的周长一样吗？你发现了什么？" — 动手操作+发现规律。\n🚫 禁止书本挖空：不要出"正方形的周长公式是（）""分数的定义是（）"这类公式/定义挖空题；课本例题只作知识点来源，题目必须换情境换数据重新设计。',
    builtin: true
  },
  {
    ...
    content: '素养立意命题范式（新课标真题风格，范例示范命题思路，不是让你复制这些题）：\n① 真实情境综合应用："小区花园是平行四边形，底16.8米，高9.5米，铺草坪每平方米35元，一共要多少元？（得数保留整数）" — 面积+估算+实际问题。\n② 开放题："一个长方体的体积是48立方厘米，它的长、宽、高可能是多少？写出3种可能。" — 多解开放+逆向思维。\n③ 说理题："为什么除以一个不为0的数，等于乘这个数的倒数？用具体例子说明。" — 说算理+推理。\n④ 探究发现："看折线统计图，某月气温上升最快，你判断的依据是什么？你还能提出什么数学问题？" — 数据解读+提问。\n🚫 禁止书本挖空：不要出"圆的面积公式是（）""比例的基本性质是（）"这类公式/性质直接挖空题；课本例题原题照搬属严重问题——必须换情境、换数据重新设计。',
    builtin: true
  },
  {
  ...
  {
    id: 'block_example_exam_it', name: '【质量范例】信息科技试卷', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '信息科技,信息技术', stage: '', genType: 'exam,practice', specialSubType: 'new_standard',
    content: '素养立意命题范式（新课标真题风格，范例示范命题思路，不是让你复制这些题）：\n① 算法思维（情境）：学校图书馆有5000本书，要按编号排序——分析不同排序算法（冒泡/插入/快速）在该场景下的效率，并说明选择理由 — 计算思维+算法分析。\n② 数据安全（案例）：小明在社交平台分享了含个人信息的照片，分析存在的隐私风险并给出3条防护建议 — 信息意识+信息社会责任。\n③ 数字化实践（操作）：设计一个"用表格软件统计班级图书角借阅数据"的方案，描述操作步骤和数据处理逻辑 — 数字化学习+实践应用。\n④ 开放探究：讨论"人工智能是否会取代人类工作"，结合具体案例从正反两方面分析 — 计算思维+批判性思维。\n🚫 禁止纯概念背诵：禁止考"CPU的全称是（）""RAM的中文意思是（）"式挖空，改考真实场景中的技术选择和问题解决。',
    builtin: true
  },
  {
    id: 'block_example_exam_music', name: '【质量范例】音乐试卷', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '音乐', stage: '', genType: 'exam,practice', specialSubType: 'new_standard',
    content: '素养立意命题范式（新课标真题风格，范例示范命题思路，不是让你复制这些题）：\n① 审美感知（聆听）：播放/描述一段乐曲片段（如《春节序曲》），要求辨别乐器音色、速度力度变化，并描述音乐情绪 — 审美感知+艺术表现。\n② 文化理解（对比）：对比中国民歌（如《茉莉花》）与西方古典音乐片段，从调式、节奏、文化背景分析异同 — 文化理解+审美感知。\n③ 艺术表现（创编）：给定一个节奏型，要求在此基础上创编2小节旋律并标注力度记号 — 创意实践+艺术表现。\n④ 开放赏析："为什么说音乐是跨越文化的语言？"结合至少两个国家的音乐作品说明理由 — 文化理解+批判性思维。\n🚫 禁止纯记忆考查：禁止考"XX的作曲家是（）""XX是几分之几拍"式孤立知识点考查，改考在聆听/演唱/创编情境中的综合审美判断。',
    builtin: true
  },
  {
    id: 'block_example_exam_art', name: '【质量范例】美术试卷', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '美术', stage: '', genType: 'exam,practice', specialSubType: 'new_standard',
    content: '素养立意命题范式（新课标真题风格，范例示范命题思路，不是让你复制这些题）：\n① 图像识读（赏析）：展示一幅名画（如《清明上河图》局部），从构图、色彩、细节三个角度分析画家如何表现市井繁华 — 审美感知+图像识读。\n② 创意实践（设计）：为学校运动会设计海报，描述设计思路（色彩选择/构图布局/视觉中心），说明设计理念 — 创意实践+艺术表现。\n③ 文化理解（比较）：比较中国画"留白"与西方油画"满构图"的空间处理方式，分析各自审美追求 — 文化理解+审美感知。\n④ 开放评价：有人说"抽象画就是乱画"，你是否同意？结合一幅抽象画作品阐述观点 — 审美感知+批判性思维。\n🚫 禁止纯知识背诵：禁止考"三原色是（）""透视的分类有（）"式概念填空，改考在真实赏析/创作情境中的审美判断与表达。',
    builtin: true
  },
  {
    id: 'block_example_exam_pe', name: '【质量范例】体育试卷', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '体育,体育与健康', stage: '', genType: 'exam,practice', specialSubType: 'new_standard',
    content: '素养立意命题范式（新课标真题风格，范例示范命题思路，不是让你复制这些题）：\n① 运动能力（情境）：校运会100米比赛中，小明起跑慢了0.3秒——分析起跑技术要点和改进方法，并设计3个针对性练习 — 运动能力+健康行为。\n② 健康行为（案例）：小李每天久坐8小时，设计一份包含运动、拉伸、放松的课间微运动方案，说明每项动作的健身原理 — 健康行为+运动技能。\n③ 体育品德（辨析）：足球比赛中对方球员摔倒受伤，你方球员选择踢球出界而非进攻——分析此行为体现的体育精神，并结合规则说明依据 — 体育品德+运动能力。\n④ 综合实践：制定一份"每周运动计划"，要求覆盖有氧、力量、柔韧性三类，标注每项运动的时长、心率和恢复方式 — 运动能力+健康行为+体育品德。\n🚫 禁止纯规则条文背诵：禁止考"篮球场长宽是（）""马拉松全程多少米"式记忆题，改考在运动实践情境中的技术分析、方案设计和品德辨析。',
    builtin: true
  },
  {
    id: 'block_example_exam_politics_high', name: '【质量范例】高中思想政治试卷', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '思想政治,政治', stage: 'high', genType: 'exam,practice', specialSubType: 'new_standard',
  ...
  },
  {
    ...
    content: '知识结构示例：每个知识点独立成块，用<div class="knowledge-block">包裹——①概念定义（一句话说清楚）；②核心要点（2-3个bullet point，每点简洁概括）；③典型例题（1道，配完整答案与解析）；④易错提醒（1-2个常见错误及正确理解）。\n表格示例：知识辨析用左右两列表格——左列"易错概念"（如"质量=重量"），右列"正确理解"（如"质量是物体所含物质的多少，单位kg；重量是地球引力，单位N，二者概念不同"）。',
    builtin: true
  },
  {
  ...
  },
  {
    ...
    content: '复习资料结构示例：\n知识梳理区（板块标题须含“知识框架”或“思维导图”字样）：用表格/思维导图形式纵向对比本单元核心知识点，配重难点星级标注（★基础/★★重点/★★★难点、高频考点）。每个知识点后配"易错提醒"（常见错误、正确理解）。\n典型题析区（标题须含“典型题”字样）：选3-5道涵盖本单元核心考点的典型题，每道题标注"考查知识点→思路分析→规范解答→易错提醒"完整四段式解析。\n易错聚焦区：整理本单元最高频的3-5个易错点，每个易错点配"典型错误示范→错误原因分析→正确解法对比→避坑口诀"。\n综合自测区：设计1套覆盖全部核心考点的自测题（10-15题），题型与教材单元练习风格一致，答案放文末配完整解析。',
    builtin: true
  },

  // ═══════════════════════════════════════
  
  // ── summary 知识点总结 per-stage ──
  { id: 'block_example_summary_primary_low', name: '【质量范例】低段知识点总结', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'primary_low', genType: 'summary', specialSubType: 'new_standard',
    content: '知识卡片格式示例：每个知识点一张小卡片。表格示例：用两列对比易混点——左列"容易搞混"，右列"记住这样分"。语言要求：多用比喻和口诀，每个知识点配趣味图示或emoji提示。',
    builtin: true },
  { id: 'block_example_summary_primary_mid', name: '【质量范例】中段知识点总结', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'primary_mid', genType: 'summary', specialSubType: 'new_standard',
    content: '知识结构示例：每个知识点分三步——①概念（一句话说清）②要点（2-3个bullet point）③范例（1道典型题+答案）。对比表格示例：左列"易错概念"，右列"正确理解"，每格≤20字。学习策略提示：每条核心知识点配"记忆口诀"或"辨析技巧"。',
    builtin: true },
  { id: 'block_example_summary_primary_high', name: '【质量范例】高段知识点总结', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'primary_high', genType: 'summary', specialSubType: 'new_standard',
    content: '知识网络示例：用思维导图/层级图展示知识点之间的关联。典型题析示例：每道题标注"考查知识点→思路分析→规范解答→易错提醒"。重难点标注：每个知识点标星级（★基础/★★重点/★★★难点），标注常考题型。',
    builtin: true },
  { id: 'block_example_summary_middle', name: '【质量范例】初中知识点总结', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'middle', genType: 'summary', specialSubType: 'new_standard',
    content: '知识体系示例：按"章→节→知识点"三级结构组织，每个知识点标注中考考频（高频/中频/低频）和常见题型。典型题析示例：选中考同类题，解析含"命题思路→审题要点→分步解答→评分标准→变式迁移"。易错辨析：左右对比表格——左"典型错误及原因"，右"正确理解与避坑指南"。知识关联：标注跨章节联系。',
    builtin: true },
  { id: 'block_example_summary_high', name: '【质量范例】高中知识点总结', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'high', genType: 'summary', specialSubType: 'new_standard',
    content: '知识体系示例：按"模块→专题→核心考点"三级组织，每个考点标注高考要求（了解/理解/掌握/综合运用）和近3年考查频率。典型题析：选近3年高考同类题，解析含"命题溯源→审题拆解→多解对比→满分规范→易错警示→变式预测"。思维建模：归纳通用解题策略和思维框架。跨模块整合：展示知识点综合应用场景和典型考法。',
    builtin: true },

  // ── review 复习 per-stage ──
  { id: 'block_example_review_primary_low', name: '【质量范例】低段复习', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'primary_low', genType: 'review', specialSubType: 'new_standard',
    content: '知识梳理示例：用“知识树”形式（板块标题须含“知识树”或“知识框架”字样）——树干是单元主题，树枝是大知识点，树叶是小知识点，每片叶子配一个趣味图标。典型题示例：设至少3道“典型题”（题目小标题须含“典型题”或“例题”字样），每道配“解题思路+完整解析+易错提醒”，可用闯关趣味形式包装（如“第一关·典型题：生字关”），从★到★★★难度递进，每关配鼓励语。易错提示示例：用“小马虎的故事”形式呈现典型错误。答案放文末配笑脸/星星等正向反馈标记。',
    builtin: true },
  { id: 'block_example_review_primary_mid', name: '【质量范例】中段复习', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'primary_mid', genType: 'review', specialSubType: 'new_standard',
    content: '知识梳理示例：用"知识地图"形式（板块标题须含“知识地图”字样）——以核心概念为中心，用箭头连接相关知识，标注"基础→进阶→挑战"三个层级。典型题析示例（标题须含“典型题”字样）：3-5道覆盖各考点的典型题，每道配"考查什么→怎样思考→完整解答→哪里容易错"四段式解析。易错聚焦示例：整理3个最高频易错点，用"错例→纠错→口诀"三段式呈现。',
    builtin: true },
  { id: 'block_example_review_primary_high', name: '【质量范例】高段复习', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'primary_high', genType: 'review', specialSubType: 'new_standard',
    content: '知识框架示例（板块标题须含“知识框架”或“思维导图”字样）：用思维导图+表格双重呈现——思维导图展示关联，表格展示对比。典型题析（标题须含“典型题”字样）：4-6道涵盖核心考点+小升初衔接的典型题，解析含"考点定位→模型识别→解题路径→规范书写→陷阱预警"。易错聚焦示例：整理5个最高频失分点，用"典型错解→错因诊断→正解→防错策略"完整呈现。综合自测示例：8-12道覆盖全部核心考点的自测题。',
    builtin: true },
  { id: 'block_example_review_middle', name: '【质量范例】初中复习', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'middle', genType: 'review', specialSubType: 'new_standard',
    content: '知识框架示例：按"章→考点→考查方式"三级梳理（板块标题须含“知识框架”字样），每个考点标注中考考频（★☆☆~★★★）和近年命题趋势。典型题析（标题须含“典型题”字样）：5-8道涵盖各核心考点的中考同类题/变式题，解析含"命题意图→审题关键→标准解答→评分细则→类题归纳"五层。易错聚焦：整理5-8个最高频失分点，每个配"典型错解+错因分析+正解对比+防错策略+变式巩固"五段式深度剖析。综合自测：12-15道按中考题型比例设计。',
    builtin: true },
  { id: 'block_example_review_high', name: '【质量范例】高中复习', category: '生成-质量范例', type: 'fragment',
    prompt_order: 70,
    subject: '', stage: 'high', genType: 'review', specialSubType: 'new_standard',
    content: '知识框架示例：按"专题→核心考点→命题角度→答题模板"四级梳理（板块标题须含“知识框架”字样），每个考点标注高考要求和近3年真题索引。典型题析（标题须含“典型题”字样）：6-10道涵盖各专题的高考真题/高质量模拟题，解析含"命题溯源→知识定位→审题拆解→通法解答→巧法优化→满分标准→类题预测"七层。易错聚焦：整理8-10个高频失分点，每个配"典型错解→思维偏差诊断→正解→防错策略→同类题训练"完整闭环。综合自测：1套完整自测卷，按高考题型结构。',
    builtin: true },
  ...
  },
  {
    ...
    prompt_order: 27,
    subject: '语文', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
  ...
  },
  {
    ...
    prompt_order: 27,
    subject: '数学', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
  ...
  },
  {
    ...
    prompt_order: 27,
    subject: '英语', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
  ...
  },
  {
    ...
    prompt_order: 27,
    subject: '物理,化学,生物', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
  ...
  },
  {
    ...
    prompt_order: 27,
    subject: '历史,地理,道德与法治,政治,思想政治', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,dictation,summary,review',
  ...
  { id: 'block_content_norm_exam_primary', name: '【内容规范】考卷-教材锚定（小学）', category: '生成-内容规范', type: 'fragment',
    ...
    content: '🔴 【考卷教材锚定】\n✅ 知识点100%覆盖所选教材章节，每个小题对应教材具体章节，不遗漏\n✅ 单元课文全覆盖（详见红线③）：每篇课文至少1处考点涉及\n✅ 阅读至少1篇为课内课文片段，文末标注出处：【选自教材X年级第X课《XX》】；🔴 即使使用课内课文，设问必须原创——禁止照搬教材课后练习题，须变换角度设问（信息提取→理解→推断递进），避免学生凭记忆作答\n⚠️ 可配1篇同主题课外语段（可选）：主题/难度/篇幅匹配本单元教材水平，文末标注作者/来源；低段(1-2年级)以课内为主\n✅ 古诗文默写篇目须全部来自教材必背篇目，古诗阅读篇目须全部来自教材\n✅ 选择题干扰项来源须可追溯（学生常见错误/教材易混点）', builtin: true },
  ...
  { id: 'block_content_norm_practice', name: '【内容规范】课时练-教材锚定', category: '生成-内容规范', type: 'fragment',
    ...
    content: '🔴 【课时练教材锚定强化】\n✅ 每个学习任务的情境必须基于教材原文（课文内容/单元主题/教材插图/课后习题场景），不得凭空创设与教材无关的情境。\n⛔ 严禁引入教材未涉及的话题、人物、事件作为任务情境。\n✅ 任务中的字词、句式、知识点严格限于本课教材范围。',
    builtin: true },
  ...
  { id: 'block_special_graph_primary_low', name: '【专项要求】图形-小学低段', category: '生成-专项要求', prompt_order: 20, type: 'fragment',
    ...
    content: '【图形要求】数学图形一律按 EduRender 结构化格式输出（禁止纯文字描述图形）——数轴用 [GRAPH] TYPE:COORDINATE、简单图形（点/线/三角形/长方形/圆）用 [GRAPH] TYPE:SHAPES（POINT/LINE/POLYGON/CIRCLE），格式模板详见【EduRender模板】；低段图形保持简单（坐标为整数、图形不复杂）。', builtin: true },
  ...
  { id: 'block_special_graph_primary_high', name: '【专项要求】图形-小学高段', category: '生成-专项要求', prompt_order: 20, type: 'fragment',
    ...
    content: '【图形要求】数学图形一律按 EduRender 结构化格式输出（禁止纯文字描述图形）——数轴用 [GRAPH] TYPE:COORDINATE、几何与函数图像用 [GRAPH] TYPE:SHAPES（POINT/LINE/POLYGON/CIRCLE/FUNCTION/ANGLE），并标注关键点坐标、边长、角度；格式模板详见【EduRender模板】。', builtin: true },
  ...
  { id: 'block_special_chart_primary_low', name: '【专项要求】图表-小学低段', category: '生成-专项要求', prompt_order: 20, type: 'fragment',
    subject: '数学,科学', stage: 'primary_low', genType: 'exam,practice,special,errorbook,reading,summary',
    content: '【图表要求】统计图一律按 EduRender 结构化格式输出（格式模板详见【EduRender模板】）；数据不超过5组、优先柱状图。', builtin: true },
  ...
  { id: 'block_special_chart_primary_high', name: '【专项要求】图表-小学高段', category: '生成-专项要求', prompt_order: 20, type: 'fragment',
    subject: '数学,科学', stage: 'primary_high', genType: 'exam,practice,special,errorbook,reading,summary',
    content: '【图表要求】统计图一律按 EduRender 结构化格式输出（格式模板详见【EduRender模板】）。', builtin: true },
  ...
  { id: 'block_special_image_primary_low', name: '【专项要求】配图-小学低段', category: '生成-专项要求', prompt_order: 20, type: 'fragment',
    ...
    content: '【配图要求——⚠️ 铁律：能出纯文字题的绝不用图！仅当知识点非图不可考时才配图。仅限7类场景：①看图写话/看图说话 ②图表解读/数据图表 ③地图定位/地形判读 ④实验装置图/几何图形 ⑤看图选词、看图连线 ⑥听力配图题（听音选图/判断/排序/词图匹配/涂色）⑦看图列式、图文应用题、方格纸操作图。非以上场景严禁出现[IMAGE]标记；确属以上场景时按 EduRender 结构化格式输出（TYPE:SD 文生图/TYPE:ICON 图标检索，模板详见【EduRender模板】）。\n小学低中段配图应卡通化、线条简洁、贴近儿童生活；试卷类资料用黑白线稿风格（真题卷黑白印刷），练习类资料可用彩色。', builtin: true },
  ...
  { id: 'block_special_image_primary_high', name: '【专项要求】配图-小学高段', category: '生成-专项要求', prompt_order: 20, type: 'fragment',
    ...
    content: '【配图要求——⚠️ 铁律：能出纯文字题的绝不用图！仅当知识点非图不可考时才配图。仅限7类场景：①看图写话/看图说话 ②图表解读/数据图表 ③地图定位/地形判读 ④实验装置图/几何图形 ⑤看图选词、看图连线 ⑥听力配图题（听音选图/判断/排序/词图匹配/涂色）⑦看图列式、图文应用题、方格纸操作图。非以上场景严禁出现[IMAGE]标记；确属以上场景时按 EduRender 结构化格式输出（TYPE:SD 文生图/TYPE:ICON 图标检索，模板详见【EduRender模板】）。\n小学高段配图可减少卡通风格，增加示意图、图表等半抽象视觉元素。', builtin: true },
  { id: 'block_special_image_middle', name: '【专项要求】配图-初中', category: '生成-专项要求', prompt_order: 20, type: 'fragment',
    ...
    content: '【配图要求——⚠️ 铁律：能出纯文字题的绝不用图！仅当知识点非图不可考时才配图。仅限7类场景：①看图写话/看图说话 ②图表解读/数据图表 ③地图定位/地形判读 ④实验装置图/几何图形 ⑤看图选词、看图连线 ⑥听力配图题（听音选图/判断/排序/词图匹配/涂色）⑦看图列式、图文应用题、方格纸操作图。非以上场景严禁出现[IMAGE]标记；确属以上场景时按 EduRender 结构化格式输出（TYPE:SD 文生图/TYPE:ICON 图标检索，模板详见【EduRender模板】）。', builtin: true },
  ...
  { id: 'block_type_answer', name: '【题型专项要求】解答题', category: '生成-题型专项要求', prompt_order: 39, type: 'fragment',
    ...
    content: '- 题目应有明确的解答指向（不是"谈谈你的看法"这种空泛设问）\n- 如有多个小问，难度应递进\n- 解答区域：题目下方用多个 <br> 换行留出书写空间（或用 <div style="min-height: 80px;"> 占位），⛔ 严禁用 blank-N 或 blank-line 做解答区——解答区是整块留白，不是填空横线', builtin: true },
  ...
  },
  {
    ...
    content: '- 大标题用<h1>\n- 按字词/句子/段落分节，用<h2>标题\n- 每个默写项用<div class="dictation-item">包裹，格式：序号 + 提示 + 留空书写区（用 <span class="blank-line">&emsp;...&emsp;</span> 留白，不写答案，勿用 blank-N）\n- 每个词语之间留足间距，用 <span class="spacer"></span> 分隔\n- 标准答案集中放在文末，用<div class="answer-section">包裹\n- 直接返回HTML片段，不要用<html>、<head>、<body>或```html包裹',
    builtin: true
  },
  {
    ...
    prompt_order: 23,
    subject: '语文', stage: '', genType: 'dictation', specialSubType: 'new_standard',
    content: '- 生字用<span class="tian-zi-ge">字</span>包裹；留空书写区用<span class="tian-zi-ge">&emsp;</span>（格内放&emsp;不放答案），一字一格、格数=答案字数\n- 拼音提示放在生字上方\n- 句子默写给出上句/标题提示，用 <span class="blank-line">&emsp;...&emsp;</span> 留空写下句（&emsp; 数量=答案字数×2，独立成行）\n- ✒️ 横线书写区统一用 <span class="blank-line">&emsp;...&emsp;</span>（不是 u.blank-N 横线填空！二者不可混用）。&emsp; 数量按手写余量：单词/单字→4个；短语→8个；整句默写→14-20个',
    builtin: true
  },
  {
    ...
    prompt_order: 23,
    subject: '英语', stage: '', genType: 'dictation', specialSubType: 'new_standard',
    content: '- ⚠️ 汉译英/单词默写等写英文的题型：书写区用四线三格\n- ⚠️ 英译汉等写中文的题型：书写区用 <span class="blank-line">&emsp;...&emsp;</span> 普通横线（禁止四线格/田字格，勿用 blank-N 横线填空）\n- 每5-8个词设一个休息分隔线',
    builtin: true
  },
  {
  ...
  {
    id: 'block_markup_general', name: '【学科标记】通用（连线/配对题格式）', category: '生成-学科标记', type: 'fragment',
    prompt_order: 21,
    subject: '', stage: '', genType: '',
    content: '【通用学科标记——连线/配对题格式（全学科适用）】\n- 连线题/配对题：必须用 <div class="match-question"> 包裹，内含两个 <div class="match-col">（左列和右列），每列内用 <div class="match-item">内容</div> 逐项排列；左右项数对等。\n  示例：<div class="match-question"><div class="match-col"><div class="match-item">项A</div><div class="match-item">项B</div></div><div class="match-col"><div class="match-item">项1</div><div class="match-item">项2</div></div></div>\n- ⛔ 严禁用文本"------"或"A ------ B"等纯文本格式表示连线，必须使用上述HTML结构。',
    builtin: true
  },
  {
    id: 'block_markup_english', name: '【学科标记】英语专用', category: '生成-学科标记', type: 'fragment',
    prompt_order: 22,
    subject: '英语', stage: '', genType: 'exam,practice,special,errorbook,reading,preview,summary,dictation,review',
  ...
  },
  {
    ...
    prompt_order: 21,
    subject: '', stage: '', genType: '',
    content: '文生图完整模板：[IMAGE]\nTYPE:SD\nPROMPT:画面详细描述（含风格：黑白线稿/简笔画/卡通等，图内无文字、无答案暗示）\nNEGATIVE:文字,水印\nWIDTH:800\nHEIGHT:600\nSTYLE:line_art\n[/IMAGE]\n图标检索：[IMAGE] TYPE:ICON KEYWORDS:关键词 STYLE:flat [/IMAGE]',
    builtin: true
  },
  ...
  // （已删除 gen_stage_* 学段适配 5 块——与「生成-学段控制」双轨重复，学段控制为详细版并已吸收其独特点，v27）
  ...
  { id: 'gen_subj_math_primary_low', name: '【数学-小学低段】', category: '生成-学科适配', prompt_order: 17, type: 'fragment',
    ...
    content: '- 数学（低段1-2年级）：联系生活实际；口算和竖式格式规范；以100以内加减法和表内乘法（九九乘法口诀）为主，不涉及乘除法竖式；禁止出现万以上数字和分数小数', builtin: true },
  ...
  { id: 'gen_subj_science_primary_mid', name: '【科学-小学中段】', category: '生成-学科适配', prompt_order: 17, type: 'fragment',
    ...
    content: '- 科学（中段3-4年级）：实验设计与简单变量控制；数据记录与基础图表；物质科学+生命科学初步；引入少量科学阅读', builtin: true },
  ...
  { id: 'gen_struct_practice_moral_primary', name: '【课时练结构-道德与法治-小学】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '道德与法治,政治', stage: 'primary_low,primary_mid,primary_high', genType: 'practice', specialSubType: 'new_standard',
    content: '结构参考：\n一、情境感知·基础建构（生活情境、规则认知）\n二、行为辨析·能力进阶（情境判断、价值辨析）\n三、实践行动·素养提升（知行合一、习惯养成）', builtin: true },
  // ── 音乐/美术/体育 practice（2022 艺术/体育课标，全学段通用）──
  { id: 'gen_struct_practice_music', name: '【课时练结构-音乐】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '音乐', stage: '', genType: 'practice', specialSubType: 'new_standard',
    content: '结构参考：\n一、基础建构（聆听感知、节奏模仿）\n二、表现实践·能力进阶（演唱演奏、合作表现）\n三、创意实践·素养提升（即兴创编、文化理解）', builtin: true },
  { id: 'gen_struct_practice_art', name: '【课时练结构-美术】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '美术', stage: '', genType: 'practice', specialSubType: 'new_standard',
    content: '结构参考：\n一、基础建构（观察感知、造型基础）\n二、创作实践·能力进阶（表现练习、合作创作）\n三、创意实践·素养提升（创意表达、审美判断）', builtin: true },
  { id: 'gen_struct_practice_pe', name: '【课时练结构-体育】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '体育', stage: '', genType: 'practice', specialSubType: 'new_standard',
    content: '结构参考：\n一、基础建构（动作要领、规则认知）\n二、学练实践·能力进阶（技能练习、合作学练）\n三、拓展应用·素养提升（运动应用、健康行为）', builtin: true },
  ...
  { id: 'gen_struct_preview_moral_primary', name: '【预习结构-道德与法治-小学】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '道德与法治,政治', stage: 'primary_low,primary_mid,primary_high', genType: 'preview', specialSubType: 'new_standard',
    content: '结构参考：\n一、学习目标\n二、生活情境导入\n三、价值观念预读\n四、预习检测', builtin: true },
  // ── 音乐/美术/体育 preview（2022 艺术/体育课标，全学段通用）──
  { id: 'gen_struct_preview_music', name: '【预习结构-音乐】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '音乐', stage: '', genType: 'preview', specialSubType: 'new_standard',
    content: '结构参考：\n一、学习目标\n二、聆听导入\n三、乐理与作品预读\n四、预习检测', builtin: true },
  { id: 'gen_struct_preview_art', name: '【预习结构-美术】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '美术', stage: '', genType: 'preview', specialSubType: 'new_standard',
    content: '结构参考：\n一、学习目标\n二、图像观察导入\n三、造型语言预读\n四、预习检测', builtin: true },
  { id: 'gen_struct_preview_pe', name: '【预习结构-体育】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '体育', stage: '', genType: 'preview', specialSubType: 'new_standard',
    content: '结构参考：\n一、学习目标\n二、运动情境导入\n三、动作要领预读\n四、安全提示\n五、预习检测', builtin: true },
  ...
  { id: 'gen_struct_summary_chinese_primary_mid', name: '【总结结构-语文-小学中段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、字词积累与梳理（语境中归纳，勿孤立罗列）\n二、词语运用（情境中积累）\n三、课文精读与鉴赏\n四、知识点归纳与典型题精讲（结构化呈现）\n五、素养拓展与反思（文化感知、整本书阅读启蒙）', builtin: true },
  ...
  { id: 'gen_struct_summary_math_primary_low', name: '【总结结构-数学-小学低段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、知识要点与核心概念（结合生活情境理解，聚焦数感·运算·图形认识）\n二、典型例题（情境嵌入，训练运算能力、空间观念、数据意识）\n三、易错辨析（归因+正解、变式训练）\n四、素养拓展与反思（生活应用、趣味探究）', builtin: true },
  ...
  { id: 'gen_struct_summary_math_primary_high', name: '【总结结构-数学-小学高段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、知识网络（概念·公式·图形几何）\n二、核心知识精讲（抽象能力、推理意识、模型意识）\n三、典型例题剖析（变式训练、易错辨析）\n四、错题精析与分层自测\n五、素养拓展与反思（跨学科应用、反思评价）', builtin: true },
  ...
  { id: 'gen_struct_summary_math_high', name: '【总结结构-数学-高中】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、知识体系构建（概念·公式·定理·知识网络）\n二、核心知识与数学思想（逻辑推理、数学建模、数据分析）\n三、典型例题全方位解析（变式训练、易错辨析）\n四、易错与难点突破\n五、高考真题研习\n六、素养拓展与反思（跨学科融合、创新思维）', builtin: true },
  ...
  // ── 英语小学段 summary（2022 课标要求小学以听说为主、读写跟进，注重文化意识启蒙，按 low/mid/high 学段细分条目生效）──

  // ── 专项突破 special（按学段拆分：通用 new_standard 模板，subType 未命中时兜底）──
  // 语文 special — 五学段
  { id: 'gen_struct_special_chinese_primary_low', name: '【专项结构-语文-小学低段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '语文', stage: 'primary_low', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、兴趣启蒙（情境导入、趣味感知）\n二、基础方法（识字写字、简单阅读策略）\n三、趣味练习（游戏化巩固）\n四、展示分享（朗读、复述）', builtin: true },
  { id: 'gen_struct_special_chinese_primary_mid', name: '【专项结构-语文-小学中段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '语文', stage: 'primary_mid', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、方法点拨（阅读策略、习作方法）\n二、情境典例（典型文本示例）\n三、分层变式（基础巩固→能力提升）\n四、综合实践（整本书、跨媒体）', builtin: true },
  { id: 'gen_struct_special_chinese_primary_high', name: '【专项结构-语文-小学高段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '语文', stage: 'primary_high', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、策略精讲（阅读策略体系、习作技法）\n二、情境典例（文学类+实用类文本）\n三、分层变式（基础→进阶→挑战）\n四、迁移创新（跨学科、综合性学习）', builtin: true },
  { id: 'gen_struct_special_chinese_middle', name: '【专项结构-语文-初中】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '语文', stage: 'middle', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、方法体系（文体阅读法、写作策略）\n二、中考典例（典型题型解析）\n三、分层变式（基础→能力→素养）\n四、实战演练（限时训练、中考对标）', builtin: true },
  { id: 'gen_struct_special_chinese_high', name: '【专项结构-语文-高中】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '语文', stage: 'high', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、方法论（文本解读体系、思辨写作）\n二、高考典例（典型题型深度剖析）\n三、分级训练（基础→进阶→拔高）\n四、限时实战（高考模拟、专题探究）', builtin: true },
  // 数学 special — 五学段
  { id: 'gen_struct_special_math_primary_low', name: '【专项结构-数学-小学低段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '数学', stage: 'primary_low', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、算理启蒙（直观操作、数形结合）\n二、游戏化典例（生活情境、趣味任务）\n三、阶梯练习（模仿→巩固→应用）\n四、趣味挑战（思维拓展）', builtin: true },
  { id: 'gen_struct_special_math_primary_mid', name: '【专项结构-数学-小学中段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '数学', stage: 'primary_mid', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、方法点拨（解题策略、巧算技巧）\n二、情境典例（典型例题精析）\n三、分层变式（基础巩固→能力提升）\n四、综合实践（生活应用、跨学科启蒙）', builtin: true },
  { id: 'gen_struct_special_math_primary_high', name: '【专项结构-数学-小学高段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '数学', stage: 'primary_high', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、策略精讲（建模策略、解题路径）\n二、情境典例（典型题型深度剖析）\n三、分层变式（基础→进阶→挑战）\n四、迁移创新（跨学科应用、开放探究）', builtin: true },
  { id: 'gen_struct_special_math_middle', name: '【专项结构-数学-初中】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '数学', stage: 'middle', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、方法体系（代数/几何思想方法）\n二、中考典例（典型题型深度剖析）\n三、分层变式（基础→能力→素养）\n四、实战演练（限时训练、中考对标）', builtin: true },
  { id: 'gen_struct_special_math_high', name: '【专项结构-数学-高中】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '数学', stage: 'high', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、核心方法（建模、化归、分类讨论）\n二、高考典例（典型题型深度剖析）\n三、分级训练（基础→进阶→拔高）\n四、限时实战（高考模拟、创新思维）', builtin: true },
  // 英语 special — 五学段
  { id: 'gen_struct_special_english_primary_low', name: '【专项结构-英语-小学低段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '英语', stage: 'primary_low', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、语感启蒙（歌曲、韵律、游戏）\n二、趣味典例（图片情境、简单对话）\n三、阶梯练习（模仿→认读→表达）\n四、展示分享（表演、歌曲展示）', builtin: true },
  { id: 'gen_struct_special_english_primary_mid', name: '【专项结构-英语-小学中段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '英语', stage: 'primary_mid', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、策略点拨（阅读策略、交际策略）\n二、情境典例（多模态语篇示例）\n三、分层变式（基础巩固→能力提升）\n四、综合实践（跨文化、跨学科活动）', builtin: true },
  { id: 'gen_struct_special_english_primary_high', name: '【专项结构-英语-小学高段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '英语', stage: 'primary_high', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、方法精讲（阅读策略体系、写作方法）\n二、情境典例（任务型阅读、写作示例）\n三、分层变式（基础→进阶→挑战）\n四、迁移创新（跨文化理解、项目实践）', builtin: true },
  { id: 'gen_struct_special_english_middle', name: '【专项结构-英语-初中】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '英语', stage: 'middle', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、方法体系（阅读策略、写作框架）\n二、中考典例（典型题型深度剖析）\n三、分层变式（基础→能力→素养）\n四、实战演练（限时训练、中考对标）', builtin: true },
  { id: 'gen_struct_special_english_high', name: '【专项结构-英语-高中】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '英语', stage: 'high', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、深度策略（批判性阅读、学术写作）\n二、高考典例（典型题型深度剖析）\n三、分级训练（基础→进阶→拔高）\n四、限时实战（高考模拟、思辨训练）', builtin: true },
  // ── 阅读训练 reading ──
  ...
  { id: 'gen_struct_dictation_chinese', name: '【默写结构-语文】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '语文', stage: '', genType: 'dictation', specialSubType: 'new_standard',
    content: '结构参考：\n一、语境默写（在句段中默写字词，勿孤立罗列）\n二、理解运用（语境辨析、迁移）\n三、综合默写（语篇/段落）', builtin: true },
  { id: 'gen_struct_dictation_english', name: '【默写结构-英语】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '英语', stage: '', genType: 'dictation', specialSubType: 'new_standard',
    content: '结构参考：\n一、语境默写（在句子中拼写单词，勿孤立罗列）\n二、理解运用（语境辨析、句型转换）\n三、综合默写（语篇/短对话/短文填空）', builtin: true },
  ...
  { id: 'gen_struct_errorbook_music', name: '【错题本结构-音乐】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '音乐', stage: '', genType: 'errorbook', specialSubType: 'new_standard',
    content: '结构参考：\n一、错题整理\n二、错误归因与素养反思\n三、正确示范\n四、变式巩固', builtin: true },
  { id: 'gen_struct_errorbook_art', name: '【错题本结构-美术】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '美术', stage: '', genType: 'errorbook', specialSubType: 'new_standard',
    content: '结构参考：\n一、错题整理\n二、错误归因与素养反思\n三、正确示范\n四、变式巩固', builtin: true },
  { id: 'gen_struct_errorbook_pe', name: '【错题本结构-体育】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
  ...
  { id: 'gen_struct_summary_moral_primary', name: '【总结结构-道德与法治-小学】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、知识结构化梳理（规则认知·价值观念·知识网络）\n二、核心能力聚焦（行为辨析、法治启蒙）\n三、典型题型解析（变式训练、易错辨析）\n四、素养拓展与反思（知行合一、习惯养成）', builtin: true },
  // ── 音乐/美术/体育 summary（2022 艺术/体育课标，全学段通用）──
  { id: 'gen_struct_summary_music', name: '【总结结构-音乐】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '音乐', stage: '', genType: 'summary', specialSubType: 'new_standard',
    content: '结构参考：\n一、知识结构化梳理（乐理常识·名曲听赏·知识网络）\n二、核心能力聚焦（审美感知、艺术表现）\n三、典型题型解析（变式训练、易错辨析）\n四、素养拓展与反思（创意实践、文化理解）', builtin: true },
  { id: 'gen_struct_summary_art', name: '【总结结构-美术】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '美术', stage: '', genType: 'summary', specialSubType: 'new_standard',
    content: '结构参考：\n一、知识结构化梳理（造型语言·名家名作·知识网络）\n二、核心能力聚焦（图像识读、美术表现）\n三、典型题型解析（变式训练、易错辨析）\n四、素养拓展与反思（创意实践、文化理解）', builtin: true },
  { id: 'gen_struct_summary_pe', name: '【总结结构-体育】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '体育', stage: '', genType: 'summary', specialSubType: 'new_standard',
    content: '结构参考：\n一、知识结构化梳理（动作要领·规则知识·知识网络）\n二、核心能力聚焦（运动能力、健康行为）\n三、典型题型解析（变式训练、易错辨析）\n四、素养拓展与反思（体育品德、安全防护）', builtin: true },
  ...
  { id: 'gen_struct_special_moral_primary', name: '【专项结构-道德与法治-小学】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、核心素养方法（行为辨析方法）\n二、情境典例（校园与家庭生活）\n三、分层变式（基础巩固→情境迁移）\n四、综合实践（行动践行、习惯养成）', builtin: true },
  // ── 音乐/美术/体育 special（2022 艺术/体育课标，全学段通用）──
  { id: 'gen_struct_special_music', name: '【专项结构-音乐】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '音乐', stage: '', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、核心素养方法（听赏策略、表现方法）\n二、情境典例（经典作品赏析）\n三、分层变式（基础巩固→创意表现）\n四、综合实践（合作创编、文化体验）', builtin: true },
  { id: 'gen_struct_special_art', name: '【专项结构-美术】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '美术', stage: '', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、核心素养方法（观察方法、表现技法）\n二、情境典例（经典作品赏析）\n三、分层变式（基础巩固→创意表达）\n四、综合实践（主题创作、文化体验）', builtin: true },
  { id: 'gen_struct_special_pe', name: '【专项结构-体育】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '体育', stage: '', genType: 'special', specialSubType: 'new_standard',
    content: '结构参考：\n一、核心素养方法（动作要领、练习方法）\n二、情境典例（运动情境分析）\n三、分层变式（基础巩固→技能提升）\n四、综合实践（合作竞赛、健康生活）', builtin: true },
  ...
  // ── 初中/高中知识边界补充（音体美为真实开设课程；科学/跨学段名称说明）──
  { id: 'kb_arts_middle', name: '【知识边界】音乐/美术/体育-初中', category: '生成-知识边界', prompt_order: 80, type: 'fragment', subject: '音乐,美术,体育,体育与健康', stage: 'middle', genType: '', content: '音体美初中知识边界：\n- 严格限于教材内容（音乐：乐理基础/作品欣赏/演唱实践；美术：造型/色彩/设计/欣赏；体育：运动技能/健康知识/规则）\n- 不考查专业术语深析、不涉及竞技训练细节\n- 题目以体验、识别、欣赏、判断、设计类为主', builtin: true },
  { id: 'kb_arts_high', name: '【知识边界】音乐/美术/体育-高中', category: '生成-知识边界', prompt_order: 80, type: 'fragment', subject: '音乐,美术,体育,体育与健康', stage: 'high', genType: '', content: '音体美高中知识边界：\n- 严格限于教材内容（音乐：音乐鉴赏/模块实践；美术：美术鉴赏/绘画/设计；体育：体能/专项技能/健康）\n- 对标学业水平合格考要求，不涉及专业院校艺考内容\n- 题目以鉴赏分析、实践设计、健康方案类为主', builtin: true },
  { id: 'kb_science_course_middle', name: '【知识边界】科学（综合课）-初中', category: '生成-知识边界', prompt_order: 80, type: 'fragment', subject: '科学', stage: 'middle', genType: '', content: '科学初中知识边界：\n- 部分地区初中设综合"科学"课（物理/化学/生物/地理综合），严格以所选教材实际内容为准\n- 不考查超出教材范围的理科深度内容\n- 探究题体现观察→假设→实验→结论完整过程', builtin: true },
  { id: 'kb_science_high', name: '【知识边界】科学-高中', category: '生成-知识边界', prompt_order: 80, type: 'fragment', subject: '科学', stage: 'high', genType: '', content: '科学高中知识边界：\n- 高中阶段不单独开设综合"科学"课（分设物理/化学/生物），如遇教材请严格以教材实际内容为准\n- 不考查超出教材范围的内容', builtin: true },
  { id: 'kb_moral_high', name: '【知识边界】道德与法治-高中（高中称思想政治）', category: '生成-知识边界', prompt_order: 80, type: 'fragment', subject: '道德与法治,政治', stage: 'high', genType: '', content: '道德与法治高中知识边界：\n- 高中阶段本课程名称为"思想政治"，如遇"道德与法治"教材请严格以教材实际内容为准\n- 对标高考政治考试说明，经济/政治/文化/哲学四模块\n- 时政材料数据来源须真实可查', builtin: true },
  { id: 'kb_politics_middle', name: '【知识边界】思想政治-初中（初中称道德与法治）', category: '生成-知识边界', prompt_order: 80, type: 'fragment', subject: '思想政治', stage: 'middle', genType: '', content: '思想政治初中知识边界：\n- 初中阶段本课程名称为"道德与法治"，如遇"思想政治"教材请严格以教材实际内容为准\n- 仅涉及道德修养+法治观念+国情教育基础\n- 法律条文仅涉及宪法和与学生相关的法律', builtin: true },
  ...
  // ═══════════════════════════════════════
  { id: 'answer_exam', name: '【答案规范】考卷/课时练', category: '生成-答案与解析规范', prompt_order: 26, type: 'fragment', subject: '', stage: '', genType: 'exam', content: '考卷答案规范：考卷答案规范（对齐正式考试参考答案册样式）：\n1. 每道题的答案必须标注分值"（X分）"（与题目分值一一对应，方便对照改卷计分）——如"1. B（2分）""3. 谷像黄金粒粒香（2分）"\n2. 客观题（选择/判断/填空）：答案明确唯一，附1-2句简要解析说明对错原因或解题关键\n3. 解答题/简答题：给出参考答案要点 + 分步给分说明（标注每个要点的分值比重，各要点分值之和=该题总分）\n4. 主观题（作文/习作/写话/书面表达）：给出评分标准（内容/语言/结构/书写四维度各占比例，总分=该题分值）+ 1篇参考范文\n5. 每道题答案用<div class=\"answer-item\">分隔，标注题号与分值\n6. 🔴 禁止出现\"易错提示\"\"命题思路\"\"拓展延伸\"等教辅栏目——正式考试答案册只有答案、解析与评分标准\n7. 🔴 禁止出现任何质检、自审、生成说明文字（如\"知识点准确性\"\"课标对齐\"等检查条目）——正式试卷无此类内容', builtin: true },
  ...
  { id: 'typedist_practice_chinese_primary_mid', name: '【题型分布】语文课时练-小学中高段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '语文', stage: 'primary_mid', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（字词句运用）：8-12个任务\n- 能力提升（阅读理解）：5-8个任务\n- 拓展探究（小练笔）：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_chinese_primary_high', name: '【题型分布】语文课时练-小学高段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '语文', stage: 'primary_high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（字词句运用）：10-15个任务\n- 能力提升（阅读理解）：7-10个任务\n- 拓展探究（小练笔）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },
  { id: 'typedist_practice_chinese_secondary', name: '【题型分布】语文课时练-中学', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '语文', stage: 'middle', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关：8-12个任务\n- 能力提升：5-8个任务\n- 拓展探究：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_math_primary_low', name: '【题型分布】数学课时练-小学低段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '数学', stage: 'primary_low', genType: 'practice', content: '总任务数控制在12-18个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（计算与概念填空）：6-9个任务\n- 能力提升（解决问题与应用）：4-6个任务\n- 拓展探究（思维挑战）：2-3个任务\n', typeDist: '基础建构任务:6-9,探究进阶任务:4-6,迁移创新任务:2-3', builtin: true },
  { id: 'typedist_practice_math_primary_mid', name: '【题型分布】数学课时练-小学中段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '数学', stage: 'primary_mid', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（计算与概念填空）：8-12个任务\n- 能力提升（解决问题与应用）：5-8个任务\n- 拓展探究（思维挑战）：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_math_primary_high', name: '【题型分布】数学课时练-小学高段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '数学', stage: 'primary_high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（计算与概念填空）：10-15个任务\n- 能力提升（解决问题与应用）：7-10个任务\n- 拓展探究（思维挑战）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },
  { id: 'typedist_practice_math_secondary', name: '【题型分布】数学课时练-中学', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '数学', stage: 'middle', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关：8-12个任务\n- 能力提升：5-8个任务\n- 拓展探究：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_english_primary_low', name: '【题型分布】英语课时练-小学低段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '英语', stage: 'primary_low', genType: 'practice', content: '总任务数控制在12-18个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（词汇认读与句型操练）：6-9个任务\n- 能力提升（语篇理解与语法）：4-6个任务\n- 拓展探究（情境交际/写作）：2-3个任务\n', typeDist: '基础建构任务:6-9,探究进阶任务:4-6,迁移创新任务:2-3', builtin: true },
  { id: 'typedist_practice_english_primary_mid', name: '【题型分布】英语课时练-小学中段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '英语', stage: 'primary_mid', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（词汇认读与句型操练）：8-12个任务\n- 能力提升（语篇理解与语法）：5-8个任务\n- 拓展探究（情境交际/写作）：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_english_primary_high', name: '【题型分布】英语课时练-小学高段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '英语', stage: 'primary_high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（词汇认读与句型操练）：10-15个任务\n- 能力提升（语篇理解与语法）：7-10个任务\n- 拓展探究（情境交际/写作）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },
  { id: 'typedist_practice_english_secondary', name: '【题型分布】英语课时练-中学', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '英语', stage: 'middle', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关：8-12个任务\n- 能力提升：5-8个任务\n- 拓展探究：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_science_primary_low', name: '【题型分布】科学课时练-小学低段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '科学', stage: 'primary_low', genType: 'practice', content: '总任务数控制在10-15个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念填空与观察）：5-7个任务\n- 能力提升（实验分析）：3-5个任务\n- 拓展探究（科学小实践）：2-3个任务\n', typeDist: '基础建构任务:5-7,探究进阶任务:3-5,迁移创新任务:2-3', builtin: true },
  { id: 'typedist_practice_science_primary_mid', name: '【题型分布】科学课时练-小学中段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '科学', stage: 'primary_mid', genType: 'practice', content: '总任务数控制在12-18个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念填空）：6-9个任务\n- 能力提升（实验分析）：4-6个任务\n- 拓展探究（科学小实践）：2-3个任务\n', typeDist: '基础建构任务:6-9,探究进阶任务:4-6,迁移创新任务:2-3', builtin: true },
  { id: 'typedist_practice_science_primary_high', name: '【题型分布】科学课时练-小学高段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '科学', stage: 'primary_high', genType: 'practice', content: '总任务数控制在16-22个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念填空）：8-11个任务\n- 能力提升（实验分析）：5-7个任务\n- 拓展探究（科学小实践）：3-4个任务\n', typeDist: '基础建构任务:8-11,探究进阶任务:5-7,迁移创新任务:3-4', builtin: true },

  // ── 理科 practice 补充（物理/化学/生物）──
  { id: 'typedist_practice_physics_middle', name: '【题型分布】物理课时练-初中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '物理', stage: 'middle', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念辨析与公式训练）：8-12个任务\n- 能力提升（计算与应用）：5-8个任务\n- 拓展探究（实验设计与创新）：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_physics_high', name: '【题型分布】物理课时练-高中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '物理', stage: 'high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念辨析与公式训练）：10-15个任务\n- 能力提升（计算与应用）：7-10个任务\n- 拓展探究（实验设计与创新）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },
  { id: 'typedist_practice_chemistry_middle', name: '【题型分布】化学课时练-初中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '化学', stage: 'middle', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念与方程式）：8-12个任务\n- 能力提升（推断与计算）：5-8个任务\n- 拓展探究（实验设计与创新）：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_chemistry_high', name: '【题型分布】化学课时练-高中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '化学', stage: 'high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念与方程式）：10-15个任务\n- 能力提升（推断与计算）：7-10个任务\n- 拓展探究（实验设计与创新）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },
  { id: 'typedist_practice_biology_middle', name: '【题型分布】生物课时练-初中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '生物', stage: 'middle', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念填空与识图）：8-12个任务\n- 能力提升（实验设计与数据分析）：5-8个任务\n- 拓展探究（综合应用与探究）：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_biology_high', name: '【题型分布】生物课时练-高中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '生物', stage: 'high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念填空与识图）：10-15个任务\n- 能力提升（实验设计与数据分析）：7-10个任务\n- 拓展探究（综合应用与探究）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },

  // ── 文科 practice 补充（历史/地理）──
  { id: 'typedist_practice_history_middle', name: '【题型分布】历史课时练-初中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '历史', stage: 'middle', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（时序与史实）：8-12个任务\n- 能力提升（材料分析与论述）：5-8个任务\n- 拓展探究（历史评价与反思）：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_history_high', name: '【题型分布】历史课时练-高中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '历史', stage: 'high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（时序与史实）：10-15个任务\n- 能力提升（材料分析与论述）：7-10个任务\n- 拓展探究（历史评价与反思）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },
  { id: 'typedist_practice_geo_middle', name: '【题型分布】地理课时练-初中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '地理', stage: 'middle', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念与读图）：8-12个任务\n- 能力提升（区域分析与综合）：5-8个任务\n- 拓展探究（地理实践与探究）：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_geo_high', name: '【题型分布】地理课时练-高中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '地理', stage: 'high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念与读图）：10-15个任务\n- 能力提升（区域分析与综合）：7-10个任务\n- 拓展探究（地理实践与探究）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },

  // ── 道法/政治 practice 补充 ──
  { id: 'typedist_practice_moral_primary_low', name: '【题型分布】道法课时练-小学低段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '道德与法治', stage: 'primary_low', genType: 'practice', content: '总任务数控制在10-15个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念判断与辨析）：5-7个任务\n- 能力提升（情境分析与应用）：3-5个任务\n- 拓展探究（综合实践与反思）：2-3个任务\n', typeDist: '基础建构任务:5-7,探究进阶任务:3-5,迁移创新任务:2-3', builtin: true },
  { id: 'typedist_practice_moral_primary_mid', name: '【题型分布】道法课时练-小学中段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '道德与法治', stage: 'primary_mid', genType: 'practice', content: '总任务数控制在12-18个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念判断与辨析）：6-9个任务\n- 能力提升（情境分析与应用）：4-6个任务\n- 拓展探究（综合实践与反思）：2-3个任务\n', typeDist: '基础建构任务:6-9,探究进阶任务:4-6,迁移创新任务:2-3', builtin: true },
  { id: 'typedist_practice_moral_primary_high', name: '【题型分布】道法课时练-小学高段', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '道德与法治', stage: 'primary_high', genType: 'practice', content: '总任务数控制在16-22个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念判断与辨析）：8-11个任务\n- 能力提升（情境分析与应用）：5-7个任务\n- 拓展探究（综合实践与反思）：3-4个任务\n', typeDist: '基础建构任务:8-11,探究进阶任务:5-7,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_moral_middle', name: '【题型分布】道法课时练-初中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '道德与法治', stage: 'middle', genType: 'practice', content: '总任务数控制在16-24个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念判断与辨析）：8-12个任务\n- 能力提升（情境分析与应用）：5-8个任务\n- 拓展探究（综合实践与反思）：3-4个任务\n', typeDist: '基础建构任务:8-12,探究进阶任务:5-8,迁移创新任务:3-4', builtin: true },
  { id: 'typedist_practice_politics_high', name: '【题型分布】政治课时练-高中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '政治,思想政治', stage: 'high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念判断与辨析）：10-15个任务\n- 能力提升（情境分析与应用）：7-10个任务\n- 拓展探究（综合实践与反思）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },

  // ── 主科 practice 高中段补充（语文/数学/英语）──
  { id: 'typedist_practice_chinese_high', name: '【题型分布】语文课时练-高中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '语文', stage: 'high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（文言实词/虚词/翻译、名句默写）：10-15个任务\n- 能力提升（阅读理解、诗歌鉴赏）：7-10个任务\n- 拓展探究（写作片段/语言运用）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },
  { id: 'typedist_practice_math_high', name: '【题型分布】数学课时练-高中', category: '生成-题型分布建议', prompt_order: 52, type: 'fragment', subject: '数学', stage: 'high', genType: 'practice', content: '总任务数控制在20-30个任务。课时练任务分布建议如下（如上方已手动配置任务数量，以手动配置为准；此建议仅作参考）：\n- 基础过关（概念辨析与公式训练）：10-15个任务\n- 能力提升（综合运用与证明）：7-10个任务\n- 拓展探究（压轴题与创新思维）：3-5个任务\n', typeDist: '基础建构任务:10-15,探究进阶任务:7-10,迁移创新任务:3-5', builtin: true },
 ...
 // 【生成-核心任务】块级指令 — 按资料类型(genType)匹配
  ...
  // ═══════════════════════════════════════
  { id: 'core_task_exam', name: '【核心任务】考卷', category: '生成-核心任务', prompt_order: 10, type: 'fragment', subject: '', stage: '', genType: 'exam', content: '生成一份完整考试卷（页数要求见角色身份）。🔴 新课标考卷方向：以情境为载体考查知识运用，情境化试题占比≥70%。题量以完整覆盖全部知识点为准——每个大题组至少含3-6小题，每题独立考查不同角度，禁止大题只含1小题凑数；核心知识点配3-5小题，次要知识点配1-2小题。严格按下方结构参考和难度配置执行，试题需原创改编（同资料内不同题目考查角度、情境、数据不得雷同）（可变换教材原题的数据/情境/问法）。正文只含题目，参考答案与解析统一放文末。', builtin: true },
  { id: 'core_task_practice', name: '【核心任务】课时练', category: '生成-核心任务', prompt_order: 10, type: 'fragment', subject: '', stage: '', genType: 'practice', content: '生成一份课堂同步练习（页数要求见角色身份）。🔴 以学习任务为组织单位（非传统练习题堆砌）。全卷至少8-12个学习任务，每个任务含真实情境+核心活动+成果产出，任务之间体现认知递进（识记→理解→应用→分析→评价）。禁止传统题型孤立堆砌——所有选择/判断/填空必须嵌入真实任务情境中。鼓励学生展现思维过程，至少30%任务允许多元答案。', builtin: true },
  // ── 学科专用 practice（比通用条目更精细的每类型题量指导）──
  { id: 'core_task_practice_chinese', name: '【核心任务-语文课堂练习】', category: '生成-核心任务', prompt_order: 10, type: 'fragment', subject: '语文', stage: '', genType: 'practice', content: '生成一份语文课堂同步练习（页数要求见角色身份）。🔴 以学习任务为组织单位。4类任务群配比：语言文字积累与梳理（30-40%，在语境中识字学词，如在阅读短文中理解生字词义、制作生字积累卡含部首+笔画+结构+语境例句）| 文学阅读与创意表达（30-40%，角色对话仿写/精彩句段赏析仿写/续编改编故事/配图说明）| 思辨性阅读与表达（15-25%，用表格比较角色异同/判断文中观点并说明理由/开放讨论）| 跨学科学习（5-10%，观察记录/配图说明/实践活动）。全卷至少8-12个学习任务，每个任务有情境+活动+成果。🔴 严禁：孤立看拼音写词、脱离语境组词填空、纯抄写、机械判断对错。所有传统题型必须嵌入任务情境中，不可孤立堆砌。',  builtin: true },
  { id: 'core_task_practice_math', name: '【核心任务-数学课堂练习】', category: '生成-核心任务', prompt_order: 10, type: 'fragment', subject: '数学', stage: '', genType: 'practice', content: '生成一份数学课堂同步练习（页数要求见角色身份）。🔴 以数学任务为组织单位（非计算题堆砌）。设计三类任务：基础运算与应用（40-50%，在真实生活情境中设置计算任务，如购物找零/测量长度/统计数据，要求写出完整解题过程）| 问题解决与推理（30-40%，开放性问题/多步推理/策略比较，要求说清"你是怎么想的"）| 跨学科实践（10-20%，数学与科学/美术/生活的融合，如制作统计图表/设计测量方案）。全卷至少8-12个学习任务，每个任务有真实情境+数学活动+成果产出。禁止孤立计算题堆砌——所有计算必须嵌入真实情境。', builtin: true },
  { id: 'core_task_practice_english', name: '【核心任务-英语课堂练习】', category: '生成-核心任务', prompt_order: 10, type: 'fragment', subject: '英语', stage: '', genType: 'practice', content: '生成一份英语课堂同步练习（页数要求见角色身份）。🔴 以语言任务为组织单位（非词汇句型孤立操练）。设计三类任务：语言知识积累（30-40%，在对话/短文中学习词汇和句型，如角色扮演中运用新词、情境对话中操练句型）| 语言技能运用（30-40%，听说读写的综合任务，如听后复述/读后仿写/情景对话创编）| 跨文化理解与实践（20-30%，中英文化比较/真实场景模拟/创意表达，如制作英文海报/写英文邮件）。全卷至少8-12个学习任务，每个任务有真实语言情境+交际活动+成果产出。禁止孤立单词默写和脱离语境的句型转换——词汇句型必须嵌入真实交际情境中。', builtin: true },
  { id: 'core_task_summary', name: '【核心任务】知识点总结', category: '生成-核心任务', prompt_order: 10, type: 'fragment', subject: '', stage: '', genType: 'summary', content: '生成一份知识点归纳总结。🔴 内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖所选知识点的全部要点（概念/公式/方法/应用），不得遗漏。以知识要点整理为核心，按下方结构参考组织内容。语言精炼规范，便于打印复习，不掺杂无关内容。', builtin: true },
  { id: 'core_task_special', name: '【核心任务】专项突破', category: '生成-核心任务', prompt_order: 10, type: 'fragment', subject: '', stage: '', genType: 'special', content: '生成一份专项突破训练（页数要求见角色身份）。🔴 题量以完整覆盖全部知识点为准——每个训练阶梯至少含3-6题，每题独立考查不同角度，禁止一个阶梯只放1题凑数。围绕指定专项能力，按下方结构参考进行"方法指导→典例剖析→阶梯训练→真题检验"的系统训练，帮助学生从"模仿"到"迁移"再到"创新"。', builtin: true },
  ...
  { id: 'core_task_preview', name: '【核心任务】课前预习', category: '生成-核心任务', prompt_order: 10, type: 'fragment', subject: '', stage: '', genType: 'preview', content: '生成一份课前预习资料（页数要求见角色身份）。🔴 预习内容完整、篇幅充分——完整覆盖新课全部核心知识点，每个知识点配至少1项预习任务或检测题，不能只列标题了事。按下方结构参考组织，帮助学生对新课内容有初步感知、带着问题进课堂。预习任务可操作可检查，预习检测答案需标注教材原文定位以便自查。', builtin: true },
  ...
  { id: 'core_task_review', name: '【核心任务】单元/期末复习', category: '生成-核心任务', prompt_order: 10, type: 'fragment', subject: '', stage: '', genType: 'review', content: '生成一份单元/期末系统复习资料（页数要求见角色身份）。🔴 内容完整、篇幅充分——完整覆盖所选范围的考点和易错点，每个核心考点配至少1道典型例题+解析，综合自测需覆盖全部考点。不仅是知识罗列，更要帮助学生构建知识关联和解题策略。按下方结构参考组织，知识梳理+典型题析+易错聚焦+综合自测四位一体。典型题必须配完整解析过程，自测题需覆盖全部核心考点。', builtin: true },
  ...
  { id: 'core_task_exam_primary_mid', name: '【核心任务-考卷-小学中段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学中段（3-4年级）的考试卷（页数要求见角色身份）。🔴 题量以完整覆盖全部知识点为准——每个大题组至少含3-6小题，每题独立考查不同角度。题干清晰。增加2-3道情境题和图表题，注重知识运用而非机械记忆。试题需原创改编（同资料内不同题目考查角度、情境、数据不得雷同）。正文只含题目，参考答案与解析统一放文末。', builtin: true },
  { id: 'core_task_exam_primary_high', name: '【核心任务-考卷-小学高段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学高段（5-6年级）的考试卷（页数要求见角色身份）。🔴 题量以完整覆盖全部知识点为准——每个大题组至少含3-6小题，每题独立考查不同角度。题型贴近小升初衔接要求，增加综合性、应用性题目。注重考查学生的分析、概括和表达能力。试题需原创改编（同资料内不同题目考查角度、情境、数据不得雷同）。正文只含题目，参考答案与解析统一放文末。', builtin: true },
  { id: 'core_task_exam_middle', name: '【核心任务-考卷-初中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份对标中考要求的考试卷（页数要求见角色身份）。🔴 题量以完整覆盖全部知识点为准——每个大题组至少含3-6小题，每题独立考查不同角度。题型仿照中考真题设计，注重考查核心素养和综合运用能力。试题区分度合理，基础题、中档题、较难题比例恰当。需原创改编（变换教材原题数据/情境/问法，同资料内考查角度不得雷同）。正文只含题目，参考答案与解析统一放文末。', builtin: true },
  { id: 'core_task_exam_high', name: '【核心任务-考卷-高中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份对标高考要求的考试卷（页数要求见角色身份）。🔴 题量以完整覆盖全部知识点为准——每个大题组至少含3-6小题，每题独立考查不同角度。题型仿照高考真题设计，注重考查学科核心素养、深度理解和创新思维。试题区分度合理，基础题、中档题、较难题比例恰当。需原创改编。正文只含题目，参考答案与解析统一放文末。', builtin: true },
  ...
  { id: 'core_task_practice_primary_low', name: '【核心任务-课时练-小学低段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学低段（1-2年级）的课堂同步学习任务单（页数要求见角色身份）。🔴 以学习任务为组织单位（非传统练习题堆砌）。全卷至少6-8个学习任务，每个任务含真实情境+核心活动+成果产出，任务之间体现认知递进。任务以游戏化、活动化方式呈现，符合低段学生特点。', builtin: true },
  { id: 'core_task_practice_primary_mid', name: '【核心任务-课时练-小学中段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学中段（3-4年级）的课堂同步学习任务单（页数要求见角色身份）。🔴 以学习任务为组织单位（非传统练习题堆砌）。全卷至少8-10个学习任务，每个任务含真实情境+核心活动+成果产出，任务之间体现认知递进（识记→理解→应用→分析→评价）。基础任务配简要解析，进阶任务配详细解析。', builtin: true },
  { id: 'core_task_practice_primary_high', name: '【核心任务-课时练-小学高段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学高段（5-6年级）的课堂同步学习任务单（页数要求见角色身份）。🔴 以学习任务为组织单位（非传统练习题堆砌）。全卷至少8-12个学习任务，每个任务含真实情境+核心活动+成果产出，任务之间体现认知递进（识记→理解→应用→分析→评价）。增加综合运用和探究性任务，贴近小升初衔接。进阶任务配详细解析。', builtin: true },
  { id: 'core_task_practice_middle', name: '【核心任务-课时练-初中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份初中课堂同步学习任务单（页数要求见角色身份）。🔴 以学习任务为组织单位（非传统练习题堆砌）。全卷至少8-12个学习任务，每个任务含真实情境+核心活动+成果产出，任务之间体现认知递进（识记→理解→应用→分析→评价）。任务设计注重知识迁移和综合运用，对标中考要求。进阶任务配详细解析和评分要点。', builtin: true },
  { id: 'core_task_practice_high', name: '【核心任务-课时练-高中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份高中课堂同步学习任务单（页数要求见角色身份）。🔴 以学习任务为组织单位（非传统练习题堆砌）。全卷至少8-12个学习任务，每个任务含真实情境+核心活动+成果产出，任务之间体现认知递进（识记→理解→应用→分析→评价）。任务设计注重深度理解和创新思维，对标高考要求。进阶任务配详细解析和评分标准。', builtin: true },

  // ── summary 知识点总结（按学段拆分）──
  { id: 'core_task_summary_primary_low', name: '【核心任务-总结-小学低段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    subject: '', stage: 'primary_low', genType: 'summary', specialSubType: 'new_standard',
    content: '生成一份适合小学低段（1-2年级）的知识点总结。🔴 内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖所选知识点的全部要点。知识呈现生动有趣，用符号和编号区分重点。语言亲切活泼，多用口诀和儿歌帮助记忆，减少抽象概念描述。按下方结构参考组织，便于打印复习。', builtin: true },
  { id: 'core_task_summary_primary_mid', name: '【核心任务-总结-小学中段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    subject: '', stage: 'primary_mid', genType: 'summary', specialSubType: 'new_standard',
    content: '生成一份适合小学中段（3-4年级）的知识点总结。🔴 内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖所选知识点的全部要点。知识呈现开始引入表格、思维导图等结构化工具，逐步培养归纳整理能力。语言清晰规范，重点内容用★标注和<strong>加粗突出。按下方结构参考组织，便于打印复习。', builtin: true },
  { id: 'core_task_summary_primary_high', name: '【核心任务-总结-小学高段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    subject: '', stage: 'primary_high', genType: 'summary', specialSubType: 'new_standard',
    content: '生成一份适合小学高段（5-6年级）的知识点总结。🔴 内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖所选知识点的全部要点。知识呈现以结构化梳理为主（思维导图/表格/层级图），注重知识点之间的联系和对比。语言精炼规范，标注重难点星级和考查频率。按下方结构参考组织，便于打印复习。', builtin: true },
  { id: 'core_task_summary_middle', name: '【核心任务-总结-初中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    subject: '', stage: 'middle', genType: 'summary', specialSubType: 'new_standard',
    content: '生成一份初中知识点总结。🔴 内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖所选知识点的全部要点。知识梳理系统深入，注重知识网络构建和跨章节联系。标注中考考查频率和常见题型，渗透学科思维方法。语言精炼规范，不掺杂无关内容。按下方结构参考组织，便于打印复习和备考。', builtin: true },
  { id: 'core_task_summary_high', name: '【核心任务-总结-高中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    subject: '', stage: 'high', genType: 'summary', specialSubType: 'new_standard',
    content: '生成一份高中知识点总结。🔴 内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖所选知识点的全部要点。知识梳理深度系统，注重概念原理的深层理解和跨模块整合。标注高考考查要求和命题趋势，渗透学科思想方法和解题策略。语言精炼规范。按下方结构参考组织，便于打印复习和高考备考。', builtin: true },
  ...
  { id: 'core_task_reading_primary_low', name: '【核心任务-阅读-小学低段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学低段（1-2年级）的阅读训练（页数要求见角色身份）。🔴 选文短小有趣（80-150字），以儿歌/童谣/简短童话为主，至少2篇。每篇配3-5题。（80-150字），以儿歌/童谣/简短童话为主。题目以信息提取和简单判断为主，注重培养阅读兴趣和习惯。每题配参考答案。', builtin: true },
  ...
  { id: 'core_task_preview_primary_low', name: '【核心任务-预习-小学低段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学低段（1-2年级）的课前预习单。🔴 预习内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖新课核心知识点，每个知识点均有对应的趣味预习任务。用"我能..."句式呈现学习目标，预习检测2-3道基础题。注重趣味性和亲子共读引导，帮助学生对新课有初步感知、带着兴趣进课堂。', builtin: true },
  { id: 'core_task_preview_primary_mid', name: '【核心任务-预习-小学中段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学中段（3-4年级）的课前预习单。🔴 预习内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖新课核心知识点，每个知识点均有对应的预习任务或检测题。增加自主学习和简单批注任务，预习检测3-4道基础题，引导学生带着问题进课堂。预习任务可操作可检查，检测答案标注教材原文定位。', builtin: true },
  { id: 'core_task_preview_primary_high', name: '【核心任务-预习-小学高段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学高段（5-6年级）的课前预习单。🔴 预习内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖新课核心知识点，每个知识点均有对应的预习任务或检测。侧重自主学习和探究，增加资料搜集、批注思考、提出疑问等任务。预习检测4-5道题（基础、理解），帮助学生对新课有系统感知、带着思考进课堂。', builtin: true },
  { id: 'core_task_preview_middle', name: '【核心任务-预习-初中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份初中课前预习资料。🔴 预习内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖新课全部核心知识点，每个知识点均有对应的预习任务或检测。预习任务强调自主研读教材、梳理知识框架、标注疑难问题。预习检测侧重理解性和应用性题目。帮助学生对新课有深度感知、带着问题和思考进课堂。', builtin: true },
  { id: 'core_task_preview_high', name: '【核心任务-预习-高中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份高中课前预习资料。🔴 预习内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖新课全部核心知识点，每个知识点均有对应的预习任务或检测。预习任务强调自主构建知识框架、关联已有知识、提出批判性问题。预习检测增加综合性和探究性题目。帮助学生对新课有系统深度理解、带着思考和独到见解进课堂。', builtin: true },

  // ── review 单元/期末复习（按学段拆分）──
  { id: 'core_task_review_primary_low', name: '【核心任务-复习-小学低段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    subject: '', stage: 'primary_low', genType: 'review', specialSubType: 'new_standard',
    content: '生成一份适合小学低段（1-2年级）的单元/期末复习资料。🔴 内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖所选范围的考点和易错点。知识梳理以趣味表格和图片配对为主，典型题析每题配完整图解式解析，自测题以选择+填空+连线为主（8-12题），帮助学生轻松回顾、快乐巩固。', builtin: true },
  { id: 'core_task_review_primary_mid', name: '【核心任务-复习-小学中段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    subject: '', stage: 'primary_mid', genType: 'review', specialSubType: 'new_standard',
    content: '生成一份适合小学中段（3-4年级）的单元/期末复习资料。🔴 内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖所选范围的考点和易错点。知识梳理用对比表格和知识树，典型题析每题配完整解析过程，易错聚焦整理3-5个高频错误，综合自测覆盖全部核心考点。帮助学生构建知识关联，系统查漏补缺。', builtin: true },
  { id: 'core_task_review_primary_high', name: '【核心任务-复习-小学高段】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    subject: '', stage: 'primary_high', genType: 'review', specialSubType: 'new_standard',
    content: '生成一份适合小学高段（5-6年级）的单元/期末复习资料。🔴 内容完整、篇幅精炼（页数要求见角色身份）。完整覆盖所选范围的考点和易错点。知识梳理注重知识体系构建和方法归纳，典型题析选涵盖全部核心考点的经典题并配详细解析，易错聚焦深入分析错误根源，综合自测涵盖3种以上题型、难度梯度合理。帮助学生从"学会"到"会学"，系统提升应试能力。', builtin: true },
  ...
  { id: 'core_task_special_primary', name: '【核心任务-专项突破-小学】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份适合小学阶段的专项突破训练。🔴 页数要求见角色身份。每个训练阶梯至少含3-6题，禁止一个阶梯只放1题凑数。覆盖专项能力的全部训练维度。围绕指定专项能力，按下方结构参考进行"方法指导→典例剖析→阶梯训练→真题检验"的系统训练。方法指导用简单易懂的语言，训练题难度梯度平缓，帮助学生从"模仿"到"迁移"再到"创新"。', builtin: true },
  { id: 'core_task_special_middle', name: '【核心任务-专项突破-初中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份初中专项突破训练。🔴 页数要求见角色身份。每个训练阶梯至少含3-6题，禁止一个阶梯只放1题凑数。覆盖专项能力的全部训练维度。围绕指定专项能力，按下方结构参考进行"方法指导→典例剖析→阶梯训练→真题检验"的系统训练。方法指导注重策略和技巧，训练题对标中考难度和题型，帮助学生从"模仿"到"迁移"再到"创新"。', builtin: true },
  { id: 'core_task_special_high', name: '【核心任务-专项突破-高中】', category: '生成-核心任务', prompt_order: 10, type: 'fragment',
    ...
    content: '生成一份高中专项突破训练。🔴 页数要求见角色身份。每个训练阶梯至少含3-6题，禁止一个阶梯只放1题凑数。覆盖专项能力的全部训练维度。围绕指定专项能力，按下方结构参考进行"方法指导→典例剖析→阶梯训练→真题检验"的系统训练。方法指导注重思维建模和解题通法，训练题对标高考难度和题型，帮助学生从"模仿"到"迁移"再到"创新"。', builtin: true },
  ...
  { id: 'quality_redlines_special', name: '【红线】专项突破最高优先级约束', category: '生成-红线约束', type: 'fragment', subject: '', stage: '', genType: 'special',
    content: '🔴🔴 【红线清单——最高优先级，违反即不合格】\n① 每道题必须独立原创设计：换情境、换数据、换设问角度，禁止课本例题/习题原题照搬。\n② 结构以【结构大纲】为准，不得自行另起骨架。\n③ 🔴 题型多样：同一专项内禁止连续2道以上使用完全相同的题型和设问方式，应变换考查角度。\n④ 设问层级覆盖理解/应用，每板块至少1题考查应用层次。\n⑤ 🔴 自校纠错：生成完成后必须通读全文自查，重点检查——①错别字/多字/漏字；②选项是否完整给出；③题干是否自洽；④格式是否规范。发现错误立即修正。', builtin: true },
  { id: 'quality_redlines_summary', name: '【红线】知识点总结最高优先级约束', category: '生成-红线约束', type: 'fragment', subject: '', stage: '', genType: 'summary',
    content: '🔴🔴 【红线清单——最高优先级，违反即不合格】\n① 每道题必须独立原创设计：换情境、换数据、换设问角度，禁止课本例题/习题原题照搬。\n② 知识点必须完整覆盖所选章节，不得只写部分章节。\n③ 知识梳理应展示思维结构（概念图/比较表/分类框架），不止罗列知识点。\n④ 🔴 自校纠错：生成完成后必须通读全文自查，重点检查——①错别字/多字/漏字；②知识点是否有遗漏；③格式是否规范。发现错误立即修正。', builtin: true },
  { id: 'quality_redlines_preview', name: '【红线】课前预习最高优先级约束', category: '生成-红线约束', type: 'fragment', subject: '', stage: '', genType: 'preview',
    content: '🔴🔴 【红线清单——最高优先级，违反即不合格】\n① 每道题必须独立原创设计：换情境、换数据、换设问角度，禁止课本例题/习题原题照搬。\n② 预习检测必须紧扣教材原文，不得脱离教材自拟内容。\n③ 🔴 自校纠错：生成完成后必须通读全文自查，重点检查——①错别字/多字/漏字；②拼音/笔画/部首是否正确；③格式是否规范。发现错误立即修正。', builtin: true },
  { id: 'quality_redlines_reading', name: '【红线】阅读训练最高优先级约束', category: '生成-红线约束', type: 'fragment', subject: '', stage: '', genType: 'reading',
    content: '🔴🔴 【红线清单——最高优先级，违反即不合格】\n① 每道题必须独立原创设计：换情境、换数据、换设问角度，禁止课本例题/习题原题照搬。\n② 选文必须原创编写，不得复制教材课文或网络文章。\n③ 🔴 自校纠错：生成完成后必须通读全文自查，重点检查——①错别字/多字/漏字；②选文是否有语病/逻辑不通；③设问是否自洽（题干信息与答案是否匹配）；④格式是否规范。发现错误立即修正。', builtin: true },
  { id: 'quality_redlines_review', name: '【红线】复习总结最高优先级约束', category: '生成-红线约束', type: 'fragment', subject: '', stage: '', genType: 'review',
    content: '🔴🔴 【红线清单——最高优先级，违反即不合格】\n① 每道题必须独立原创设计：换情境、换数据、换设问角度，禁止课本例题/习题原题照搬。\n② 考点梳理必须覆盖所选章节全部知识点。\n③ 🔴 题型多样：同一复习板块内禁止连续2道以上使用完全相同的题型和设问方式，应变换考查角度。\n④ 设问层级兼顾识记/理解/应用/分析，每板块至少1题考查理解或以上层次。\n⑤ 🔴 自校纠错：生成完成后必须通读全文自查，重点检查——①错别字/多字/漏字；②知识点是否有遗漏；③格式是否规范。发现错误立即修正。', builtin: true },
  { id: 'quality_redlines_errorbook', name: '【红线】错题本最高优先级约束', category: '生成-红线约束', type: 'fragment', subject: '', stage: '', genType: 'errorbook',
    content: '🔴🔴 【红线清单——最高优先级，违反即不合格】\n① 每道题必须独立原创设计：换情境、换数据、换设问角度，禁止课本例题/习题原题照搬。\n② 错题必须为典型错例，不得编造无意义的低级错误。\n③ 🔴 自校纠错：生成完成后必须通读全文自查，重点检查——①错别字/多字/漏字；②错因分析是否正确；③答案是否准确；④格式是否规范。发现错误立即修正。', builtin: true },
  { id: 'quality_redlines_dictation', name: '【红线】默写训练最高优先级约束', category: '生成-红线约束', type: 'fragment', subject: '', stage: '', genType: 'dictation',
  ...
  { id: 'quality_textbook_std', name: '【品质】教辅编辑标准·全类型', category: '生成-品质标准', type: 'fragment',
    ...
    content: '🔴【教辅达标标准】1.充实度：禁"只列标题无内容"骨架章节、禁1-2行充一页。2.答案完整度：与题一一对应，客观唯一，主观含"思路→步骤→易错点"三层解析，禁"略"式空泛。3.自洽：题干/选项/答案/解析数据单位一致，无错别字。', builtin: true },


  // ── 学科专属命题技法（按 subject + genType 精确匹配，与通用命题技法块同时注入）──
  { id: 'quality_exam_prop_chinese', name: '【品质】考卷命题技法-语文', category: '生成-品质标准', type: 'fragment',
    subject: '语文', stage: '', genType: 'exam',
    content: '🔴 【语文考卷题型规范】\n1. 文言文（初中/高中）：断句、实词虚词、句子翻译（直译为主）、文意理解；选文课内优先，不考课外超纲篇目。\n2. 古诗鉴赏：围绕炼字/意象/情感/手法四类设问，避免套话式提问。\n3. 作文：命题/半命题/材料/话题四种形式轮换；明确字数与文体；评分要点=内容+语言+结构+书写。\n4. 综合性学习（初中/高中）：活动设计类题（拟标语/写开场白/设计活动流程），设问贴近中考/高考真题形式。', builtin: true },
  { id: 'quality_exam_prop_math', name: '【品质】考卷命题技法-数学', category: '生成-品质标准', type: 'fragment',
    subject: '数学', stage: '', genType: 'exam',
    content: '🔴 【数学考卷题型规范】\n1. 计算题格式：竖式/脱式/简便运算格式规范，验算要求明确。\n2. 作图题：尺规作图要求明确，图例与标注规范。\n3. 证明题（初中/高中）：步骤严谨、逻辑链条完整，不考偏题怪题。\n4. 统计与概率题：图表数据真实，设问指向数据分析能力。', builtin: true },
  { id: 'quality_exam_prop_english', name: '【品质】考卷命题技法-英语', category: '生成-品质标准', type: 'fragment',
    subject: '英语', stage: '', genType: 'exam',
    content: '🔴 【英语考卷题型规范】\n1. 词汇题：低段看图选词/连线，中高段按首字母/中文提示填空（置于语境中）。\n2. 选择题：考查语法/词汇辨析，选项语法全部正确（不以语法错误制干扰项）。\n3. 连词成句/句型转换：考查目标句式，答案唯一。\n4. 完形填空（初中）：上下文线索明确，无争议答案。\n5. 补全对话/情景交际：情境真实，交际用语地道；设空位置合理，答案唯一。', builtin: true },
  { id: 'quality_exam_prop_physics', name: '【品质】考卷命题技法-物理', category: '生成-品质标准', type: 'fragment',
    subject: '物理', stage: '', genType: 'exam',
    content: '🔴 【物理考卷题型规范】\n1. 实验题：器材、步骤、现象、结论四要素完整；控制变量明确。\n2. 作图题：光路图/受力分析图规范（箭头、标号、虚实线）。', builtin: true },
  { id: 'quality_exam_prop_chemistry', name: '【品质】考卷命题技法-化学', category: '生成-品质标准', type: 'fragment',
    subject: '化学', stage: '', genType: 'exam',
    content: '🔴 【化学考卷题型规范】\n1. 化学式/方程式书写规范（配平、反应条件、气体/沉淀箭头）。\n2. 实验题：操作步骤、现象描述、结论分析三要素。\n3. 推断题：物质转化关系清晰、题眼明确、无多解歧义。', builtin: true },
  { id: 'quality_exam_prop_biology', name: '【品质】考卷命题技法-生物', category: '生成-品质标准', type: 'fragment',
    subject: '生物', stage: '', genType: 'exam',
    content: '🔴 【生物考卷题型规范】\n1. 实验设计：变量控制、对照实验、结论表述规范。\n2. 图表题：结构图、曲线图、柱状图标注准确。\n3. 术语使用规范（如写"液泡"不写"泡"，写"细胞膜"不写"膜"）。', builtin: true },
  { id: 'quality_exam_prop_politics', name: '【品质】考卷命题技法-道法/政治', category: '生成-品质标准', type: 'fragment',
    subject: '道德与法治,政治,思想政治', stage: '', genType: 'exam',
    content: '🔴 【道法/政治考卷题型规范】\n1. 材料分析题：结合具体情境与法条/观点，采用"观点+材料+分析"三段式作答。\n2. 主观题：设问指向明确（是什么/为什么/怎么做三类型）。', builtin: true },
  { id: 'quality_exam_prop_history', name: '【品质】考卷命题技法-历史', category: '生成-品质标准', type: 'fragment',
    subject: '历史', stage: '', genType: 'exam',
    content: '🔴 【历史考卷题型规范】\n1. 年代/人物/事件准确无误，不出现史实错误。\n2. 论述题：史实+观点+论证结构完整，按"是什么-为什么-怎么样"展开。', builtin: true },
  { id: 'quality_exam_prop_geography', name: '【品质】考卷命题技法-地理', category: '生成-品质标准', type: 'fragment',
    subject: '地理', stage: '', genType: 'exam',
    content: '🔴 【地理考卷题型规范】\n1. 读图题：地图/示意图图例规范，设问指向读图能力。\n2. 地名/数据准确，区域定位无错误。', builtin: true },

  // ── 学科专属考卷反套路（按 subject + genType 精确匹配，只注入当前学科的）──
  { id: 'quality_exam_antipattern_chinese', name: '【品质-考卷-语文】反套路', category: '生成-品质标准', type: 'fragment', subject: '语文', stage: '', genType: 'exam',
    content: '语文考卷禁止：孤立看拼音写词、无上下文组词填空、纯问答式阅读理解（改带任务阅读：批注/比较/推理）。', builtin: true },
  { id: 'quality_exam_antipattern_math', name: '【品质-考卷-数学】反套路', category: '生成-品质标准', type: 'fragment', subject: '数学', stage: '', genType: 'exam',
    content: '数学考卷禁止：纯计算题堆砌、脱离实际情境的应用题、考查超纲公式或定理。', builtin: true },
  { id: 'quality_exam_antipattern_english', name: '【品质-考卷-英语】反套路', category: '生成-品质标准', type: 'fragment', subject: '英语', stage: '', genType: 'exam',
    content: '英语考卷禁止：孤立单词默写（改在语篇中考查）、纯语法选择题堆砌（改情境语法运用）、无语境翻译题。', builtin: true },
  { id: 'quality_exam_antipattern_science', name: '【品质-考卷-科学】反套路', category: '生成-品质标准', type: 'fragment', subject: '科学', stage: '', genType: 'exam',
    content: '科学考卷禁止：纯概念填空堆砌、脱离实验情境的知识考查、无探究过程的结论题。', builtin: true },
  { id: 'quality_exam_antipattern_physics', name: '【品质-考卷-物理】反套路', category: '生成-品质标准', type: 'fragment', subject: '物理', stage: '', genType: 'exam',
    content: '物理考卷禁止：脱离真实情境的公式套用、无实验背景的结论判断题、纯计算堆砌。', builtin: true },
  { id: 'quality_exam_antipattern_chemistry', name: '【品质-考卷-化学】反套路', category: '生成-品质标准', type: 'fragment', subject: '化学', stage: '', genType: 'exam',
    content: '化学考卷禁止：脱离实验情境的方程式默写、无实际应用的化学计算堆砌、忽略安全规范的操作题。', builtin: true },
  { id: 'quality_exam_antipattern_biology', name: '【品质-考卷-生物】反套路', category: '生成-品质标准', type: 'fragment', subject: '生物', stage: '', genType: 'exam',
    content: '生物考卷禁止：脱离真实情境的概念填空、无图表/模式图的识记题、忽略实验设计的结论题。', builtin: true },
  { id: 'quality_exam_antipattern_history', name: '【品质-考卷-历史】反套路', category: '生成-品质标准', type: 'fragment', subject: '历史', stage: '', genType: 'exam',
    content: '历史考卷禁止：纯年代/人名/事件填空、脱离史料的结论判断题、无材料分析的观点题。', builtin: true },
  { id: 'quality_exam_antipattern_geography', name: '【品质-考卷-地理】反套路', category: '生成-品质标准', type: 'fragment', subject: '地理', stage: '', genType: 'exam',
  ...
  { id: 'quality_exam_antipattern_art', name: '【品质-考卷-美术】反套路', category: '生成-品质标准', type: 'fragment', subject: '美术', stage: '', genType: 'exam',
    content: '美术考卷禁止：纯技法名词填空、脱离视觉体验的判断题。', builtin: true },
  { id: 'quality_exam_antipattern_pe', name: '【品质-考卷-体育】反套路', category: '生成-品质标准', type: 'fragment', subject: '体育', stage: '', genType: 'exam',
    content: '体育考卷禁止：纯规则条文填空、脱离运动实践的判断题。', builtin: true },

  // ── 学科专属课时练反套路（按 subject + genType 精确匹配）──
  { id: 'quality_practice_antipattern_chinese', name: '【品质-课时练-语文】反套路', category: '生成-品质标准', type: 'fragment', subject: '语文', stage: '', genType: 'practice',
    content: '⛔ 【语文课时练反套路】\n❌ 语文反例1：孤立看拼音写词语 —— "kàn jiàn (    )" ← 严禁！必须嵌入句段语境中考查\n❌ 语文反例2：无上下文组词填空 —— "美(    ) (    )明" ← 严禁！必须结合课文语境\n❌ 语文反例3：纯抄写类机械训练 —— "把下面生字抄写5遍" ← 严禁！改为在语境中运用（如"用这个字组词并写一句话"）\n⚠️ 以上3条为语文课时练硬性红线。', builtin: true },
  { id: 'quality_practice_antipattern_math', name: '【品质-课时练-数学】反套路', category: '生成-品质标准', type: 'fragment', subject: '数学', stage: '', genType: 'practice',
    content: '⛔ 【数学课时练反套路】\n❌ 数学反例1：纯算式堆砌 —— "25×4=(    ) 36÷6=(    )" ← 严禁！必须嵌入生活情境\n❌ 数学反例2：脱离情境的应用题 —— "小明有10个苹果，吃了3个，还剩几个？"之后连续10道同类改数题 ← 严禁！每道题需变换情境和问题类型\n⚠️ 所有计算和应用必须嵌入真实生活或数学实践情境，杜绝机械操练。', builtin: true },
  ...
  { id: 'quality_reading', name: '【品质】阅读训练专属', category: '生成-品质标准', type: 'fragment', genType: 'reading',
    content: '阅读训练品质：短文配3-5道分层理解题（信息提取→推断解释→评价反思），题型多样化（选择/填空/简答/批注）。题目不可直接在原文找到原句答案。', builtin: true },
  { id: 'quality_summary', name: '【品质】知识点总结专属', category: '生成-品质标准', type: 'fragment', genType: 'summary',
    content: '知识点总结品质：知识结构化呈现（思维导图/表格/时间线优先），每条知识点标注教材出处。重点用★或加粗标注，不堆砌大段文字。', builtin: true },
  { id: 'quality_special', name: '【品质】专项突破专属', category: '生成-品质标准', type: 'fragment', genType: 'special',
    content: '专项突破品质：聚焦薄弱知识点的深度训练，含方法指导+情境典例+分层变式+综合实践，难度阶梯递进，避免同难度简单重复。', builtin: true },
  { id: 'quality_errorbook', name: '【品质】错题本专属', category: '生成-品质标准', type: 'fragment', genType: 'errorbook',
    content: '错题本品质：每题含原题重现+错误归因（具体到知识点层面）+正确解法（分步思路），同类错题归纳解题策略。归因禁止笼统描述"粗心""不会"。', builtin: true },
  // ── 学科专属默写反套路（按 subject + genType 精确匹配）──
  { id: 'quality_dictation_antipattern_chinese', name: '【品质-默写-语文】格式', category: '生成-品质标准', type: 'fragment', subject: '语文', stage: '', genType: 'dictation',
    content: '语文默写禁止：孤立看拼音写汉字（改在语境中默写）、无上下文的词语填空。', builtin: true },
  { id: 'quality_dictation_antipattern_english', name: '【品质-默写-英语】格式', category: '生成-品质标准', type: 'fragment', subject: '英语', stage: '', genType: 'dictation',
    content: '英语默写禁止：孤立单词拼写（改在句子中默写）、不标音标词性的单词表。', builtin: true },
  // （已删除 quality_dictation_type——与 quality_dictation 内容重复 90%+，同 category 同 genType 双注入，v28 去冗余）
  { id: 'quality_review', name: '【品质】复习专属', category: '生成-品质标准', type: 'fragment', genType: 'review',
    content: '复习资料品质：知识整合+易错梳理+变式训练三合一。知识整合以结构化方式呈现（表格/对比/思维导图），易错点标注典型错误和正确解法，变式题不少于3道。', builtin: true },

  // ═══════════════════════════════════════
  // 【生成-命题风格】块级指令 — 按命题风格值匹配
  // ═══════════════════════════════════════
  { id: 'style_traditional', name: '【命题风格】传统命题', category: '生成-命题风格', prompt_order: 36, type: 'fragment', subject: '', stage: '', genType: 'traditional', content: '题型结构清晰，设问直接，知识点考查明确。每道题须有简短真实情境或明确任务载体（一句话场景即可），确保符合新课标情境化要求，但不需要跨题统一故事线。', builtin: true },
  ...
  { id: 'gen_struct_dictation_chinese_primary_mid', name: '【默写结构-语文-小学中段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、语境默写（在句段中默写字词成语，勿孤立罗列）\n二、理解运用（语境辨析、迁移运用）\n三、综合默写（段落/短篇）', builtin: true },
  ...
  { id: 'gen_struct_dictation_english_primary_high', name: '【默写结构-英语-小学高段】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、语境默写（在句子/语篇中拼写单词，勿孤立罗列）\n二、句型默写（语境中考查）\n三、综合默写（短文填空/语篇）', builtin: true },
  // ── 语文 dictation（初中/高中）──
  ...
  { id: 'gen_struct_dictation_english_high', name: '【默写结构-英语-高中】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、语境默写（在语篇中考查词汇搭配，勿孤立罗列）\n二、句型变换（语境中考查语法活用）\n三、综合默写（语篇/短文）', builtin: true },
  ...
  { id: 'gen_struct_review_generic', name: '【复习结构-通用】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    ...
    content: '结构参考：\n一、知识框架\n二、考点梳理\n三、典型题析\n四、易错聚焦\n五、综合自测', builtin: true },
  // ── 道德与法治 review（小学段）──
  { id: 'gen_struct_review_moral_primary', name: '【复习结构-道德与法治-小学】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '道德与法治,政治', stage: 'primary_low,primary_mid,primary_high', genType: 'review', specialSubType: 'new_standard',
    content: '结构参考：\n一、知识框架\n二、核心观念回顾\n三、行为辨析\n四、易错聚焦\n五、综合自测', builtin: true },
  // ── 音乐/美术/体育 review（2022 艺术/体育课标，全学段通用）──
  { id: 'gen_struct_review_music', name: '【复习结构-音乐】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '音乐', stage: '', genType: 'review', specialSubType: 'new_standard',
    content: '结构参考：\n一、知识框架\n二、核心作品回顾\n三、表现实践\n四、易错聚焦\n五、综合自测', builtin: true },
  { id: 'gen_struct_review_art', name: '【复习结构-美术】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '美术', stage: '', genType: 'review', specialSubType: 'new_standard',
    content: '结构参考：\n一、知识框架\n二、核心作品回顾\n三、创作实践\n四、易错聚焦\n五、综合自测', builtin: true },
  { id: 'gen_struct_review_pe', name: '【复习结构-体育】', category: '生成-资料类型结构', prompt_order: 11, type: 'fragment',
    subject: '体育', stage: '', genType: 'review', specialSubType: 'new_standard',
    content: '结构参考：\n一、知识框架\n二、核心技能回顾\n三、实践演练\n四、易错聚焦\n五、综合自测', builtin: true },
  ...
  // ═══════════════════════════════════════
  { id: 'format_tail_question', name: '【格式尾约束】命题类', category: '生成-格式尾约束', prompt_order: 98, type: 'fragment', subject: '', stage: '', genType: 'exam,special,reading,preview', content: '⛔ 每道题目必须用 <p class="question"> 标签包裹！禁止使用无 class 的 <p>、<div> 等标签代替题目行', builtin: true },
  { id: 'format_tail_other', name: '【格式尾约束】非命题类', category: '生成-格式尾约束', prompt_order: 98, type: 'fragment', subject: '', stage: '', genType: 'summary,dictation,errorbook,practice,review', content: '⛔ 严格遵循上方【输出格式】中的标签规范，禁止使用 Markdown 语法。任务/条目类资料以语义化标签组织结构，不使用 <p class="question"> 包裹。', builtin: true },
  ...
  { id: 'section_title_redline_constraint', name: '【红线约束】', category: '生成-指令块标题', prompt_order: 60, type: 'fragment', subject: '', stage: '', genType: 'redline_constraint', content: '红线约束', builtin: true },
 ...
 { id: 'block_coverage_special_english_grammar', name: '【知识点全覆盖-英语语法】', category: '生成-知识点全覆盖', type: 'fragment',
  ...
  { id: 'stage_high', name: '【学段控制】高中', category: '生成-学段控制', prompt_order: 52, type: 'fragment', subject: '', stage: 'high', genType: '',
    content: '当前为高中阶段：知识模块化，思辨能力成熟；认知层次应用、分析、评价、创造均允许；考查深度理解、创新思维和综合运用，对标高考题型和难度，可设压轴题。', builtin: true },

  // 【生成-题量控制】块级指令 — 按学段精准匹配（补建 D）
  // ═══════════════════════════════════════
  // ── 语文专用（语篇类，含阅读理解小题，题量低于数理类）──
  { id: 'layout_chinese_primary_low', name: '【题量控制】语文-小学低段', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '语文', stage: 'primary_low', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量10-18题（含阅读理解的各小题），每大题含2-5小题；以汉字书写和简单阅读为主，不追求大数量；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_chinese_primary_mid', name: '【题量控制】语文-小学中段', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '语文', stage: 'primary_mid', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量12-22题（含阅读理解的各小题），每大题含3-5小题；阅读理解1-2篇，开始片段写作；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_chinese_primary_high', name: '【题量控制】语文-小学高段', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '语文', stage: 'primary_high', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量15-25题（含阅读理解的各小题），每大题含3-6小题；阅读理解2篇+完整作文1篇；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_chinese_middle', name: '【题量控制】语文-初中', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '语文', stage: 'middle', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量15-28题（含阅读理解的各小题），每大题含4-8小题；现代文+文言文+基础+作文，各板块按权重分配题量；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_chinese_high', name: '【题量控制】语文-高中', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '语文', stage: 'high', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量15-30题（含阅读理解的各小题），每大题含4-8小题；多篇阅读+语言文字运用+作文，各板块按高考权重分配题量；考查面均衡，避免同一知识点重复出题。', builtin: true },
  // ── 英语专用（语篇驱动，题量低于数理类）──
  { id: 'layout_english_primary_low', name: '【题量控制】英语-小学低段', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '英语', stage: 'primary_low', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量8-15题，以字母/单词认读和简单书写为主；题干配中文提示；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_english_primary_mid', name: '【题量控制】英语-小学中段', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '英语', stage: 'primary_mid', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量12-20题，词汇拼写+简单句型+短文阅读；逐步减少中文提示；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_english_primary_high', name: '【题量控制】英语-小学高段', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '英语', stage: 'primary_high', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量15-25题，短文阅读+简单写作+基础语法；逐步增加英文题干；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_english_middle', name: '【题量控制】英语-初中', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '英语', stage: 'middle', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量15-30题，多语篇阅读+完形填空+写作；对标中考题型和分值；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_english_high', name: '【题量控制】英语-高中', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '英语', stage: 'high', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量15-35题，多语篇深度阅读+完形+语法填空+写作；对标高考题型和分值；考查面均衡，避免同一知识点重复出题。', builtin: true },
  // ── 通用（数理等离散题类，无学科专用条目时兜底）──
  { id: 'layout_primary_low', name: '【题量控制】小学低段', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '', stage: 'primary_low', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量15-25题，填空2-3字答案，选择题2-3选项，每大题含2-4小题；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_primary_mid', name: '【题量控制】小学中段', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '', stage: 'primary_mid', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量20-30题，填空3-5字答案，选择题3-4选项，每大题含3-5小题；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_primary_high', name: '【题量控制】小学高段', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '', stage: 'primary_high', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量25-35题，填空3-6字答案，选择题4选项，每大题含3-6小题；考查面均衡，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_middle', name: '【题量控制】初中', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '', stage: 'middle', genType: 'exam,practice,special,errorbook',
    content: '题量建议：总题量25-40题，选择题4选项，每大题含4-8小题；考查面均衡，各知识点按权重分配题量，避免同一知识点重复出题。', builtin: true },
  { id: 'layout_high', name: '【题量控制】高中', category: '生成-题量控制', prompt_order: 53, type: 'fragment', subject: '', stage: 'high', genType: 'exam,practice,special,errorbook',
  ...
  { id: 'core_framework_middle', name: '【核心素养框架】初中', category: '生成-学科核心素养', prompt_order: 55, type: 'fragment', subject: '', stage: 'middle', genType: '',
    content: '初中阶段核心素养考查侧重：在较复杂情境中运用学科知识解决问题，体现学科思维方法的初步形成，可设置少量跨学科融合题目。', builtin: true },
...

  {
    ...
    content: '小学低段（1-2年级）：情境化≥40%（与真题蓝本学段条款一致），以趣味主题贯穿，情境贴近低龄学生生活（校园/家庭/游戏），避免成人化。题干简短（≤15字）。字词在句段语境中考查，阅读以课内原文为准；如配课外短文120-180字。写话配词语支架。设问兼顾识记与简单理解，每大题至少1题需简单推理后作答（非直接默写）。',
    builtin: true
  },
  {
    id: 'quality_exam_primary_mid', name: '⭐ 品质标准-试卷-小学中段', category: '生成-品质标准', type: 'fragment',
    subject: '', stage: 'primary_mid', genType: 'exam',
    content: '小学中段（3-4年级）：情境化≥60%（与真题蓝本学段条款一致），基础60:能力30:拓展10。单篇阅读200-400字。所有题型嵌入情境，情境源于学生可感知的真实世界（社区/自然/日常），同知识点变换角度考查，杜绝孤立默写填空。设问层级兼顾识记/理解/应用，每大题至少1题考查理解或应用层次。',
    builtin: true
  },
  {
    id: 'quality_exam_primary_high', name: '⭐ 品质标准-试卷-小学高段', category: '生成-品质标准', type: 'fragment',
    subject: '', stage: 'primary_high', genType: 'exam',
    content: '小学高段（5-6年级）：情境化≥60%（与真题蓝本学段条款一致），以小初衔接为导向。难度按基础约50%、中档约30%、提高约20%（与难度配比一致）。含非连续性文本阅读（图表/说明书），开放性试题附评分要点。设问角度新颖，考查知识综合运用与迁移，杜绝陈题套路。设问层级覆盖理解/应用/分析，每板块至少1题考查分析层级。',
    builtin: true
  },
  {
    id: 'quality_exam_middle', name: '⭐ 品质标准-试卷-初中', category: '生成-品质标准', type: 'fragment',
    subject: '', stage: 'middle', genType: 'exam',
    content: '初中：情境化≥60%，难度按基础约60%、中档约30%、提高约10%（与难度配比一致）。情境真实有社会性（科技/环保/社会），重视实验探究、材料分析、综合运用能力，答案需完整解题思路。设问层级覆盖理解/应用/分析/综合，每板块至少1题考查分析或综合层级。',
    builtin: true
  },
  {
  ...
  },
  {
    ...
    content: '小学低段（1-2年级）：任务形式多样化（读/写/说/做/演），每任务含情境+活动+产出，全卷8-10个学习任务。',
    builtin: true
  },
  {
    ...
    content: '小学中段（3-4年级）：任务形式多样化（读/写/说/做/演），每任务含情境+活动+产出，全卷10-14个学习任务。',
    builtin: true
  },
  {
  ...
  // ── dictation 默写逐学段品质标准 ──
  {
    ...
    content: '小学低段（1-2年级）：语文拼音+田字格，英语四线三格。',
    builtin: true
  },
  {
    ...
    content: '小学中段（3-4年级）：语文田字格+语境默写，英语四线三格。',
    builtin: true
  },
  {
...
// 自动为明显关键条目标注 guarantee:true，便于后续复核和持久化
builtinInstructions.forEach(item => {
  const id = (item.id || '').toString();
  const cat = (item.category || '').toString();
  if (/^(role_|topconst_|tailconst_)/.test(id) || id === 'answer_anchor_question' || cat === '生成-答案区强制锚定' || cat === '生成-顶层约束' || cat === '生成-角色身份') {
    item.guarantee = true;
  }
});
...
export const getMatchingBlockInstructions = (options = {}) => {
  ...
  const matches = all.filter(i => {
    ...
    // 🔧 P0 修复：块侧 stage='primary'（通用小学）也等价于任一小学子段
    //   之前仅查询侧 'primary' 做展开，块侧用 'primary' 的条目（如 gen_ctx_primary_it、
    //   stage_primary_low/mid/high 等）在 gradeSegment='primary_low' 查询时失配
    if (i.stage && i.stage.trim() !== '') {
      if (!stage) return false;
      let insStages = i.stage.split(',').map(s => s.trim());
      // 块侧 primary 兜底：条目 stage='primary' 也等价于任一小学子段
      if (insStages.includes('primary')) {
        insStages = [...insStages, 'primary_low', 'primary_mid', 'primary_high'];
      }
      // 查询侧 primary 兜底：传入 primary 时，等价于任一小学子段
  ...
  if (matches.length > 1) {
    // 先为每个匹配项计算显式保留与特异性分数，便于后续按保证/特异性排序
    matches.forEach(item => {
      // 自动判断 guarantee（必须保留的关键块）：仅基于明确的 id 前缀或类别，避免误判
      const id = (item.id || '').toString();
      const cat = (item.category || '').toString();
      item._guarantee = /^(role_|topconst_|tailconst_)/.test(id) || id === 'answer_anchor_question' || cat === '生成-答案区强制锚定' || cat === '生成-顶层约束' || cat === '生成-角色身份';
      let s = 0;
    ...
    // 保证项优先，然后按匹配分降序
    matches.sort((a, b) => {
      if ((b._guarantee ? 1 : 0) !== (a._guarantee ? 1 : 0)) return (b._guarantee ? 1 : 0) - (a._guarantee ? 1 : 0);
