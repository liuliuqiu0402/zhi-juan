...
export const VALIDATOR_RULES = [
  ...
  },
  {
    ...
    stages: ['*'],
    genTypes: ['exam'],
    promptHint: '分值账目自洽：小题数×每题分=大题分、空数×每空分=小题分、各大题分之和=满分，必须精确成立；多空题注明"（每空X分）"；声称的题数/空位数必须等于正文实际输出的题号/空位（不虚报、不缩水）。分值标注用中文全角括号（如"（每空2分）""（共12分）"），与卷面其余括注同形。',
    description: '大题/小题标题"每空/每组/每题/每词/每字 X 分"标注与实际载体数校验（只报不改，程序不再重算改写）：空按 DOM 空位数、配对/匹配题按匹配对数、词按拼音组数（看拼音写词语）、字按田字格格子数；声称数≠实际载体数时仅静默计数抽检（分数账目由模型自洽）。🔴 2026-09-17 括号口径统一为**全角**：原句"分值标注一律半角括号…不得用中文全角括号"与同一份提示词里委托正文的两处全角示范、以及程序侧 fixScoreLabel 的确定性改写（输出"（每空X分）"）三处相抵（实测第②类打架）；程序侧解析（examValidator 2f/2g/2h、themeConfig.parseSectionScore）半角/全角均兼容，故按"程序实际写出的形态"统一为全角。',
    enabled: true,
  },

  {
    ...
    stages: ['*'],
    genTypes: ['exam'],
    promptHint: '大题标题用明细式"共X题，每题X分，共X分"（题量按你实际命制的题数、分值按实际赋分给出），标题须含题量与分值；小题题号后/题干末尾标注"（X分）"，与卷面其余括注同形。',
  ...
  },
  {
    ...
    subjects: ['*'],
    stages: ['*'],
    promptHint: '配图块格式：凡输出配图，一律用 [IMAGE] 块（每图一个、单独成段），参数独占一行、半角冒号。',
  ...
  },
  {
    ...
    subjects: ['语文'],
    stages: ['*'],
    promptHint: '排版语义标记规范（语文加点）：题干要求“圈出加点字/给加点字选择正确读音”的字用 <span class="emphasis-dot">字</span> 标记（字下加点；表示加点时不得用 <u> 下划线）；🔴 自洽性硬性：题干要求“圈出加点字/给加点字选择读音”的题，正文必须恰好存在对应 emphasis-dot 标记，题干有要求而正文无标记即视为无效题。',
    description: '语文“圈出加点字/给加点字选择读音”题型的加点标记与自洽性硬性，按学科精确注入（仅语文，其他学科不注入，杜绝跨学科诱导）；生成后自洽检测由汇总规则 text-format-fix 统一执行。',
    enabled: true,
  },
  {
    ...
    subjects: ['语文', '英语'],
    stages: ['*'],
    promptHint: '排版语义标记规范（画线）：题干要求“画线句子/画线词语/画线部分/划出文中…句”处用 <u class="underline-sentence">…</u> 标记（划出/画出=画线用 <u>，划去/删去=删除用 <del>）；画线部分特指须在词内标出的目标字母/字词（如英语“选出画线部分发音不同的单词”：选项词内把目标字母组合画线，see → <u class="underline-sentence">ee</u>）；🔴 自洽性硬性：题干要求“画线句子/画线词语/画线部分”的题，正文必须恰好有对应标记，题干有要求而正文无标记即视为无效题。',
  ...
  },
  {
    ...
    subjects: ['数学', '化学', '物理'],
    stages: ['*'],
    promptHint: '排版语义标记规范（上下标）：凡需上升/下降排版的字符一律用语义标记——上标用 <sup>…</sup>（如 10<sup>2</sup> 的 2），下标用 <sub>…</sub>（如 X<sub>n</sub> 的 n）；学科中出现幂/指数、单位中的指数、化学式原子数、离子电荷、物理量角标等情形按此标记；禁止直接输出 Unicode 上下标字符（²³⁺ₙ等，导出后字号/基线不统一）。',
  ...
  },
  {
    ...
    subjects: ['*'],
    stages: ['*'],
    promptHint: '任何书写载体（横线/格子/空白行）均为空位，严禁预填字或答案，格内留空供作答（载体类型按已注入的书写载体协议与作答空间条款，不重复展开）；作图按学科协议输出作答区。',
  ...
  },
  {
    ...
    subjects: ['语文', '英语'],
    stages: ['*'],
    promptHint: '写话/习作/作文/书面表达等表达类题不得混入书写格（田字格/四线三格等）；表达类书写载体（作文格/书写行/横线，按学科与题型由系统配置对应形态）不要在正文自行输出"××格"等载体字样；',
...

  {
    ...
    subjects: ['*'],
    stages: ['*'],
    description: '题面带选项（A./B./C. 等）的题，其答案括号应在题干前（题首）；选项行内/选项末尾出现 blank 空位（模型常把作答位误挂选项后，2026-09 实证）时静默计数（debug 级）——不自动修复：位置矫正属生成语义，靠生成前约束（作答空间条款）根治。',
    enabled: true,
  },
