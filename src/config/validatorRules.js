...
export const VALIDATOR_RULES = [
  ...
  },
  {
    ...
    stages: ['*'],
    genTypes: ['exam'],
    promptHint: '分值账目自洽：小题数×每题分=大题分、空数×每空分=小题分、各大题分之和=满分，必须精确成立；多空题注明"（每空X分）"；声称的题数/空数/连线组数必须等于正文实际输出的题号/空位/连线组数。',
    description: '大题/小题标题"每空/每线/每组/每题/每词/每字 X 分"标注与实际载体数校验：空/线/组/题按 DOM 空位/连线组数（"每线/每组"同义，输出归一为"组"）/子题数、词按拼音组数（看拼音写词语）、字按田字格格子数，不一致时以实际载体数为准按「声称单位分×实际载体数」重算总分（不凑数；数不到载体时保留原标注）。',
    enabled: true,
  },
