...
export const VALIDATOR_RULES = [
  ...
  {
    id: 'text-format-fix',
    name: '排版语义标记规范（加点/画线/加粗/注音自洽）',
    category: 'fix',
    subjects: ['*'],
    stages: ['*'],
    promptHint: '排版语义标记规范：①加点字（题干要求"圈出加点字"的字）用 <u>加点字</u> 标记；②画线词语/句子用 <u>…</u> 标记；③强调用 <b>…</b>；④拼音注音统一"（háng xíng）"半角括号格式。🔴 自洽性硬性：题干要求"圈出加点字/画线句子"的题，正文必须恰好存在对应的 <u> 标记，严禁题干有要求而正文无标记（无标记=废题）；"加点"与"画线"由题干措辞区分，标记统一用 <u>，题目自身必须自洽。',
    description: '定义排版语义标记规范（加点/画线/加粗/拼音注音），并静默检测"题干要求加点/画线但正文无 <u> 标记"的废题（无法程序补全哪个字应加点，靠生成前约束 + 静默计数）。',
    enabled: true,
  },
