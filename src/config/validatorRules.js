...
export const VALIDATOR_RULES = [
  ...
  },
  {
    ...
    stages: ['*'],
    genTypes: ['exam'],
    promptHint: '分值账目自洽：小题数×每题分=大题分、空数×每空分=小题分、各大题分之和=满分，必须精确成立；多空题注明"（每空X分）"；声称的题数/空数/连线数必须等于正文实际输出的题号/空位/连线数。书写类题按词/字计分时：词数=拼音组数、字数=田字格格子数，声称总分必须=单位分×实际载体数，精确成立。',
    description: '大题/小题标题"每空/每线/每题/每词/每字 X 分"标注与实际载体数校验：空/线/题按 DOM 空位/连线/子题数、词按拼音组数（看拼音写词语）、字按田字格格子数，不一致时以实际载体数为准按「声称单位分×实际载体数」重算总分（不凑数；数不到载体时保留原标注）。',
    enabled: true,
  },
...

  {
    ...
    subjects: ['*'],
    stages: ['*'],
    promptHint: '排版语义标记规范：强调用 <b>…</b>；删除/划去用 <del>…</del>；分数统一“分子/分母”半角斜杠（如 1/2）；统计表/数位表/乘法表/关系表用 <table> 标准结构。',
  ...
  },
  {
    ...
    subjects: ['语文'],
    stages: ['*'],
    promptHint: '排版语义标记规范（语文加点）：题干要求“圈出加点字”的字用 <span class="emphasis-dot">字</span> 标记（字下加点，🔴严禁用 <u> 下划线表示加点）；🔴 自洽性硬性：题干要求“圈出加点字”的题，正文必须恰好存在对应 emphasis-dot 标记，严禁题干有要求而正文无标记（无标记=废题）。',
  ...
  },
  {
    ...
    subjects: ['语文', '英语'],
    stages: ['*'],
    promptHint: '排版语义标记规范（画线句子）：题干要求“画线句子/划出文中…句”处用 <u class="underline-sentence">…</u> 标记；🔴 自洽性硬性：题干要求“画线句子”的题，正文必须恰好有对应标记，严禁题干有要求而正文无标记（无标记=废题）。',
  ...
  },
  {
    ...
    subjects: ['*'],
    stages: ['*'],
    promptHint: '写话/作文等表达类题不得混入书写格（用作文格或留白）；写字/抄写类题的格子由书写惯例条款约束（见【输出格式】）。书写格必须为空格子（严禁在格内填写任何字/拼音/答案）；格子紧跟对应词语（拼音）之后输出，不得集中单独成段。',
