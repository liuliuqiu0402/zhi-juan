...
export const VALIDATOR_RULES = [
  ...
  },
  {
    ...
    stages: ['*'],
    genTypes: ['exam'],
    promptHint: '分值账目自洽：小题数×每题分=大题分、空数×每空分=小题分、各大题分之和=满分，必须精确成立；多空题注明"(每空X分)"；声称的题数/空位数必须等于正文实际输出的题号/空位（不虚报、不缩水）。',
...

  {
    ...
    stages: ['*'],
    genTypes: ['exam'],
    promptHint: '大题标题用明细式"共X题，每题X分/共X分"（按【卷面结构】），标题须含题量与分值；小题题号后/题干末尾标注"(X分)"。',
  ...
  },
  {
    ...
    subjects: ['*'],
    stages: ['*'],
    promptHint: '写话/作文等表达类题不得混入书写格（用作文格或留白）；写字/抄写类题按书写惯例输出书写载体。',
    description: '书写格按学段与题型双向规范（数据源=排版规格库 WRITING_CARRIER/CARRIER_RULES）：按 学科×学段 允许载体列表越界自动剥离（如中段以上田字格/四线三格、非语英学科混入格子）；表达/写话类题内混入书写格自动剥离保留文字；载体缺失仅对无歧义强要求（题干明确写"田字格中写/在田字格/方格中写"却没格子）静默提示抽检（2026-08 收敛：移除"看拼音写→推断须田字格"软推断，接受语境式空格形态）。生成前约束仅保留本规则独有增量（表达类禁格子）；载体格式规范由指令库 QUESTION_FORMAT 承载（必须真实输出/输出在题后）、场景细节由学段指引 SUBJECT_STAGE_EXTRAS 承载（三维度），本库不再重复（单通道）。',
    enabled: true,
  },
