...
export const VALIDATOR_RULES = [
  ...
  },
  {
    ...
    subjects: ['*'],
    stages: ['*'],
    promptHint: '连线题把左右两列内容逐行写出、两列间用至少2个全角空格分隔（如"雀　　隹字旁"），右列顺序打乱；🔴 严禁画线/连字符连接、严禁右列只写序号或"序号＋内容单独列下方"拆分（格式详见模板【输出格式】作答空规范）；输出前逐对自检配对正确性。',
  ...
  },
  {
    ...
    subjects: ['*'],
    stages: ['*'],
    promptHint: '书写格按学段：语文1-2年级田字格、3年级起方格/横线；英语小学四线三格、中学起单线（详见模板【输出格式】）。',
