...
export const VALIDATOR_RULES = [
  ...
  },
  {
    ...
    subjects: ['*'],
    stages: ['*'],
    promptHint: '书写/作图类题按书写惯例输出对应书写载体（留白或格子）；任何作答空位/格子内严禁预填字或答案（空位才可作答）。',
  ...
  },
  {
    ...
    subjects: ['语文', '英语'],
    stages: ['*'],
    promptHint: '写话/作文/书面表达等表达类题不得混入书写格（田字格/四线三格等）；作文作答区由系统按题型自动配置（作文题自动补格、写话给书写行），正文不要自行输出"作文格"字样；写话/作文题须完整呈现题目要求描述（不得仅标题行）。',
...

  {
    ...
    subjects: ['*'],
    stages: ['*'],
    promptHint: '解答类题留出与之匹配的作答空位；有分值的按分值给足空间，无分值的按题型书写惯例留出合理空间；客观题无需额外空间（其选项/括号式空位已可作答）；作答空位不得被压缩或与下一题粘连。',
