 ...
 *  🔴 2026-09-17 用户实证第三卷（综合检测）·补"点名图片形态"的词：原表靠"据图"这种**偶然子串**命中
 *    "根据图片提示"（"根**据图**片"），可一旦换写成"图片提示…""看图片…""或图片"就全漏——
 *    实证：听力大题标题「听录音，选出你所听到的单词**或图片**」，选项全是单词、本大题无任何图，
 *    校验侧因不认"图片"二字而静默放过。凡题面点名"图片/配图/插图"即"声称要图"，与"看图/读图"同理。
 */
export const FIGURE_DEPENDENCY_RE = /看图|读图|看图形|识图|据图|依图|如图|图表|统计图|图片|配图|插图|观察[^\n]{0,8}图形|格图/;
