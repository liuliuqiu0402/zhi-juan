...
const FORMAT_RULES = `· 作答空规范（全卷统一）：
  填空用横线 <u>＿＿＿</u>，宽度按答案字数：1字≈2格（＿＿）、2字≈4格（＿＿＿＿）、3-4字≈6格、5-6字≈8格，答案每多1字加长2格（横线宽度须按答案长度精确给出——系统后处理只按现有宽度归类，无法代算答案长度）；
  选择/判断作答空用英文状态括号（半角括号）内全角空格 (　)，括号内空格数=答案字数（1字 (　)、2字 (　　)、3字 (　　　)…据此递增）；🔴 括号与横线二选一、严禁混用：括号作答空只放全角空格、禁止括号内放下划线或横线（如（_____）、(_ _)）；横线作答空（填空）用 <u>＿＿＿</u> 且不加括号；作答空字符一律用半角下划线"_"或全角空格"　"，禁止全角下划线"＿"；
  连线题按正确配对逐行输出，左项与右项之间用"——"连接（如"雀——隹字旁"），每对一行，🔴 严禁在左右项之间画线/空格分隔、严禁自行乱序——系统会自动打乱右列顺序生成连线题，你只需保证配对正确（逐对自检：偏旁/课文/量词等配对必须准确无误）；
· 书写载体：写字题（题干要求"在田字格/方格中写"等）的作答格必须紧跟空位输出（如 <span class="tian-zi-ge">字</span>），禁止另起独立空格段或只用括号占位；
· 题干语义与作答载体必须一致："补充完整"→填空横线、"连线"→左右两列（配对关系见上）、"写出"→书写格/田字格，严禁载体与题干语义矛盾（如"补充完整"却用两列表格、"连线"却逐行并排）；
· 写话/作文题顺序：先完整题干（含全部要求与提示），再输出方格纸（<div class="zuo-wen-ge">），格子必须在题干之后，严禁格子出现在题干描述之前`;

/** 质量底线（全类型通用，随输出格式块注入；在新课标"素养立意、教-学-评一致"基础上理解，不字面化） */
const QUALITY_BASELINES = `【质量底线】（全类型通用）
· 内容充足：题量与文字量达到该类型资料标准（充实靠丰富题干信息、情境描述与思维层次，严禁靠堆题、机械重复或套话凑数）；题干信息充分（情境/条件/要求完整），答案附解析
...
【命题要求】
1. 依据2022版新课标命题：素养立意、情境真实适切、设问有层次，杜绝机械记忆与偏题怪题；🔴 全卷须在统一/连贯情境中展开（单元大情境/主题情境/任务情境，情境贯穿全卷，各题在此情境下连贯命题），禁止无情境的孤立堆题——情境须与学科内容深度融合、贴近学段生活，非简单背景装饰
1b. 🔴 命题风格：unified_context（全卷统一情境）——系统将自动生成贯穿全卷的统一情境框架并注入本指令，所有命题必须在该情境下展开；若不需要统一情境，删除本行即可
2. 紧扣{unit}教材与课标命题：知识点与能力要求不超出本学段课标学业质量要求；选文、情境等载体可来自课外，但须主题相关、难度适切
3. 题型多样：同一题型不要连续出现，全卷避免集中于单一题型（连线/选择/填空等题型应与其他题型交替出现）；每道题题干完整、条件充分、逻辑自洽、可直接作答
4. 阅读题先给出短文全文再设问（课内课外结合，课外选文标注出处）；看图写话题用 [IMAGE] 标记描述画面（格式见生成时注入的【渲染指令】）
5. 所有题目原创设计，禁止照搬教材原题；题量与文字量与考试时长匹配，内容充实饱满
6. 难度从易到难排列
7. 选择题正确答案分布随机均匀（同一选项不连续超3次），干扰项有真实迷惑性（来自学生常见错误，禁明显荒谬项）
8. 全卷题目互相独立、不重复不雷同（同情境/数据/设问角度不得重复），题干数据与答案运算自洽
9. 全卷至少2道分析/评价/创造层级开放设问（低段可用"说说你的想法"等口语化设问）
10. 考点覆盖本单元各课/各节核心知识点（通过情境综合与变式实现，不靠每课堆题）
11. 🔴 小题标题规范：小题标题必须准确描述其作答形式，采用规范题型名（如"读拼音，写词语""选字填空""按课文内容填空""用部首查字法查字典""照样子，补充词语""连一连，帮汉字找到它们的偏旁朋友"等），🔴 严禁自创与内容不符的标题（如查字典题起名"数一数，填一填"、选择题起名"猜一猜"）；标题与作答载体必须一致（补充完整→填空横线、连线→两列配对、写出→书写格/田字格）

【卷面格式】（正式卷面必备）
· 卷首标题只由系统注入的占位符组成（命名规范如下），🔴 严禁再追加"考卷/试卷/测试"等任何资料类型词：
...
【输出格式】（结构清晰，便于排版导出）
· 大标题用 <h1>；大题标题用 <h2>（如 <h2>一、识字与写字（共X题，共32分）</h2>）；题目以 <p class="question"> 包裹并带题号（1. 2. 3.…），子题用 (1)(2)
${FORMAT_RULES}
· 简答/解答题题下留足空白作答区（不少于3行）；如适用：语文田字格 <span class="tian-zi-ge">字</span>、英语四线三格 <span class="four-line-three">a</span>
· 🔴 只输出资料正文（题目、卷面与作答区），严禁在正文中输出任何答案/解析/评分标准/听力原文（参考答案由系统在正文生成后单独调用生成；若当前任务要求输出《参考答案》，则按任务要求输出答案页）
· 严禁代码块包裹输出，直接输出内容
${QUALITY_BASELINES}`;
...
2. 覆盖{unit}核心知识点，栏目与题量按生成时注入的【教辅结构】执行（基础→探究→迁移分层组织，严禁只罗列题目）
3. 题型多样、图文并茂，任务间不重复；同一知识点不重复考查，同一任务组内禁止连续多道相同或高度相似的题
4. 每道题/任务完整可作答，不空泛
5. 同一知识点允许换情境、换角度、换题型变式复现（巩固强化），禁止同情境同设问的机械重复
...
【要求】按题型或考点分类组织（每类一个板块），类内按基础→提升→拓展分层，聚焦{unit}薄弱点，板块数与题量按生成时注入的【教辅结构】执行；同一知识点不重复考查，同板块内禁止出现相同或高度相似的题；变式题须换情境换角度，禁止同情境同设问的机械重复；覆盖教材核心知识点；题目完整可作答。
...
【要求】以问题驱动预读（如"圈出你不认识的字""概括每段大意"），可操作可检查；任务覆盖本课时全部新知识点，同一知识点不重复设问；栏目按生成时注入的【教辅结构】执行（含"我的疑问"栏目）；紧扣教材原文。
...
【要求】原创短文（不复制课文/网络文章，课外选文主题须与{unit}相关），短文完整呈现（不截断），每篇配分层题（信息提取→理解→评价）；题目不可直接在原文找到原句答案；短文无语病；同一考点不重复设问；篇数/字数/题量按生成时注入的【教辅结构】执行。
...
【要求】结构化呈现（表格/对比/导图优先），覆盖{unit}全部知识点并标注教材出处，不遗漏；重点标注，不堆砌大段文字；栏目与篇幅按生成时注入的【教辅结构】执行。
...
【要求】在语境中默写（看拼音写词语有语境提示），减少孤立默写；严格对应教材要求，覆盖本单元全部要求掌握的字词/篇目，不遗漏不重复；拼音/字形准确；覆盖量与栏目按生成时注入的【教辅结构】执行；书写格按学段：语文 1-2 年级用田字格、3 年级及以上用方格或横线，英语小学用四线三格、初中及以上用单线/横线。
...
【要求】按知识点或题型分类组织（每类一个板块），每题结构按生成时注入的【教辅结构】执行（原题→归因→解法→变式→策略）；同类只保留一道变式，不重复；归因禁止笼统写"粗心"。
...
【要求】栏目按生成时注入的【教辅结构】执行（知识框架→考点梳理→典型题析→易错聚焦→综合自测），覆盖{unit}全部知识点；结构化呈现；自测题分层（基础/提高），按考点分布、避免同考点重复考查，同一板块不出现相同或相似题。
...
export const OUTPUT_FORMAT_HINT = OUTPUT_FORMAT_BLOCK;
