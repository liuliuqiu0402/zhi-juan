...
const QUESTION_FORMAT = (ctx = {}) => {
  ...
  const lines = [];
  // 🔴 图是交付物（2026-09 恢复正文级驱动）：看图/读图/图形/统计图类题，该图是本题必不可少的内容本身，
  //    不是可选项——必须紧贴该题实际输出图标记（画面类 [IMAGE]、数据/几何图形类 [GRAPH]），
  //    与题干人物/场景/数量/数据严格一致，严禁省略、严禁以"图形隐含"或纯文字描述代替。
  //    放在题干决断点（正文）首位，不进 system：system 契约一旦被降权，正文仍强制出图。
  lines.push('🔴 图是本题交付物（看图/读图/看图形/统计图类题）：该图是本题必不可少的内容，须紧贴该题实际输出——画面类用 [IMAGE]...[/IMAGE]（一句 PROMPT 画面描述，图内无字、人物/场景/数量与题干严格一致），数据/几何图形类用 [GRAPH]...[/GRAPH]（TYPE 取值与数据严格取自题干）。严禁省略、严禁以"图形隐含"/纯文字描述代替；格式细节见注入的渲染指令。');
  ...
  lines.push('· 卷面自洽（编辑自查总纲，成稿前逐题核对）：①任务词与实际作答形态一致——说"选择/选一选"必须给出选项、说"判断/对的打√"须留 √× 作答空、说"填空/填一填"须留空位、说"写在横线上/格子里"须给对应书写位、题干凡声明了作答容器（圈类："填入圈里/圈中/○里"、括号、横线、格子、方框等），输出载体须与题干措辞同名一致——分类填圈题题面说"圈"即给圈形空位（○）或括号空，严禁题面说"圈"正文给横线（"圈出/圈一圈/画圈"是在已给内容上作记号，不是作答容器，不得补空位）、说"直接写得数/写得数/算一算"的得数结果位只留空白书写区（＝后不画横线、不框）；任务词与作答形态不符即改；②每道题（含 (1)(2)(3) 等子题）必须可作答——有处填、有处选、有处写、有处连，不得出现只有任务描述却没有作答位置/作答对象（如"给加点字选择读音"却无加点字与选项）的题；③题干文字不加下划线/删除线等修饰（画线语义只作用于题干声明要求画线的那句话/词语对象）；④作答空行只跟在题干/任务要求之后，不跟在大类/大题标题（<h2>）或栏目标题之后——标题后直接出题，标题与第一题之间不插空作答行；⑤行内已含作答空位的题（各并列小题自带空位/待填处），不得再在该题作答位置之外另起整行空白作答段——整行空白作答区只用于确需整段书写作答的长答题；⑥同一道题内各小题的作答载体形态须保持一致，不得同一题混用不同形态的作答位置；⑦作答空位显式载体化：凡需学生作答/填写的空位（概念/算理/算式中的空位、列式空位、选择判断√×位等）一律输出显式载体（填空横线 <u class="blank-N">&emsp;</u>、括号空位、算式填空圈等，见注入渲染指令），严禁用裸空格或全角空格串充当空位——空格只作数字与符号的排版间隔、不构成书写空位；空位宽度与所需答案长度相当；直接写得数/计算结果的"＝ 后"得数结果位除外——按①只留空白书写区（不画横线、不框）；⑧同卷空位形态统一：性质相同的填空空位在整份卷内用同一种空位形态（同为填空一律下划线空 或 一律括号空），不得同一份卷内同类空位混用下划线空/括号空/裸空三种形态，判断题√×位、选择题（）位等同类作答位同样统一；');
  return lines.join('\n');
};
...
· 定稿前按下列底线自查，不达标即修正；
· 内容充实：篇幅与容量足额覆盖注入的结构与栏目，不遗漏、不缩水；充实靠丰富题干信息、情境与思维层次；
· 组织合理：同类条目分层递进、互不雷同，同一知识或能力点不扎堆反复出现；题类资料中，同一大题及相邻题不设相同或相似题；
· 卷内唯一性（2026-09 系统性根治，学科中立）：同一份卷内不得复现相同材料/数据组合/题组结构（并列对照子题除外）；同一生活主题（如"图书角""运动会接力"）最多承载 2 题；命中即换题换情境换数据；
· 设问重思维过程：设问指向思维过程与问题解决（能展开推理、说理与运用过程，展示理解而非仅凭记忆），避免死记硬背、机械刷题式作答；
· 内容正确：图文一致（配图描述与正文相符）；逻辑自洽（条件充分、前后表述一致、设问或任务与结果对应）；知识与能力要求不超出本学段课标学业质量要求——材料可取自课外但须主题相关、难度适切；内容不偏离本资料主题与范围；
· 情境按考查意图选配：考查基础概念、规则与技能时（尤为基础建构/基础题）情境从简或不用，避免无关信息干扰，不得为凑情境而把简单概念题硬套长篇故事；考查应用、综合与表达时用贴近学生生活的真实情境，情境信息完整、可判、无歧义
...
【教材原文（仅供理解：题型结构与算理/知识梯度、覆盖点核对；教材原文为已确认文本，理解与引用须出自原文字面表述，不得改写、不得以训练记忆代原文；命题的数据、情境、人名、句式一律自拟，禁止沿用原文连续字面）】
{material}
...
const OUTPUT_FORMAT_BLOCK = (mode = 'question', ctx = {}) => {
  ...
  const head = mode === 'exam'
    ...
    : '如 <h2>基础建构任务</h2>——蓝图结构=大类栏目标题，独立成行、不带"一、"序号（居中/顶格由版面样式控制），居于本栏全部内容之前';
  ...
  const itemRule = isContent
    ...
    ? ''
    : '· 大类层级：蓝图结构栏目（如"基础建构任务"）作为本卷大类标题，用 <h2> 独立成行呈现，不加"一、"序号，居于该栏全部内容之前；大类不作大题使用、不承载题目本身\n· 栏内组织：每栏内题目按内容/任务分组成大题，组前以 <h3> 大题标题起——标题由你自拟本组内容概括，用"一、二、三…"汉字序号（全卷连续、跨大类顺延不重复，如 <h3>一、理解小数乘法的意义</h3>、<h3>二、掌握计算方法</h3>），概括题组实际练什么，不写"覆盖/考查/练习"等字样、不罗列知识点清单；同组小题共用语境递进或并列；若整栏为单一连贯任务链（如一个探究任务含多个步骤）可不用分组、该栏大题标题即栏目任务本身\n· 题目以 <p class="question"> 包裹并带题号（1. 2. 3.…），子题用 (1)(2)，题号全卷连续（跨大题、跨大类不断号）\n· 段落组织：每道题、每个任务（情境/活动/成果各条）独立成 <p> 段落，段间空行分隔；严禁把多个题目或条目堆叠在同一段落内';
...
· 细致与深度要求：任务沿"基础建构→探究进阶→迁移创新"递进，任务间有层次与关联；每个任务情境完整、步骤清晰、有过程性设问；变式换情境、换角度、换设问，不只换数字；设问指向真实思维，避免机械重复。

【教材原文（仅供理解：题型结构与知识梯度、覆盖点核对；题目的数据、情境、句式一律自拟，禁止沿用原文连续字面）】
{material}
...
· 细致与深度要求：按核心知识精准聚焦；板块内由易到难；每板块含易错点辨识与思路点拨，解析讲明"错因—如何避免—正确思路"，不只给答案。
...
· 细致与深度要求：任务沿"基础通读→重难点深入→延伸挑战"递进，覆盖本课全部新知（以【本资料须覆盖的核心知识】为覆盖清单，不得整点遗漏）；每个任务写明"做什么+怎么做+完成标准（能自查）"，不写空泛的"了解/掌握"式口号；用真实情境或生活经验引出待学问题；引导用已有知识联结新知、带着问题进课堂；「我的疑问」给出思考支架（圈画关键处、联系旧知、先自查再记疑）。
...
· 细致与深度要求：选文完整、无语病、主题与单元相关；设问由浅入深，指向信息提取→推断→整体感受/评价的思维层次；设问引导理解，不宜在原文找现成字面答案。
...
【创作要求】覆盖{unit}全部核心知识并标注出处（按注入的【教材原文】章节），不遗漏；重点标注、文字精炼；栏目与篇幅按生成时注入的【教辅结构】执行。
知识总结中「典型例题」栏目为讲解示范定位：典型例题给出完整题目后**立即同步给出答案与解析**，不预留学生作答空间，用于知识点应用示范讲解。
· 细致与深度要求：按"概念—法则/规则/方法—易错—辨析"组织，每个知识点同时给出内涵、适用前提、使用注意、常见误区，不只罗列要点；含规律与方法归纳；易错点睛到具体易混处。
...
· 细致与深度要求：内容置于语境/情境呈现，不孤立罗列；覆盖本课时/单元全部要求掌握内容（以【本资料须覆盖的核心知识】为覆盖清单，不得遗漏）；含易错点辨析；按学段体现层次（低段重书写规范、中高段在情境中考查）。
...
【创作要求】栏目以注入的【教辅结构】为准；核心知识覆盖与出处标注以注入的【教材原文】章节为准。
· 细致与深度要求：知识网络化呈现；核心知识按"识记→理解→运用"分层梳理，逐条给要点与教材出处；典型题给解题思路剖析；易错辨析具体到易混处；自测分层（基础/提高），覆盖本单元全部核心知识。
...
export const SUBJECT_STAGE_EXTRAS = {
  // ── 语文 ──
  '语文|primary_low': { text: '识字写字认读与书写并重、在语境中识记运用；口语交际敢说会听、乐于表达；写话有兴趣，写自己想说的话与想象中的事物。书写/拼音/轻声按教材规范口径（轻声不标调号，同卷一致）；笔画/部首/汉字结构名称一律用教材规范名称。', source: '2022义教语文·第一学段识字与写字/表达与交流（口语交际与写话）+ 语言文字积累与梳理任务群' },
  '语文|primary_mid': { text: '词句在真实语境中理解运用；阅读能初步把握主要内容、体会思想感情；习作乐于书面表达，观察周围世界，不拘形式写下见闻与想象。轻声/儿化标注与标点书写按教材规范口径且同卷一致；笔画/部首名称用规范名称。', source: '2022义教语文·第二学段阅读与鉴赏/表达与交流（初步把握主要内容、乐于书面表达）' },
  ...
  // ── 数学 ──
  '数学|primary_low': { text: '口算与简单应用结合生活情境；图形与几何（图形认识与测量）以直观操作呈现；问题解决列式、计算、作答完整；概念、法则与规律须显性考查其理解（能辨识含义、适用条件与易混区别），不以机械套算或仅作任务背景代替概念考查。', source: '2022义教数学·第一学段学段目标（数的抽象、整数四则运算、长度测量）+ 2022义教数学命题建议（注重考查数学本质的理解，避免机械训练）' },
  '数学|primary_mid': { text: '计算与竖式规范；单位换算在情境中考查；实际问题真实情境、列式计算作答完整；概念、法则与规律须显性考查其理解（能辨识含义、适用条件与易混区别），不以机械套算或仅作任务背景代替概念考查。', source: '2022义教数学·第二学段学段目标（较复杂整数运算、数量关系）+ 2022义教数学命题建议（注重考查数学本质的理解，避免机械训练）' },
  '数学|primary_high': { text: '计算与方程在情境中运用；几何与统计结合操作；实际问题体现建模过程（理解→列式→计算→作答）；概念、法则与规律须显性考查其理解（能辨识含义、适用条件与易混区别），不以机械套算或仅作任务背景代替概念考查。', source: '2022义教数学·第三学段学业质量描述（数感/符号意识/模型意识）+ 2022义教数学命题建议（注重考查数学本质的理解，避免机械训练）' },
  '数学|middle': { text: '数与式、方程不等式、函数、几何、统计概率综合；解题过程完整（推理链清晰）；实际问题真实情境建模；概念、法则与规律须显性考查其理解（能辨识含义、适用条件与易混区别），不以机械套算或仅作任务背景代替概念考查。', source: '2022义教数学·第四学段学段目标（数与式/方程与不等式/函数）+ 推理能力核心素养 + 2022义教数学命题建议（注重考查数学本质的理解，避免机械训练）' },
  '数学|high': { text: '函数与导数、几何、概率统计综合；解题过程完整、逻辑严谨、步骤规范；开放设问考查数学表达与推理；概念、法则与规律须显性考查其理解（能辨识含义、适用条件与易混区别），不以机械套算或仅作任务背景代替概念考查。', source: '高中数学课标(2017/2020)·必修内容 + 核心素养（数学抽象/逻辑推理/数学建模/直观想象/数学运算/数据分析）+ 高考评价体系（注重考查关键能力与思维品质，重理解防机械）' },
  // ── 英语 ──
  '英语|primary_low': { text: '词汇在图片/情境中识别与认读；以听说认读为主，书写正确规范。英文书写口径：句首与专有名词大写；用英文标点（句号用半角"."、撇号用半角"\u0027"），缩写与所有格撇号不丢（Let\u0027s/It\u0027s/children\u0027s）；省略号用三点"…"（不写中文六点"……"）；音标须与实际发音一致且卷内自洽，同一单词全卷音标不得冲突，严禁凭字母形臆断字母读音（如 Chinese 末字母 s 发 /z/）。', source: '2022义教英语·语言能力核心素养（感知与积累）' },
  '英语|primary_mid': { text: '词汇句型在语篇/情境中运用；书写规范。英文书写口径：句首与专有名词大写；用英文标点（句号用半角"."、撇号用半角"\u0027"），缩写与所有格撇号不丢（Let\u0027s/It\u0027s/children\u0027s）；省略号用三点"…"（不写中文六点"……"）；音标须与实际发音一致且卷内自洽，同一单词全卷音标不得冲突，严禁凭字母形臆断字母读音（如 Chinese 末字母 s 发 /z/）；美/英拼写与发音同卷不混用。', source: '2022义教英语·语言能力核心素养（习得与建构）' },
  '英语|primary_high': { text: '语篇阅读与书面表达结合主题语境；围绕主题表达清楚、有条理。英文书写口径：句首与专有名词大写；用英文标点（句号用半角"."、撇号用半角"\u0027"），缩写与所有格撇号不丢（Let\u0027s/It\u0027s/children\u0027s）；省略号用三点"…"（不写中文六点"……"）；音标须与实际发音一致且卷内自洽，同一单词全卷音标不得冲突，严禁凭字母形臆断字母读音（如 Chinese 末字母 s 发 /z/）；美/英拼写同卷不混用。', source: '2022义教英语·核心素养（语言能力/思维品质）' },
  '英语|middle': { text: '语篇完整地道（真实语境、无中式英语）；书面表达主题明确、意思连贯；英文语篇内省略号用三点"…"（不写中文六点"……"）；音标须与实际发音一致且卷内自洽，同一单词全卷音标不得冲突，严禁凭字母形臆断字母读音（如 Chinese 末字母 s 发 /z/）。', source: '2022义教英语·核心素养（语言能力/文化意识/思维品质/学习能力） + 命题质量要求（语篇真实地道、表达规范，为产品级表述，非课标原文）' },
  '英语|high': { text: '听力、阅读语篇真实地道；语言知识在语篇语境中理解运用；书面表达涵盖应用文、读后续写等类型；英文语篇内省略号用三点"…"（不写中文六点"……"）；音标须与实际发音一致且卷内自洽，同一单词全卷音标不得冲突，严禁凭字母形臆断字母读音（如 Chinese 末字母 s 发 /z/）。', source: '高中英语课标(2017/2020)·核心素养（语言能力/文化意识/思维品质/学习能力） + 学业质量与考试命题建议（书面表达题型含应用文、读后续写等）' },
  // ── 物理 ──
  ...
  // ── 地理 ──
  '地理|middle': { text: '运用地图与图表获取与解读地理信息；认识区域按区域定位→要素分析→人地关系综合展开；资料数据真实。', source: '2022义教地理·核心素养（人地协调观/综合思维/区域认知/地理实践力）' },
...
export function buildInjectionInstruction(opts = {}) {
  ...
  const map = {
    ...
    // 🔧 素材头占位渲染（素材线 G7 终态，2026-09 用户定稿）：教材原文不再注入委托——
    //    素材唯一途径=研读轮分批消化（→研读总账随会话前缀携带）+ 写作期 browse 按需；
    //    委托书内该行仅作模板结构提示，覆盖点名自查以"研读总账点名"为对照清单（不重复声明）
    '{material}': '（教材原文不随本委托注入：素材经研读消化，覆盖理解见会话前缀研读总账；写作需教材原文精确形态时按范围目录 browse 现取——数据/情境/句式一律自拟，禁止照搬字面）',
  };
...
· 答案按正文的大题与题号层级组织，与正文同构。不复述题干原文，不重现正文作答空位。
· 同一大题内由短数据/符号/词构成的并列小题答案（如数值、√×、词语），统排在一段内用全角空格分隔即可，不逐项独占一行（节省版面、便于对照）；凡含完整句子、推理步骤或解析的长答案，保持独立成行，不同大题之间保持结构分界。
