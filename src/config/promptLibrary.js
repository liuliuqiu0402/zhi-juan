...
const QUESTION_FORMAT = `· 作答载体（系统按作答形式渲染，输出时与题干语义保持一致）：填空空位宽度与答案字数匹配（1字≈2格，长答案适当加长）；选择/判断用括号空位；写字题按题干要求的书写载体呈现；任何作答载体（田字格/方格纸/作文格等）一律输出在所属题目之后，不得置于题干之前；写话/作文题须完整呈现题目要求（含写作要求），不得只有标题行；解答/简答题留足作答区（如适用：语文田字格 <span class="tian-zi-ge">字</span>、英语四线三格 <span class="four-line-three">a</span>）`;
