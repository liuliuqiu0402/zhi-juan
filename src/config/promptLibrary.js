...
const QUESTION_FORMAT = `· 作答载体（系统按题型渲染，输出时与题干语义保持一致）：填空空位宽度与答案字数匹配（1字≈2格，长答案适当加长）；选择/判断用括号空位；写字题按题干要求的书写载体呈现；写话/作文的方格纸由系统在题干之后生成；解答/简答题留足作答区（如适用：语文田字格 <span class="tian-zi-ge">字</span>、英语四线三格 <span class="four-line-three">a</span>）`;
...
· 内容充实：内容量与篇幅达到该类型底线（充实靠丰富题干信息、情境描述与思维层次，严禁靠堆题、机械重复或套话凑数）；栏目完整，板块间不重复不雷同
...
【创作要求】在语境中默写（看拼音写词语有语境提示），减少孤立默写；严格对应教材要求，覆盖本单元全部要求掌握的字词/篇目；拼音/字形准确；覆盖量与栏目按生成时注入的【教辅结构】执行。
