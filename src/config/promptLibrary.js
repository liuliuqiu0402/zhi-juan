...
· 情境按考查意图选配：考查基础概念、规则与技能时情境从简或不用，避免无关信息干扰；考查应用、综合与表达时用贴近学生生活的真实情境，情境信息完整、可判、无歧义
· 正文直入（2026-09 防过程自述污染）：正文直接呈现内容本身，不得以"已获取教材原文/现在编写/接下来/以下将"等过程性自述开头，不得在开头复述覆盖清单或写作计划；在正文罗列/声明"覆盖了哪些核心知识"不算覆盖——覆盖只以实际呈现的题目/条目计
...
· 可数对象取整：人、本、张、辆、件、盒、只等可数对象只能用不小于 1 的整数表示个数；小数、百分数只能表示比率、单价、折扣、概率等连续量，不得表示可数对象的个数，也严禁"0.09 人（即 9 人）"这类一个数同时充当两个量的写法；
