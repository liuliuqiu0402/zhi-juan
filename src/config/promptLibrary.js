...
const FORMAT_RULES = `· 作答载体（系统按题型渲染，输出时与题干语义保持一致）：
  ...
  连线题只输出正确配对（左项与右项的对应关系）；写字题按题干要求的书写载体呈现；
  写话/作文的方格纸由系统在题干之后生成。`;
...
【命题要求】
1. 依据2022版新课标命题：素养立意、情境真实适切、设问有层次，杜绝机械记忆与偏题怪题；全卷情境连贯（按所选组织风格展开），情境与学科内容深度融合、贴近学段生活
2. 以{unit}教材为核心内容主线：知识点与能力要求不超出本学段课标学业质量要求；选文、情境等载体可来自课外，但须主题相关、难度适切
...
6. 设问有层次：包含分析/评价/创造等认知层级的开放设问（低段可用"说说你的想法"等口语化设问），考查素养而非机械记忆
7. 考点覆盖本单元各课/各节核心知识点（在情境中综合与变式呈现）；小题标题准确描述其作答形式
...
【要求】按题型或考点分类组织（每类一个板块），类内按基础→提升→拓展分层，聚焦{unit}薄弱点，板块数与题量按生成时注入的【教辅结构】执行；同板块内题目不雷同（换情境/角度/设问呈现变式）；覆盖教材核心知识点；题目完整可作答。
...
【要求】以问题驱动预读（如"圈出你不认识的字""概括每段大意"），可操作可检查；任务覆盖本课时全部新知识点；栏目按生成时注入的【教辅结构】执行（含"我的疑问"栏目）；紧扣教材原文。
...
【要求】原创短文（不复制课文/网络文章，课外选文主题须与{unit}相关），短文完整呈现（不截断），每篇配分层题（信息提取→理解→评价）；题目不可直接在原文找到原句答案；短文无语病；篇数/字数/题量按生成时注入的【教辅结构】执行。
...
【要求】在语境中默写（看拼音写词语有语境提示），减少孤立默写；严格对应教材要求，覆盖本单元全部要求掌握的字词/篇目；拼音/字形准确；覆盖量与栏目按生成时注入的【教辅结构】执行；书写格按学段：语文 1-2 年级用田字格、3 年级及以上用方格或横线，英语小学用四线三格、初中及以上用单线/横线。
...
【要求】按知识点或题型分类组织（每类一个板块），每题结构按生成时注入的【教辅结构】执行（原题→归因→解法→变式→策略）；归因禁止笼统写"粗心"。
...
【要求】栏目按生成时注入的【教辅结构】执行（知识框架→考点梳理→典型题析→易错聚焦→综合自测），覆盖{unit}全部知识点；结构化呈现；自测题分层（基础/提高），按考点分布。
