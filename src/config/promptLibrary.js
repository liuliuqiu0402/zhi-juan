...
const QUESTION_FORMAT = (ctx = {}) => {
  ...
  lines.push('· 卷面自洽（编辑自查总纲，成稿前逐题核对）：①任务词与实际作答形态一致——说"选择/选一选"必须给出选项、说"判断/对的打√"须留 √× 作答空、说"填空/填一填"须留空位、说"写在横线上/格子里"须给对应书写位，任务词与作答形态不符即改；②每道题（含 (1)(2)(3) 等子题）必须可作答——有处填、有处选、有处写、有处连，不得出现只有任务描述却没有作答位置/作答对象（如"给加点字选择读音"却无加点字与选项）的题；③题干文字不加下划线/删除线等修饰（画线语义只作用于题干声明要求画线的那句话/词语对象）；④作答空行只跟在题干/任务要求之后，不跟在大类/大题标题（<h2>）或栏目标题之后——标题后直接出题，标题与第一题之间不插空作答行；⑤行内已含作答空位的题（各并列小题自带空位/待填处），不得再在该题作答位置之外另起整行空白作答段——整行空白作答区只用于确需整段书写作答的长答题；⑥同一道题内各小题的作答载体形态须保持一致，不得同一题混用不同形态的作答位置；⑦作答空位显式载体化：凡需学生作答/填写的空位（概念/算理/算式中的空位、列式/结果位、选择判断√×位等）一律输出显式载体（填空横线 <u class="blank-N">&emsp;</u>、括号空位、算式填空圈等，见注入渲染指令），严禁用裸空格或全角空格串充当空位——空格只作数字与符号的排版间隔、不构成书写空位；空位宽度与所需答案长度相当；⑧同卷空位形态统一：性质相同的填空空位在整份卷内用同一种空位形态（同为填空一律下划线空 或 一律括号空），不得同一份卷内同类空位混用下划线空/括号空/裸空三种形态，判断题√×位、选择题（）位等同类作答位同样统一；');
  return lines.join('\n');
};
...
· 卷内算式唯一性（理科，与质量底线"卷内唯一性"同源但按学科落位）：同一份卷内同一算式（同数值同运算，× ÷）不得在 ≥2 题复现（对照/比较类并列子题除外）；不得出现两题共用同一组关键数据（如两题都用 35.6/36.8/35.9）；出现即更换其中一题的数据与情境（题干与参考答案同步改）。确需用同一算式作"说一说/算理说明"时，只能紧邻承上一题刚算过的该算式直接解说（紧随其后、写明承接），不得远距或另起新题复用；`;
