...
const QUESTION_FORMAT = `· 作答载体（系统按作答形式渲染，输出时与题干语义保持一致）：填空空位宽度与答案字数匹配（1字≈2格，长答案适当加长）；选择/判断用括号空位；写字题按题干要求的书写载体呈现；看拼音写词语/写字题中田字格数量按拼音音节数逐音节一格（如"yáng shù"2 个音节给出 2 个田字格）；写话/作文题须含完整题目要求（情境与写作提示），其方格纸由系统在题干之后生成；解答/简答题留足作答区（如适用：语文田字格 <span class="tian-zi-ge">字</span>、英语四线三格 <span class="four-line-three">a</span>）`;
...
export const SUBJECT_STAGE_EXTRAS = {
  ...
  '语文|primary_high': { text: '阅读含概括与简单评价；综合性学习结合教材主题组织；习作二选一，给支架与字数提示。', source: '2022义教语文·第三学段阅读与鉴赏（抓住要点）/表达与交流（记实与想象作文）' },
  '语文|middle': { text: '古诗文课内外对比阅读，重理解与积累运用；名著阅读设问有层次；作文立意明确、结构完整。', source: '2022义教语文·第四学段阅读与鉴赏（文言文借助注释工具书、名著阅读）/写作（围绕中心表达）' },
  '语文|high': { text: '现代文、古代诗文阅读设问分层；语言文字运用重语境；作文准确理解材料、论证充分。', source: '高中语文课标(2017/2020)·语言运用核心素养 + 思辨性阅读与表达任务群' },
  // ── 数学 ──
  ...
  '数学|high': { text: '函数与导数、几何、概率统计综合；解答题逻辑严谨、步骤完整；开放设问考查数学表达与推理。', source: '高中数学课标(2017/2020)·必修内容 + 数学抽象/逻辑推理/数学建模核心素养' },
  // ── 英语 ──
  ...
  // ── 物理 ──
  '物理|middle': { text: '实验探究体现完整过程；计算题真实情境建模（画示意→列式→计算→作答）；选择填空以生活/实验现象为载体。', source: '2022义教物理·课程理念（从生活走向物理）+ 核心素养（物理观念/科学思维/科学探究）' },
  '物理|high': { text: '受力、电路、光路图规范；实验题重原理与数据处理；计算题过程完整、步骤严谨。', source: '高中物理课标(2017/2020)·核心素养（物理观念/科学思维/科学探究）' },
  // ── 化学 ──
  '化学|middle': { text: '化学用语规范（配平/条件）；实验探究含方案、现象、数据、结论；计算题步骤完整。', source: '2022义教化学·核心素养（化学观念/科学思维/科学探究与实践/科学态度与责任）' },
  '化学|high': { text: '工艺流程与反应原理结合情境；实验探究重方案与评价；计算定量、步骤规范。', source: '高中化学课标(2017/2020)·核心素养（宏观辨识与微观探析/变化观念与平衡思想/证据推理与模型认知）' },
  // ── 生物 ──
  ...
  // ── 道德与法治 ──
  '道德与法治|primary_low': { text: '行为习惯与生活常识以情境辨析呈现；设问贴近生活、表达正向。', source: '2022义教道法·课程理念（贴近生活、幼小衔接）' },
  ...
  // ── 科学 ──
  '科学|primary_low': { text: '观察与现象解释结合生活；记录题给记录表；操作型任务明确可做。', source: '2022义教科学·课程理念（保护好奇心与求知欲、联系生活）' },
  ...
  // ── 信息科技 ──
  '信息科技|primary_low': { text: '设备认知与用眼卫生以情境设问；操作步骤描述清晰。', source: '2022义教信息科技·课程理念（数字素养与技能启蒙）' },
  ...
  // ── 音乐 ──
  '音乐|primary_low': { text: '音的高低长短、节奏快慢以情境设问；表现题可用圈选/连线/涂色呈现。', source: '2022义教艺术(音乐)·课程理念（兴趣为先、表现与创造）' },
  ...
  // ── 体育 ──
  '体育|primary_low': { text: '运动安全与生活习惯以情境设问；动作要领分步清晰。', source: '2022义教体育与健康·课程理念（健康第一、激发兴趣）' },
...
export const STAGE_EXAM_EXTRAS = {
  primary_low: { text: '情境游戏化（开火车/闯关/情景剧场）；字号大、图文并茂，需配图处用 [IMAGE] 标记；难度低起点，多数学生能完成；认知底线：难度与情境符合低段认知水平，不出现未学概念与抽象符号。', source: '2022义教课标课程理念（幼小衔接、激发兴趣）+ 学业质量要求（不超学段）' },
  primary_mid: { text: '情境生活化（购物/校园/旅行）；设问常规、分层递进；认知底线：难度与情境符合中段认知，不超前引入高段知识。', source: '2022义教课标课程理念（真实情境）+ 学业质量要求（不超学段）' },
  primary_high: { text: '综合性强，阅读量加大；情境联系生活实际；增加开放性与思辨性设问，鼓励表达观点；认知底线：可适当开放思辨，但不超高段课标能力要求。', source: '2022义教课标学业质量要求（思辨性表达、不超学段）' },
...
export const STAGE_TEACHING_EXTRAS = {
  primary_low: { text: '情境游戏化、图文并茂；难度低起点，多数学生能完成；认知底线：难度与情境符合低段认知水平，不出现未学概念与抽象符号。', source: '2022义教课标课程理念（幼小衔接、激发兴趣）+ 学业质量要求（不超学段）' },
  primary_mid: { text: '情境生活化（购物/校园/旅行）；设问分层；认知底线：难度与情境符合中段认知，不超前引入高段知识。', source: '2022义教课标课程理念（真实情境）+ 学业质量要求（不超学段）' },
  primary_high: { text: '综合性强，阅读量加大；情境联系生活实际；增加开放性与思辨性设问；认知底线：可适当开放思辨，但不超高段课标能力要求。', source: '2022义教课标学业质量要求（思辨性表达、不超学段）' },
