...
· 覆盖检查前置且只内部完成：正文任何位置（含开头/结尾）禁止罗列核心知识清单、禁止写"覆盖了…/接下来将考查…"式声明或写作计划——声明不是内容，覆盖只以实际题目计（2026-09 实证：模型在正文开头声明覆盖循环小数却未出循环小数题 → 声明被程序剥离后缺漏照报）；正文只呈现题目与栏目内容
· 覆盖优先于篇幅：若按拟定题量无法逐点呈现全部核心知识，优先一题多点综合、精简情境；仍不足可适当增题以覆盖完整（考点覆盖属必要内容，不受篇幅精简限制），严禁删考点或改由声明代替
