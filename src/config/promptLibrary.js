...
export const SUBJECT_STAGE_EXTRAS = {
  ...
  '语文|primary_mid': '词句运用在真实语境中选填、仿写；阅读设问由浅入深、有层次；习作给选材与内容支架。',
  ...
  '英语|primary_high': '语篇阅读与书面表达结合主题语境；听力原文完整呈现；设问由浅入深、有层次。',
  '英语|middle': '语篇完整地道（真实语境、无中式英语）；听力原文完整呈现；书面表达给要点支架；设问由浅入深、有层次。',
  ...
  '地理|high': '区域认知与地理原理结合；综合题多要素分析；图表信息解读有层次。',
  // ── 科学 ──
...
function buildBuiltinTemplate({ stage = '', subject = '', genType = '' } = {}) {
  ...
  if (subject && stageOpensSubject) {
    const point = SUBJECT_STAGE_EXTRAS[`${subject}|${stage}`];
