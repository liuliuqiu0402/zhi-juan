...
· 卷内算式唯一性（理科，与质量底线"卷内唯一性"同源但按学科落位）：同一份卷内同一算式（同数值同运算，× ÷）不得在 ≥2 题复现（对照/比较类并列子题除外）；不得出现两题共用同一组关键数据（如两题都用 35.6/36.8/35.9）；出现即更换其中一题的数据与情境（题干与参考答案同步改）。`;
