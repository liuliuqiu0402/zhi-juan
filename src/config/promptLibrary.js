...
const QUESTION_FORMAT = (ctx = {}) => {
  ...
  //    全部 7 类题类资料（含课时练/阅读/默写/错题本/复习），非仅"卷"；与【尾约束·资料内多样】的
...
const outputFormatSections = (mode = 'question', ctx = {}) => {
  ...
  // 🔴 栏内分组依据按题类分流（2026-09-14 用户定版）：题型自拟的题类（课时练/专项/阅读）改按
...
【创作要求】短文无语病；篇数/字数/题量按生成时注入的【教辅结构】执行；在本次勾选范围的基础上安排相应充实的训练量，不因省事而缩减设问；同一知识点在同一资料内不得以相同深度重复考查，分层次呈现时各层侧重不同、错开设问角度与情境。
...
【{materialHead}（仅供理解：题型结构与知识梯度；错题题干与变式的素材口径见委托书【素材使用约定】）】
{material}
