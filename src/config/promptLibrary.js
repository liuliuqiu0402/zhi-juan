...
const MATERIAL_HINT = {
  ...
  anchor: '（本次**不注入**教材原文：涉及本套教材的内容口径、难度与版本，以开头【锚点清单】为核对依据；凡涉本套教材特有术语/编排/学段位置，**不得凭记忆断言**；覆盖下限见开头清单，清单外的补充/整合与引用约束见【素材使用约定】，以该处为准）',
};
...
· 内容唯一性：同一份资料内不得复现相同材料/数据组合（并列对照子题除外），题组结构与同质条目亦不重复；同一生活主题（如"图书角""运动会接力"）最多承载 2 题/2 处；命中即换题、换情境、换数据；
...
· 正文直入：正文直接呈现内容本身，不得以"已获取教材原文/现在编写/接下来/以下将/现依据教材原文与课标术语完成命题"等过程性自述开头，不得在开头复述覆盖清单或写作计划；在正文罗列/声明"覆盖了哪些核心知识"不算覆盖——覆盖只以实际呈现的题目/条目计；
· 覆盖完整优先：为覆盖清单所列核心知识所需的题目与条目属必要内容（不算注水），篇幅/题量不足时可适度扩充以保证覆盖完整——不得以篇幅为由删减核心知识呈现，不得以声明代替呈现；
...
【{materialHead}（仅供理解：题型结构与算理/知识梯度、覆盖点核对）】
{material}
...
const outputFormatSections = (mode = 'question', ctx = {}) => {
  ...
  const itemRule = mode === 'question'
    ? '· 栏内组织：每栏内题目按内容/任务分组成大题，组前以 <h3> 大题标题起——标题由你自拟本组内容概括，用"一、二、三…"汉字序号（全卷连续、跨大类顺延不重复，如 <h3>一、〈本组内容概括〉</h3>），概括题组实际练什么，不写"覆盖/考查/练习"等字样、不罗列知识点清单，也不得直接搬用【锚点清单】的条目名或教材板块名充当栏目标题/大题标题；同组小题共用语境递进或并列；若整栏为单一连贯任务链（如一个探究任务含多个步骤）可不用分组、该栏大题标题即栏目任务本身\n· 题目以 <p class="question"> 包裹并带题号（1. 2. 3.…），子题用 (1)(2)，题号全卷连续（跨大题、跨大类逐题递增，不得跳号、不得省略或合并任何一题；全部题目输出完成后逐题自查题号是否连续，缺号即补）\n· 段落组织：每道题、每个任务（情境/活动/成果各条）独立成 <p> 段落，段间空行分隔；严禁把多个题目或条目堆叠在同一段落内'
    : mode === 'exam'
...
3. 每道题/任务完整可作答
· 核心知识覆盖：题目须覆盖本课【锚点清单】中的每一项——每一项都要有实际呈现它的题目或任务（可多点综合于一题，也可同一项多题变式复现），不得有整项遗漏；命完对照清单自查，缺漏即补题，不依赖事后对账
· 细致与深度要求：任务间有梯度与关联，层次与顺序依内容自然形成；每个任务情境完整、步骤清晰、有过程性设问；变式换情境、换角度、换设问，不只换数字；设问指向真实思维，避免机械重复。

【{materialHead}（仅供理解：题型结构与知识梯度、覆盖点核对）】
{material}
