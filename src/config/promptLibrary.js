...
· 每题以 <p> 开头并带题号（1. 2. 3.…），子题用 (1)(2)；选择题选项 A. B. C. D. 各占一行
...
· 田字格书写（如适用）：<span class="tian-zi-ge">字</span>
· 正文结束后另起一部分：<h2>参考答案与评分标准</h2>，答案逐题对应、注明分值
· 严禁 \`\`\`html 代码块包裹，直接输出内容；答案不得混入正文

请完整输出试卷正文，再另起一部分输出《参考答案与评分标准》。${extra}`;
...
正文后另起一部分输出《参考答案与解析》。${OUTPUT_FORMAT_BLOCK}${extra}`;
...
【要求】在语境中默写（看拼音写词语有语境提示），减少孤立默写；严格对应教材要求，覆盖本单元全部要求掌握的字词/篇目，不遗漏不重复；拼音/字形准确；语文用田字格，英语按年级用四线三格或单线。
