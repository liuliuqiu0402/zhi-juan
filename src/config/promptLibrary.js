 ...
 * 🔧 2026-09：答案区不复述题干——低段课时练实测答案页把子题题干照抄一遍、答案用下划线标注，
 *    与正文归一载体（空位/书写线）两套形态，篇幅近乎翻倍且不便核对；
 *    现统一口径：按 大题号.子题号 → 答案 组织，答案本身写全，不重现题干与作答空位。
 *    低段（primary_low）答案速查一行一条，解析仅在易错/关键处给一句。
 */
export const buildAnswerFormatSpec = (subject = '', stage = '') => {
  const lowStage = stage === 'primary_low';
  return `【答案页输出格式】（HTML 规范，与正文一致，便于排版导出）
· 大题用 <h2>（如 <h2>一、〈大题名〉</h2>）；答案按题号与正文逐题对应（正文无题号的按栏目组织）再写答案与解析
· 不复述题干原文：按“大题号.子题号 → 答案”逐题给出（如 1.(2) → 答案），答案本身（算式/口诀/字词/完整句子）直接写全，不重现题干里的作答空位
${lowStage ? '· 低段答案速查：一条答案一行（如 3.(2) → 口诀：三五十五），解析仅在易错/关键处给一句，不写长篇\n' : ''}· 评分标准/等级表用 <table> 表格${HAS_LISTENING(subject) ? '；听力题附完整听力原文' : ''}
· 严禁使用 ##、**、|表格 等 Markdown 语法；严禁 \`\`\` 代码块包裹；直接输出 HTML 内容`;
};
