...
· 可数对象取整：人、本、张、辆、件、盒、只等可数对象只能用不小于 1 的整数表示个数；小数、百分数只能表示比率、单价、折扣、概率等连续量，不得表示可数对象的个数，也严禁"0.09 人（即 9 人）"这类一个数同时充当两个量的写法；列式/估算中的可数中间量不标注"支/本/人"等可数单位（如写"300÷3.5≈85.7"不写"≈85.7 支"），最终可数答案一律取整并给取整理由（去尾/进一/四舍五入+为什么）；
