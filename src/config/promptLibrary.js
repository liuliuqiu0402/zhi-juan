...
6. 设问有梯度，考查理解与运用
7. 练到本单元课标学业要求（在情境中综合与变式呈现）；小题标题准确描述其作答形式
...
2. 栏目与题量按生成时注入的【教辅结构】组织：题量与各大题内的小题数量按本学科本学段同类教辅的常规体量给足，不出现明显单薄的短题组（补充的题须换情境、换角度、换数据，同类设问重复不算新题）
3. 每道题/任务完整可作答
...
【创作要求】板块划分与题量按生成时注入的【教辅结构】执行（题量与各板块内的小题数量按本学科本学段同类教辅的常规体量给足，不出现明显单薄的短板块；补充的题须换情境、换角度、换数据，同类设问重复不算新题）；聚焦{unit}薄弱点；题目完整可作答。
...
· 细致与深度要求：任务体现由浅入深的认知进阶，本课新知都要**落到**（以【锚点清单】为核对清单——这是**下限**；本资料为预习定位，不做清单外补充）；每个任务写明"做什么+怎么做+完成标准（能自查）"，不写空泛的"了解/掌握"式口号；用真实情境或生活经验引出待学问题；引导用已有知识联结新知、带着问题进课堂；「我的疑问」给出思考支架（圈画关键处、联系旧知、先自查再记疑）。
...
【创作要求】短文无语病；篇数/字数/题量按生成时注入的【教辅结构】执行（每篇设问数量按本学科本学段同类教辅的常规体量给足，不出现只有零星设问的单薄选文；补充的设问须换角度、换层次，同类设问重复不算新题）。
...
【创作要求】{unit}全部核心知识都要落到，不落下；重点标注、文字精炼；栏目与篇幅按生成时注入的【教辅结构】执行。
...
· 细致与深度要求：内容置于语境/情境呈现，不孤立罗列；本课时/单元要求掌握的内容都要练到（以【锚点清单】为核对清单——这是**下限**；默写严格对应教材要求，不做清单外补充）；含易错点辨析；内容层次与呈现方式贴合本学段（低年级重书写规范、中高年级与初高中重语境理解与运用）。
...
【创作要求】栏目以注入的【教辅结构】为准；核心知识都要落到，不落下。
· 细致与深度要求：知识网络化呈现；核心知识按由浅入深的认知层次分层梳理，逐条给要点（分层切法依内容自定）；典型题给解题思路剖析；易错辨析具体到易混处；自测有梯度，本单元核心知识都要练到。
