...
const QUESTION_FORMAT = (ctx = {}) => {
  ...
  const clauses = [
    '填空空位（括号空位或横线）长度即留空宽度（系统按空位长度统一换算渲染宽度）',
    '选择/判断类与填空按作答形式留空位',
  ];
  if (carrier) {
...
${itemRule}
${fmt}
· 🔴 答案/解析/评分标准/听力原文仅出现在独立答案区（once 模式在正文之后，split 模式由系统单独生成）；题目区严禁混入任何答案/解析/评分标准/听力原文
${quality}`;
};
...
export const SUBJECT_STAGE_EXTRAS = {
  // ── 语文 ──
  '语文|primary_low': { text: '识字写字在语境句中考（字词置于具体语句，不孤立罗列）；写话贴近生活、给情境与词语支架。', source: '2022义教语文·第一学段识字与写字/阅读与鉴赏 + 语言文字积累与梳理任务群' },
...
· 答案区大题用 <h2>（如 <h2>参考答案与评分标准</h2>），逐题对应、注明分值；评分标准/等级表用 <table>
