...
export function buildInjectionInstruction(opts = {}) {
  ...
  const map = {
    ...
    // 🔧 素材头占位渲染（2026-09 b 方案落地）：素材头"仅供理解"口径与生成器双卡素材区一致——
    //    委托书内该行仅作模板结构提示；真实素材（依据卡+参考卡）由生成器在委托末尾追加，
    //    不再以"教材原文自动附加"表述（旧文曾暗示原文会全文随委托出现，诱导照搬）
    '{material}': '（教材原文素材由系统按本资料覆盖的知识点检索后，生成时以"素材·依据/参考"注入本委托末尾——仅供理解结构与梯度，禁止照搬字面）',
  };
