...
${itemRule}
${fmt}
· 🔴 答案/解析/评分标准${HAS_LISTENING(ctx.subject) ? '/听力原文' : ''}仅出现在独立答案区（once 模式在正文之后，split 模式由系统单独生成）；题目区严禁混入
${quality}`;
};
...
· 若正文含练习/自测/典型例题，可在正文结束后另起一部分输出对应的《参考答案与解析》，但【严禁】将前面的知识框架/重点梳理/考点梳理/易错辨析/默写内容等正文内容整体重复输出到答案区；答案区仅逐题作答
  ...
  other: (genType = '') => {
    const tip = genType === 'errorbook'
      ...
      : genType === 'summary' || genType === 'review' || genType === 'preview' || genType === 'dictation'
        ? '仅针对正文中的练习/自测/例题逐题作答【严禁将正文的知识框架/重点梳理/考点梳理/易错辨析/默写内容等梳理正文整体复述到答案区——正文已提供，答案区只写题目解答】'
