...
export const SUBJECT_STAGE_EXTRAS = {
  ...
  '语文|primary_high': { text: '阅读含概括与简单评价；跨学科学习等任务群结合教材主题组织活动；习作给选材支架与字数提示。', source: '2022义教语文·第三学段阅读与鉴赏（抓住要点）/表达与交流（记实与想象作文）/课程内容·跨学科学习任务群' },
  ...
  '英语|high': { text: '听力、阅读语篇真实地道；语言知识在语篇语境中考查；应用文与续写设问清晰、给足信息。', source: '高中英语课标(2017/2020)·核心素养（语言能力/文化意识/思维品质/学习能力）' },
  // ── 物理 ──
  ...
  // ── 信息科技 ──
  '信息科技|primary_low': { text: '设备认知与合理使用数字设备（合理安排使用时间、保护个人隐私）以情境设问；操作步骤描述清晰。', source: '2022义教信息科技·第一学段学段目标（知道数字设备使用基本规范，合理安排使用时间养成好习惯）+ 内容模块（信息交流与分享/信息隐私与安全）' },
...
· 若正文含练习/自测，可在正文结束后另起一部分输出对应的《参考答案与解析》，但【严禁】将前面的知识框架/重点梳理/考点梳理/易错辨析/默写内容等正文内容整体重复输出到答案区；答案区仅逐题作答
 ...
 *    严禁把正文的知识框架/重点梳理/考点梳理/辨析/默写等正文内容整体复述——正文已提供，答案区不复述。 */
  ...
  other: (genType = '') => {
    const tip = genType === 'errorbook'
      ...
      : genType === 'summary' || genType === 'review' || genType === 'preview' || genType === 'dictation'
        ? '仅针对正文中的练习/自测/变式逐题作答（典型例题的解答与解析已在正文讲解展示，严禁在答案区重复复述）；【严禁将正文的知识框架/重点梳理/考点梳理/易错辨析/默写内容等梳理正文整体复述到答案区——正文已提供，答案区只写题目解答】'
