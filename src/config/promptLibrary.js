...
const QUESTION_FORMAT = (ctx = {}) => {
  ...
  lines.push('· 卷面自洽（编辑自查总纲，成稿前逐题核对）：①任务词与实际作答形态一致——说"选择/选一选"必须给出选项、说"判断/对的打√"须留 √× 作答空、说"填空/填一填"须留空位、说"写在横线上/格子里"须给对应书写位，任务词与作答形态不符即改；②每道题（含 (1)(2)(3) 等子题）必须可作答——有处填、有处选、有处写、有处连，不得出现只有任务描述却没有作答位置/作答对象（如"给加点字选择读音"却无加点字与选项）的题；③题干文字不加下划线/删除线等修饰（画线语义只作用于题干声明要求画线的那句话/词语对象）；');
  return lines.join('\n');
};
