 ...
 * 仅这三类注入"题组的分组与命名由你按内容需要自定、**不以清单条目作分组或命名**"这一**结构**约束。
...
const outputFormatSections = (mode = 'question', ctx = {}) => {
  ...
  const groupRule = autonItem
    ? '· 栏内组织：每栏内题目由你按内容需要分组成大题（分组依据不是知识点清单：同一知识点可分布在多个大题里，一个大题也可含多个知识点），组前以 <h3> 大题标题起——标题由你自拟'
...
2. 栏目参照生成时注入的【教辅结构】搭建（栏目序列与学段要求）；在本次勾选范围的基础上安排相应充实的训练量，不因省事而缩减练习；同一知识点在同一资料内不得以相同深度重复考查，分层次呈现时各层侧重不同、错开设问角度与情境
3. 每道题/任务完整可作答
· 内容与分组分开看：本课【锚点清单】所列内容须在资料中**真的练到**（分散在多题中体现、或多点合成一题均可，**不要求逐项对应**）——这是**内容**要求；题组的分组与命名由你按内容需要自定，**不以清单条目作分组或命名**——这是**结构**要求。定稿前对着清单过一遍，整份都没有落点的补上
...
【创作要求】板块划分参照生成时注入的【教辅结构】（板块序列与学段要求）；在本次勾选范围的基础上安排相应充实的训练量，不因省事而缩减练习；聚焦{unit}薄弱点；同一知识点在同一资料内不得以相同深度重复考查，分层次呈现时各层侧重不同、错开设问角度与情境；题目完整可作答。
...
【创作要求】以问题驱动预读，设计少量可操作、可检查的预读任务，任务形态灵活、不预设具体形式；栏目参照注入的【教辅结构】搭建（须含"我的疑问"栏目）；紧扣教材原文。
...
【创作要求】阅读材料无语病；栏目参照生成时注入的【教辅结构】搭建；在本次勾选范围的基础上安排相应充实的训练量，不因省事而缩减设问；同一知识点在同一资料内不得以相同深度重复考查，分层次呈现时各层侧重不同、错开设问角度与情境。
...
【创作要求】在本次勾选范围的基础上全面细致地汇总知识；文字精炼、核心知识重点标注；栏目参照生成时注入的【教辅结构】搭建。
...
【创作要求】严格对应教材要求，内容准确无误；栏目参照生成时注入的【教辅结构】搭建。
...
【创作要求】栏目参照注入的【教辅结构】搭建；在本次勾选范围的基础上汇总核心知识。
