...
· 细致与深度要求：按考点精准聚焦；板块内由易到难；每板块含易错点辨识与思路点拨，解析讲明"错因—如何避免—正确思路"，不只给答案。
...
· 细致与深度要求：任务沿"基础通读→重难点深入→延伸挑战"递进，覆盖本课全部新知；每个任务写明"做什么+怎么做+完成标准（能自查）"，不写空泛的"了解/掌握"式口号；用真实情境或生活经验引出待学问题；引导用已有知识联结新知、带着问题进课堂；「我的疑问」给出思考支架（圈画关键处、联系旧知、先自查再记疑）。
...
【创作要求】覆盖{unit}全部知识点并标注出处（按注入的【教材原文】章节），不遗漏；重点标注、文字精炼；栏目与篇幅按生成时注入的【教辅结构】执行。
...
· 细致与深度要求：内容置于语境/情境呈现，不孤立罗列；覆盖本课时/单元全部要求掌握内容；含易错点辨析；按学段体现层次（低段重书写规范、中高段在情境中考查）。
...
【创作要求】栏目以注入的【教辅结构】为准；知识点覆盖与出处标注以注入的【教材原文】章节为准。
· 细致与深度要求：知识网络化呈现；考点按"识记→理解→运用"分层梳理，逐条给要点与教材出处；典型题给解题思路剖析；易错辨析具体到易混处；自测分层（基础/提高），覆盖本单元全部知识点。
