...
const OUTPUT_FORMAT_BLOCK = (mode = 'question', ctx = {}) => {
  ...
  const head = mode === 'exam'
    ...
    : '如 <h2>一、基础建构任务</h2>（蓝图结构=大类栏目标题，居中大字、与正文层级区分），不标注分值';
  const headLabel = isContent ? '栏目标题' : '大类标题';
  const itemRule = isContent
    // 内容型：条目/编号/标记规则统一由 CONTENT_FORMAT 单源给出（曾在此与 CONTENT_FORMAT 同义双写，批次 B 复核修）
    ? ''
    : '· 大类层级：蓝图结构栏目（如"基础建构任务"）作为本卷大类标题，用 <h2> 按"一、…"序号呈现，居于该栏全部内容之前；大类不作大题使用、不承载题目本身\n· 栏内组织：每栏内题目按内容/任务分组成大题，组前以 <h3> 内容大题标题起——标题由你自拟本组内容概括（如 <h3>（一）理解小数乘法的意义</h3>），概括题组实际练什么，不写"覆盖/考查/练习"等字样、不罗列知识点清单；同组小题共用语境递进或并列；若整栏为单一连贯任务链（如一个探究任务含多个步骤）可不用分组、直接排题\n· 题目以 <p class="question"> 包裹并带题号（1. 2. 3.…），子题用 (1)(2)，题号全卷连续（跨大题组不断号）\n· 段落组织：每道题、每个任务（情境/活动/成果各条）独立成 <p> 段落，段间空行分隔；严禁把多个题目或条目堆叠在同一段落内';
