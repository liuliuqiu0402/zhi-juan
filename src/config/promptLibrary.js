...
const FORMAT_RULES = `· 作答空规范（全卷统一）：
  ...
  连线题把左右两列内容逐行写出、两列之间用至少2个全角空格分隔（如"雀　　隹字旁"），右列顺序必须打乱（不得与左列一一对应，防止直接暴露答案），🔴 严禁在左右项之间画线或用 ---/——/─ 等连字符连接、严禁右列只写序号；输出前逐对自检配对正确性；
...
【命题要求】
1. 依据2022版新课标命题：素养立意、情境真实适切、设问有层次，杜绝机械记忆与偏题怪题
...
· 🔴 严禁输出"答案/解析"混入正文；页码、分页由系统生成，正文不输出页码文字
