...
· 内容正确：图文一致（配图/图形与题干严格吻合）；逻辑自洽（条件充分、因果清晰、设问与答案对应）；知识点与能力要求不超出本学段课标学业质量要求——选文/情境等材料载体可来自课外（课外阅读、整本书、生活、科技、时政均为课标要求与鼓励方向），但材料难度适切本学段、设问指向的能力点在课标内；全部内容围绕本单元主题与考点（知识锚定教材单元）`;
...
【创作要求】按考点或能力点分类组织（每类一个板块），类内按基础→提升→拓展分层，聚焦{unit}薄弱点，板块数与题量按生成时注入的【教辅结构】执行；同板块内题目不雷同（换情境/角度/设问呈现变式）；覆盖教材核心知识点；题目完整可作答。
...
【创作要求】在语境中默写（字词/篇目置于语句情境考查，不孤立罗列）；严格对应教材要求，覆盖本单元全部要求掌握的字词/篇目；拼音/字形准确；覆盖量与栏目按生成时注入的【教辅结构】执行。
...
【创作要求】按知识点或错因分类组织（每类一个板块），每题结构按生成时注入的【教辅结构】执行（原题→归因→解法→变式→策略）；归因禁止笼统写"粗心"。
...
export const SUBJECT_STAGE_EXTRAS = {
  ...
  // ── 地理 ──
  '地理|middle': '以地图/图表为载体设问；综合题沿区域定位→要素分析→人地关系；资料数据真实。',
...
export const STAGE_EXAM_EXTRAS = {
  ...
  primary_mid: '情境生活化（购物/校园/旅行）；设问常规、分层递进；认知底线：难度与情境符合中段认知，不超前引入高段知识。',
  primary_high: '综合性强，阅读量加大；情境联系生活实际；增加开放性与思辨性设问，鼓励表达观点；认知底线：可适当开放思辨，但不超高段课标能力要求。',
  middle: '对标中考结构（题量/分值比例）；能力立意（理解/应用/探究）；情境真实（生活/社会/科技）；认知底线：能力要求对标中考课标，不超纲不超学段。',
...
export const STAGE_TEACHING_EXTRAS = {
  primary_low: '情境游戏化、图文并茂；难度低起点，多数学生能完成；认知底线：难度与情境符合低段认知水平，不出现未学概念与抽象符号。',
  primary_mid: '情境生活化（购物/校园/旅行）；设问分层；认知底线：难度与情境符合中段认知，不超前引入高段知识。',
  primary_high: '综合性强，阅读量加大；情境联系生活实际；增加开放性与思辨性设问；认知底线：可适当开放思辨，但不超高段课标能力要求。',
  middle: '能力立意（理解/应用/探究）；情境真实（生活/社会/科技）；认知底线：能力要求对标本学段课标，不超纲不超学段。',
  high: '素养立意（学科核心素养导向）；复杂真实情境+综合设问；认知底线：知识内容符合高中课标，不超学业质量要求。',
};
