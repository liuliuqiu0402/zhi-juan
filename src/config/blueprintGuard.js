 ...
 *   G1 分值闭合    — 每个板块 score > 0 且板块之和 === fullScore（精确到分）
 *   G2 听力占比    — 英语卷听力板块分值占比在学段合理区间
 *                    （小学低段 40% / 中段 30-40% / 高段 30% / 初中 20-25% / 高中 20%）
 *   G3 题量×分值   — note 中可解析的"每空/每题 X 分"与板块分值自洽
 *                    （如"5小题×2分" ⇒ 板块应≈10分，允许 ±1 舍入）
 *   G4 板块粒度    — 板块分值非 0 且不碎（≥3 分，避免无意义的 1-2 分碎片板块）
 *   G5 学段结构    — 主干学科（语数英）各学段均有完整蓝本；初中含听力学科必有听力板块
...
export function validateBlueprint(bp, key = '') {
  ...
  if (Math.abs(sum - (bp.fullScore || 0)) > 0.01) {
    errors.push({ code: 'G1', severity: 'error', detail: `${key}: 板块分值之和 ${sum} ≠ 满分 ${bp.fullScore}` });
  }

  // G4 板块粒度
  for (const sec of bp.sections) {
    if (sec.score && sec.score > 0 && sec.score < 3) {
      errors.push({ code: 'G4', severity: 'warning', detail: `${key}·${sec.name}: 板块分值 ${sec.score} 过碎（<3分）` });
    }
  }
  ...
  if (subject === '英语' && stg === 'middle' && !bp.sections.some(s => /听力|听音|Listening/i.test(s.name))) {
    errors.push({ code: 'G5', severity: 'error', detail: `${key}: 初中英语卷缺少听力板块` });
  }
