...
export function buildAnswerSpaceInstruction(subject = '', stage = '') {
  ...
  //    题干末尾，作 "1. ……的是( )"。🔴 括号一律**半角（英文状态）**（既有用户规格；span 载体渲染自带半角括号），
  //    不因学科改括号形制。无学科兜底（通用模板）时落中文支（题干末尾）。故作"全学科一律题首括号"的分叉开关。
  // 🔴 2026-09-26 用户定：按"**外语类**"判定，不只认"英语"这一个字面量——语言类学科名以语言名结尾或含"外语"，
  //    今后新增外语学科自动纳入（本判定是**载体规格内的单一出口**，勿在他处另写一套学科名单）。
  const FOREIGN_LANG_SUBJECT_RE = /(?:外语|英语|日语|俄语|法语|德语|西班牙语|韩语|意大利语|葡萄牙语|阿拉伯语)/;
  const choiceBlankAtHead = FOREIGN_LANG_SUBJECT_RE.test(String(subject || ''));
  const lines = [
    ...
    '· 题面带选项（A./B./C. 等）的题，其作答位**形态一律圆括号空位**（不用下划线空/横线空——圆括号对应所填字母/序号/√×等符号，下划线空对应填词/句/数等短答，两者不得混用）；🔴 **括号一律用半角（英文状态）括号**（沿用既有用户规格，且 span 载体渲染自带半角括号）：' + (choiceBlankAtHead
      ? '**位置在题号之前的题首**，写成"( ) 1. 题干…"（外语类正式卷惯例）'
      : '**位置在题干末尾**，写成"1. ……的是( )"（中文科目正式卷惯例）') + '；选项行内、选项末尾，以及选项行之后，一律不加作答位：不给整行横线、不给空白作答行、不另设作答区，作答位只有上述那一处（题干内指示所填内容的那处空位不是作答位——它是题面的一部分、由选项作答，照常保留，不在本句限制之内）；🔴 本条是**硬约束**，题面若声明了与本条不符的形态，**改题面、不改本条**——与【题目自洽①】冲突时以本条为准；',
