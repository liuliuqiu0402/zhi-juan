 ...
 *     空位具体形态改由 buildAnswerSpaceInstruction 按"题干措辞与题型语感"倾向引导、不强制绑定
 *     （圈选判断选择类多用圆括号/短答列举多用横线空），本函数只保留"字位↔书写宽"这一可执行计数锚。
 ...
 *     题型语感选定（只给倾向与可核对锚，防孤立形态诱导）；真正定死的只有程序性硬约束：
 *      · 宽度换算锚（短答空位按答案长度换算，BLANK 单一事实源）——空位多宽是硬约束；
 *      · 圈选判断选择类（填字母/序号/√×）倾向圆括号空位（　），短答与"列举归类"空位倾向横线空——
 *        仅为倾向引导（2026-09 用户定稿：不给每个小题定死形态，交由模型语感），非强制；
 *      · 判断/选择/圈选类题（含阅读判断题）的作答位即题面空位（括号空位/圈选位），不另附长横线
 *        作答区（2026-09 用户实证：阅读判断题被模型额外出 2 行长横线；补差端对判断/选择类
 *        本不补差，本条堵生成侧源头——与 2k 结构性题型排除同口径）；
 *      · 题面带选项（A./B./C. 等）的题，其作答括号位置定死：在题干前（题首，如（　）1. 题干…），
 *        选项行内/选项末尾一律不加作答位——2026-09 用户实证：模型把答案空位挂到选项末尾
 *        （C. are; am＿）；位置属排版硬约束（与宽度锚/一空一载体同级），不交模型语感；
 *        2026-09 用户裁定：只约束"带选项的题"这一个形态，不触碰判断√×位/数学方框圆圈等既有条款，
 *        也不使用题型名（防题型诱导，纯形态描述）；
...
export function buildAnswerSpaceInstruction(subject = '', stage = '') {
  ...
  const lines = [
    '· 作答空间形态按答案类型匹配，不自行发明：',
    '· 空位具体形态（圆括号空／横线空／圈作答）由你按题干措辞与题型语感选定——填字母/序号/√×的圈选判断选择类多用圆括号空位（　）或圈作答，填词/句/数等短答与"列举归类"空位（如"有限小数有＿"）多用横线空位；形态真实可书写、同卷同题型风格统一即可，圈选类括号内宽只须容纳所填符号（1~2 字位）；判断/选择/圈选类题（含阅读判断题）的作答位即题面空位（括号空位/圈选位），不另附长横线作答区；',
    '· 题面带选项（A./B./C. 等）的题，其作答括号在题干前（题首，如（　）1. 题干…）；题干/大题任务写"填入括号内/在括号里"时题首须真实给出括号空位；选项行内与选项末尾一律不加作答位；',
  ...
  if (subject && stage) {
    ...
    if (subject === '数学') {
      ...
      //    □×□＝12 中待填的因数/得数），算式"等号后的得数结果位"（口算直接写得数/计算题答案位）
      //    —— 所有学段一律等号后直接留白书写、不使用方框（程序 normalizeMathCircleBlanks 的
      //    isResultPosition 同语义确定性剥离；与"应用题列式解答留白"一致，卷面惯例见审核基准调研）
      lines.push('· 算式中的『缺数/填数算式填空位』（如 3＋□＝8、□×□＝12 里待填的数）用方框或圆圈呈现，不用下划线空位；但算式『等号后的得数结果位』（口算直接写得数/计算题答案位）一律在等号后直接留白书写，不使用方框、不用圆圈、不用括号；');
      ...
      lines.push('· 列竖式/笔算/脱式计算等需要书写过程的计算题：题干给出算式与要求后，须为每个算式预留书写过程区（算式后留无线空白行，每式 ≥3 行供列式计算），需验算的在题面写明"并验算"并相应留位；严禁用"＝( )"填空形式代替过程书写，结果空位仅用于口算类直接写得数题；');
      ...
      if (stage === 'primary_low') {
        lines.push('· 口算/直接写得数题的结果位在算式等号后直接留白书写；低年级如需定位作答，可用「＝＿＿＿」下划线或「＝（　）」括号，不用方框（方框仅用于缺数/填数算式填空位）；同卷内同一题型形态统一，不混用。');
      } else {
        lines.push('· 口算/直接写得数题的结果位在算式等号后直接留白书写（中高年级/初中卷面惯例），不加方框、不加括号；');
      }
    }
    ...
    if (region.carrier === 'line') {
      ...
      if (subject === '语文') {
        ...
        lines.push('· 写作/表达类的整行书写横线只在整题之后集中给一处（行数按该题答案篇幅）；题干含"要求/提示"分条时，横线给在分条之后，不插在分条之间、不给每个分条各配一处作答区。');
      } else {
