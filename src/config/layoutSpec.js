...
export function buildAnswerSpaceInstruction(subject = '', stage = '') {
  ...
  //    英语（外语）：题首（题号前）半角圆括号，正式卷面作 "( ) 1. 题干…"；中文科目（语文/数学/理化生/政史地/科学）：
  //    题干末尾全角圆括号，作 "1. ……的是（　）"。无学科兜底（通用模板）时落中文支持位（题干末尾）。故作"全学科一律题首括号"的分叉开关。
  const choiceBlankAtHead = subject === '英语';
  const lines = [
    ...
    //    本句原尾"作答位只有题首那一处"易被读成"题干内不得有任何空位"，可能把正常写法删掉（带出新问题）。
    //    补一句界定：题干内指示所填内容的那处空位是**题面的一部分**、不是作答位 → 照常保留、不受本句限制。
    '· 题面带选项（A./B./C. 等）的题，其作答位**形态一律圆括号空位**（不用下划线空/横线空——圆括号对应所填字母/序号/√×等符号，下划线空对应填词/句/数等短答，两者不得混用）：' + (choiceBlankAtHead
      ? '**位置在题号之前的题首**、用**半角**圆括号，写成"( ) 1. 题干…"（外语类正式卷惯例）'
      : '**位置在题干末尾**、用**全角**圆括号，写成"1. ……的是（　）"（中文科目正式卷惯例）') + '；选项行内、选项末尾，以及选项行之后，一律不加作答位：不给整行横线、不给空白作答行、不另设作答区，作答位只有上述那一处（题干内指示所填内容的那处空位不是作答位——它是题面的一部分、由选项作答，照常保留，不在本句限制之内）；🔴 本条是**硬约束**，题面若声明了与本条不符的形态，**改题面、不改本条**——与【题目自洽①】冲突时以本条为准；',
    ...
    '· 同一大题内，"所填为字母/序号/√×等符号"的作答位**位置整段统一**（按上方学科惯例：外语类整段在题首、中文科目整段在题干末尾，不得同段混用两种位置）；括号空位**同一份资料只用一种括号形态**——中文科目用全角「（　）」、外语类用半角「( )」，不得同卷混用全角/半角；',
...
export const buildLongAnswerCarrierInstruction = (subject = '', stage = '') => {
  ...
  if (region.carrier === 'line') {
    ...
    if (subject === '语文') {
      return '· 答案须成句成段书写的题，输出整行书写横线，每行一横、行数按答案篇幅，不得省略（成篇成文类另有专用书写载体，由该载体通道约束，不在本列）；';
    }
    if (subject === '英语') {
      ...
      return '· 答案须成句成段书写的题（含成篇表达）输出整行书写横线，每行一横、行数按答案篇幅，不得省略；\n'
        + '· 需成段/成篇书写的题，其整行书写横线只在整题之后集中给一处（行数按该题答案篇幅）；题干含"要求/提示"分条时，横线给在分条之后，不插在分条之间、不给每个分条各配一处作答区。';
    }
    return '· 答案须成句成段书写的题输出整行书写横线，每行一横、行数按答案篇幅，不得省略；';
  }
  // 🔧 反误引导（2026-09）：模型端须自出空白作答行（<p><br></p> 每段一行），程序 2k 只补差额；
  //    通用禁占位句已含"不写作答区字样"，本句不再重复
  return '· 答案需成篇成段书面展开的长答题：题干后输出无线空白作答行——每个空段落 <p><br></p> 为一行，行数按该题答案篇幅逐题确定（长答多给、短答少给），不得省略；空白行不画横线、不把留白圈成方框；';
};
