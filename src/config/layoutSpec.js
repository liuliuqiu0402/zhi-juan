...
export function buildAnswerSpaceInstruction(subject = '', stage = '') {
  ...
  const lines = [
    ...
    '· 题面带选项（A./B./C. 等）的题，其作答位一律在题干前（题首）且用圆括号空位（　），写成"（　）1. 题干…"——该位是填字母的括号，不用下划线空/横线空，与大题标题是否写"填入括号内"无关；选项行内、选项末尾，以及选项行之后，一律不加作答位：不给整行横线、不给空白作答行、不另设作答区，作答位只有题首那一处（题干内指示所填内容的那处空位不是作答位——它是题面的一部分、由选项作答，照常保留，不在本句限制之内）；',
    ...
    '· 同一大题内，"所填为字母/序号/√×等符号"的作答位**位置整段统一**：整段都在题首，或整段都在句末，不得一部分在题首、一部分在句末；括号空位**同一份资料只用全角「（　）」一种形态**（不用半角「( )」）；',
