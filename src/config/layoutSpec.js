 ...
 *     空位具体形态改由 buildAnswerSpaceInstruction 按"题干措辞与作答需要"倾向引导、不强制绑定
 *     （所填为符号的题多用圆括号/短答列举多用横线空），本函数只保留"字位↔书写宽"这一可执行计数锚。
 ...
 *  🔴 2026-09 生成侧根治（两轮收敛，终稿口径）：作答空位形态不强制绑定、由模型按题干措辞与
 *     作答需要选定（只给倾向与可核对锚，防孤立形态诱导）；真正定死的只有程序性硬约束：
 *      · 宽度换算锚（短答空位按答案长度换算，BLANK 单一事实源）——空位多宽是硬约束；
 *      · 所填为字母/序号/√×等符号的题倾向圆括号空位（　），短答与"列举归类"空位倾向横线空——
 *        仅为倾向引导（2026-09 用户定稿：不给每个小题定死形态，交由模型语感），非强制；
 *      · 作答位就在题面空位内（括号空位/圈选位）、不另设独立作答区的题，不另附长横线
 *        作答区（2026-09 用户实证：阅读判断类题被模型额外出 2 行长横线；补差端本不补差，
 *        本条堵生成侧源头——与 2k 结构性排除同口径）；
 ...
 *      · 题面带选项（A./B./C. 等）的题，其作答位**位置与形态双双定死**：在题干前（题首）且用
 *        圆括号空位（　），写成"（　）1. 题干…"；选项行内/选项末尾一律不加作答位——
...
export function buildAnswerSpaceInstruction(subject = '', stage = '') {
  ...
  const lines = [
    ...
    '· 题面带选项（A./B./C. 等）的题，其作答位一律在题干前（题首）且用圆括号空位（　），写成"（　）1. 题干…"——该位是填字母的括号，不用下划线空/横线空，与大题标题是否写"填入括号内"无关；选项行内、选项末尾，以及选项行之后，一律不加作答位：不给整行横线、不给空白作答行、不另设作答区，作答位只有题首那一处；',
    `· 短答空位宽度按"恰好容纳该空答案"换算：先在心里给出该空答案并数清字符数（数字/汉字/小数点各算 1 个），写等量的全角空格（${buildBlankWidthInstruction()}），连列空位全带、不得遗漏；`,
  ...
  if (subject && stage) {
    ...
    if (subject === '数学') {
      // 🔧 2026-09 载体根治·结果位与填空位二分：方框/圆圈仅用于"缺数/填数算式填空位"（3＋□＝8、
      //    □×□＝12 中待填的因数/得数），算式"等号后的得数结果位"（算式求出、写在等号后的得数所在位）
      //    —— 所有学段一律等号后直接留白书写、不使用方框（程序 normalizeMathCircleBlanks 的
      //    isResultPosition 同语义确定性剥离；与"应用题列式解答留白"一致，卷面惯例见审核基准调研）
      lines.push('· 算式中的『缺数/填数算式填空位』（如 3＋□＝8、□×□＝12 里待填的数）用方框或圆圈呈现，不用下划线空位；但算式『等号后的得数结果位』（算式求出、写在等号后的得数所在位）一律在等号后直接留白书写，不使用方框、不用圆圈、不用括号；');
      ...
      lines.push('· 需书写计算过程（列竖式/笔算/脱式等）的算式题：题干给出算式与要求后，须为每个算式预留书写过程区（算式后留无线空白行，每式 ≥3 行供列式计算），需验算的在题面写明"并验算"并相应留位；严禁用"＝( )"填空形式代替过程书写，结果空位仅用于只需直接写出得数、无需过程的题；');
      // 🔧 结果位学段化（2026-09 实证调研：中高年级起直接写得数 = 等号后留白、初中答题卡留白；
      //    低年级口算常用"＝＿＿＿"下划线或"＝（　）"定位作答。方框在任何学段都不用于结果位——
      //    方框只在缺数/填数算式填空位出现；程序 normalizeMathCircleBlanks（含已入库内容解壳）
      //    同判据确定性剥离结果位方框，杜绝"中高段/计算题/应用题结果位配方框"的错配）
      if (stage === 'primary_low') {
        lines.push('· 只需直接写出得数、无需书写过程的算式题，其结果位在等号后直接留白书写；低年级如需定位作答，可用「＝＿＿＿」下划线或「＝（　）」括号，不用方框（方框仅用于缺数/填数算式填空位）；同卷内同一形态统一，不混用。');
      } else {
        lines.push('· 只需直接写出得数、无需书写过程的算式题，其结果位在等号后直接留白书写（中高年级/初中卷面惯例），不加方框、不加括号；');
      }
    }
    ...
    if (region.carrier === 'line') {
      ...
      if (subject === '语文') {
        lines.push('· 答案须成句成段书写的题，输出整行书写横线，每行一横、行数按答案篇幅，不得省略（成篇成文类另有专用书写载体，由该载体通道约束，不在本列）；');
      } else if (subject === '英语') {
        lines.push('· 答案须成句成段书写的题（含成篇表达）输出整行书写横线，每行一横、行数按答案篇幅，不得省略；');
        // 🔴 2026-09-14（用户实证）：英语写作题把"要求：1. 2. 3."逐条当成了需作答的题 → 每条后各给 4 行横线
        //    （实测产物：横线插在要求条目之间，卷面像"三条小题各带作答区"）。本条只定位**横线给在哪里**：
        //    整题一处、分条之后；不涉及题型、不涉及覆盖口径。
        lines.push('· 需成段/成篇书写的题，其整行书写横线只在整题之后集中给一处（行数按该题答案篇幅）；题干含"要求/提示"分条时，横线给在分条之后，不插在分条之间、不给每个分条各配一处作答区。');
      } else {
        lines.push('· 答案须成句成段书写的题输出整行书写横线，每行一横、行数按答案篇幅，不得省略；');
      }
    } else {
      // 🔧 反误引导（2026-09）：模型端须自出空白作答行（<p><br></p> 每段一行），程序 2k 只补差额；
      //    通用禁占位句已含"不写作答区字样"，本句不再重复
      lines.push('· 答案需成篇成段书面展开的长答题：题干后输出无线空白作答行——每个空段落 <p><br></p> 为一行，行数按该题答案篇幅逐题确定（长答多给、短答少给），不得省略；空白行不画横线、不把留白圈成方框；');
    }
  }
  return lines.join('\n');
}
 ...
 *  校准依据（2026-08，按课标字数要求与卷面惯例 + 标点缩进余量）；⚠️ 2026-09-17 核查更正：原"小学高段按课标 40 分钟不少于 450 字"属**误引**——"40 分钟不少于 400 字"是 2001 年实验稿条款、2011 年版已删除，"450 字"课标查无出处；第四学段现行条款才是"45 分钟能完成不少于 500 字"。数值未改（原即按卷面惯例校准）：
