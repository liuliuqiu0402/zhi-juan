...
export function buildAnswerSpaceInstruction(subject = '', stage = '') {
  const lines = [
    ...
    `· 填空类（填词/句/数等短答）在句内或行尾写下划线空位，宽度按答案长度（${buildBlankWidthInstruction()}），连列空位全带、不得遗漏；`,
    '· 作答空间只以真实留白或书写载体呈现：严禁用"答：""作答区"等文字充当或预置作答空间；',
  ];
  if (subject && stage) {
    // 🔧 空位形态一致性（2026-09 实证：题 10"0.86×3.2 ＿（　）"把同一答案位写成两种载体叠加——
    //    同题并列空位形态须统一、一位一载体，不得叠加）
    lines.push('· 同一题（含并列子题）的同性质作答空位形态保持一致——同用圆括号空或同用填空横线，一个空位只写一种载体，不得叠加；');
    // 🔧 空位宽度随答案长度（2026-09 实证：课时练 35 处填空横线全部统一 8 字位宽——模型未按答案长度
    //    逐空定宽。措辞两修：①给可执行换算（数清答案字符数 → 写等量全角空格），不依赖"字位"抽象理解；
    //    ②不以"禁统一"为约束——答案等长时空位本就该等宽，宽度一律以各空答案长度为唯一依据）
    lines.push('· 填空空位宽度以"恰好容纳该空答案"为准，换算程序：先在心里给出该空答案，数清其字符数（数字/汉字/小数点各算 1 个），再在空位里写等量的全角空格（1 个字符 ≈ 1 个全角空格 ≈ 1 em）；同一题中答案等长的空位等宽、答案不等长的空位按各自答案长度不等宽——宽度只由答案长度决定，不为整齐统一加宽、也不为凑差异改窄；');
    lines.push('· 圈选/判断空位（填 √×、字母、序号或符号）括号内宽只需容纳所填符号（1~2 字位），不得给出 6~8 字位的宽大空位；');
    ...
    if (region.carrier === 'line') {
      // 🔧 书写行学科内部再按写作载体分流（2026-09 歧义根治）：
      //    语文写作（习作/看图写话）走作文格专用通道 → 横线句排除"写作类"（否则模型画横线挤掉作文格补差）；
      //    英语写作（应用文/续写/书面表达）恰恰是横线体系（2j-5b 程序补横线同语义）→ 横线句必须包含"写作"，
      //    绝不可用"写作类另有专用载体"排除（曾误引英语写作不画横线 → 漏载体）
      if (subject === '语文') {
        lines.push('· 成句成段作答的主观书写题（句子练习/仿写/简答等；习作/写话另有专用书写载体）按答案篇幅输出整行书写横线，每行一横，不得省略；');
      } else if (subject === '英语') {
        lines.push('· 成句成段作答的主观书写题（写作/续写/书面表达/句子练习等）输出整行书写横线，每行一横、行数按答案篇幅，不得省略；');
      } else {
        lines.push('· 成句成段作答的主观题（简答/句子书写等）输出整行书写横线，每行一横、行数按答案篇幅，不得省略；');
      }
    } else {
      // 🔧 2026-09 反误引导根治：曾写"留无线空白（按分值由排版层补足）"——把模型该做的输出作答空间
      //    外包给程序（程序"补足"从无开始的判定链脆弱 → 卷面无空间实证）。模型端须做它该做的：
      //    长答题后真实输出空白作答行（空段落 <p><br></p> = 一行无线空白，行数按答案篇幅由模型给足）；
      //    程序端 2k 只做"补足"（校验行数不足才加），不再承担"从无创建"。
      lines.push('· 解答/计算/证明/论述/简答/赏析等长答题：在题干后输出无线空白作答行——每个空段落 <p><br></p> 表示一行作答空间，行数按该题答案篇幅逐题确定（写几步/几句给几行，长答多给、短答少给，不同题行数可不同），不得省略、不得全卷统一行数；空白行不画横线、不把留白圈成方框，不写"作答区"字样；');
    }
  }
  return lines.join('\n');
}
