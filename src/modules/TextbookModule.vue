    ...
    >
      ...
      <div class="filter-search-row">
        ...
        <div class="filter-selects">
          ...
          <!-- 高中：筛「册次」不筛「年级」——高中教材按必修／选择性必修分册，不绑定年级 -->
          <select
            v-if="filterStage === '高中'"
        ...
        >
          ...
          <div class="item-header">
            ...
            >
              ...
              <!-- 高中：显示册次。列表本来不展示学段/年级，同科多册（必修1/2/3…）若再不显示册次
                   就只剩文件名可辨，故册次单独加一枚小标签 -->
              <span
                v-if="book.volume"
                class="volume-tag"
              >{{ book.volume }}</span>
    ...
    >
      <div class="modal">
        ...
        >
          ...
          <!-- 高中：填「册次」而不是「年级」（教材按必修／选择性必修分册、不绑定年级；
                  ...
                  >
                    ...
                    >
                      <div style="font-size:12px;font-weight:600;color:#34495e;">
                        ...
                        >{{ ck.level }}</span>
                        <!-- 🔬 (b) 条目性质·可点改判（2026-09-14）：材料类条目生成期只作理解与难度依据、
                             不列入命题覆盖单位。点击即在「知识 ⇄ 材料」间切换——写的就是 resolveAnchorKind
                             优先读的显式 kind（与教材库同一条链、同一判据），落盘后落库/读盘/生成端一并生效 -->
                        <span
                          class="kind-chip"
...
// 高中「册次」：普通高中教材按必修／选择性必修**分册**，教材本身不绑定年级——各省教学用书目录的
// 「册次」与「使用年级」是两栏并列（使用年级写的是区间"高一年级至高三年级"），且"必修＝高一"在各省
// 并不成立（山东语文必修下排第二学年；广东思想政治/物理必修到高二上才完成）。故高中一律"选册次、
// 不选年级"，只有非高中才用 filterGrade。
...
const filterVolumeOptions = computed(() => {
  const set = new Set();
  for (const t of textbookStore.textbooks) {
    const v = t.volume;
    if (v) set.add(v);
  }
...
const filteredTextbooks = computed(() => {
  ...
  if (filterVolume.value) result = result.filter(b => b.volume === filterVolume.value);
...
const saveTextbook = async () => {
  ...
  try {
    ...
    //    （这套教材可能只有文字、没有图），别再让 read-file 抛 ENOENT 刷主进程日志。
    ...
    if (ext === 'pdf') {
      ...
      const bookmarks = committedFlat.map(item => ({
        ...
        //    同步成了当时的页码），再 +pageOffset 就等于**多加了一遍偏移量**，书签页码错位。
        //    改为与编辑器所见完全一致：所见即所签。
        page: item.page,
        level: item.level + 1
      }));
...
/* 🔬 条目性质标签·可点改判（2026-09-14 (b)）：
   「材料」是例外项 → 常显（紫底，一眼看出哪些不计入覆盖单位），点击改判为知识；
   「知识」是常态 → 默认不渲染（display:none，**零布局影响**：不占位、不改行高、不改变原有观感），
   鼠标移入该**条目**（.kind-item，含其具体概念行）才出现；悬停区=整条目 → 即使标签换行也不会闪烁。
   触屏无 hover → 媒体查询下常显，保证"知识 ⇄ 材料"双向都可改判（不是只能改一个方向）。
   点击即在两个显式取值间切换（写回 ck.kind 并立即落盘），生成期按新判据分流。 */
.kind-chip {
  margin-left: 6px;
  padding: 2px 6px;
  border-radius: 3px;
