<template>
  <div class="template-module">
    <!-- 左侧模板库面板（宽度可由中间拖动条手调，见 useLibraryView） -->
    <div
      class="library-panel"
    ...
    >
      <div class="panel-header">
        ...
        <!-- 分组折叠总开关（与教材库同源；仅桌面） -->
        <button
          v-if="!isMobile && groupedTemplates.length > 0"
          class="btn btn-sm"
        ...
        >
          ...
          <div class="item-header">
            ...
            <div class="item-actions">
              <!-- 🏷️ 元数据补标（与教材库同源）：分组按「学段→学科」，而这两项只在导入时能选，
                   老数据缺了就归不了位。缺项时标成橙色；只改元数据，不动 name/id/路径。 -->
              <button
                class="icon-btn"
                :style="needsMetaBackfill(tpl) ? 'color:#e08000;' : ''"
                :title="needsMetaBackfill(tpl)
                  ? '编辑元数据（这条缺学段／学科，补上才能归入分组）'
                  : '编辑元数据（学段 / 学科 / 册次）'"
                @click.stop="openMetaDialog(tpl)"
              >
                🏷️
              </button>
              <button
    ...
    <!-- 🏷️ 元数据补标弹窗（学段/学科/册次；组件与教材库共用一份实现） -->
    <LibraryMetaDialog
...
// 📋 列表视图：学段→学科 两级分组（纯函数）+ 分栏拖动/折叠状态（与教材库共用同一实现）
...
// 🏷️ 元数据补标（学段/学科/册次）：规则在 utils/libraryMetaEdit（纯函数、有单测），弹窗与教材库共用
    ...
    return h('div', { class: 'outline-node' }, [
      h('div', {
        ...
        (node.analyzed && node.rawText && node.rawText.trim().length > 0) ? h('span', {
          ...
          onClick: (e) => {
            e.stopPropagation();
            templateStore.openChapterAnalysis(bookData, node);
          }
        }, '👁️') : null,
        h('span', { 
...
const filteredTemplates = computed(() => {
  ...
  if (searchKeyword.value.trim()) result = result.filter(t => t.name.toLowerCase().includes(searchKeyword.value.trim().toLowerCase()));
  ...
  if (filterVersion.value) {
    result = result.filter(t => {
      const vMatch = t.name.match(/(人教版|苏教版|北师大版|外研版|粤教版|湘教版|鲁教版|浙教版|沪教版|川教版|闽教版|冀教版|鄂教版|译林版|人教PEP版)/);
...
const toggleCollapseAll = () => {
  ...
  else collapseAll(groupedTemplates.value);
};
...
const importTemplateFile = async () => {
  try {
    ...
    const count = await syncImportProcess(chapters);
...
const toggleExpand = (id) => {
  const idx = expandedTemplates.value.indexOf(id);
  idx > -1 ? expandedTemplates.value.splice(idx, 1) : expandedTemplates.value.push(id);
};

// ✅ 勾选/取消整本模板
const toggleTemplateSelection = (tpl, checked) => {
  templateStore.toggleTemplateSelection(tpl, checked);
};
...
// 清空选择
const clearSelection = () => templateStore.clearSelection();

// 批量删除
const batchDelete = async () => {
  const selected = templateStore.templates.filter(t => t.selected);
  const confirmed = await showConfirmDialogFn(`确定删除选中的 ${selected.length} 个模板吗？本地文件也将被永久删除！`);
  if (!confirmed) return;
  for (const t of selected) {
    try {
      const filePath = t.pdfPath || t.filePath;
      if (filePath) await deleteFile(filePath);
      if (t.imagesDir) await deleteDirectory(t.imagesDir);
      if (t.coverPath) await deleteFile(t.coverPath);
    } catch (e) {
      console.error('批量删除模板文件失败:', e);
    }
  }
  templateStore.templates = templateStore.templates.filter(t => !t.selected);
  templateStore.saveTemplates();
};

// 批量导出
const batchExport = () => {
  const selected = templateStore.templates.filter(t => t.selected);
  let csv = '\uFEFF模板名称,章节标题,起始页,结束页\n';
  for (const tpl of selected) {
    const flat = flattenOutline(tpl.outline || []);
    flat.forEach(c => {
      csv += `"${tpl.name}","${c.title}",${c.start},${c.end}\n`;
    });
  }
...
const openMetaDialog = (tpl) => { metaTarget.value = tpl; showMetaDialog.value = true; };
...
const deleteTemplate = async (tpl) => {
  const confirmed = await showConfirmDialogFn(`确定删除「${tpl.name}」吗？`);
  if (!confirmed) return;
  try {
    const filePath = tpl.pdfPath || tpl.filePath;
    if (filePath) await deleteFile(filePath);
    if (tpl.imagesDir) await deleteDirectory(tpl.imagesDir);
    if (tpl.coverPath) await deleteFile(tpl.coverPath);
  } catch (e) {
    console.error('删除模板文件失败:', e);
  }
...
const confirmUpload = () => {
  ...
  templateName.value = selectedFilePath.value.split('\\').pop().replace(/\.[^/.]+$/, '');
...
const startAiOutlineEditor = async () => {
  ...
  try {
    ...
    for (let i = 0; i < tocPages.length; i++) {
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          console.log(`📝 第 ${page} 页 AI原始返回(前500字):`, response?.substring(0, 500));
          ...
          let cleanedResponse = response
            ...
            .replace(/```json\s*/gi, '')
            .replace(/```\s*/g, '')
            // 🔧 移除 PaddleOCR-VL 输出的 base64 内嵌图片
          ...
          console.log(`📝 第 ${page} 页 AI清理后(前500字):`, cleanedResponse?.substring(0, 500));
          ...
          if (looksLikeJson) {
            try {
              ...
              if (fencedMatch) {
                try { const parsed = JSON.parse(fencedMatch[1].trim()); if (Array.isArray(parsed)) { items = parsed; jsonFound = true; } } catch {}
              }
              if (!jsonFound) {
                const arrMatches = [...cleanedResponse.matchAll(/\[[\s\S]*?\]/g)];
                const longest = arrMatches.reduce((best, m) => m[0].length > (best?.length || 0) ? m[0] : best, '');
                if (longest) {
                  try { const parsed = JSON.parse(longest); if (Array.isArray(parsed)) { items = parsed; jsonFound = true; } } catch {}
                }
              }
              if (!jsonFound) {
                try { const parsed = JSON.parse(cleanedResponse); if (Array.isArray(parsed)) { items = parsed; jsonFound = true; } else if (parsed && typeof parsed === 'object') { items = [parsed]; jsonFound = true; } } catch {}
              }
              if (jsonFound) console.log(`✅ 第 ${page} 页 JSON解析成功，${items.length} 条`);
            }
          }
    ...
    // 延迟显示提示（这个 setTimeout 保留，因为它只是显示提示弹窗）
    setTimeout(async () => {
      let msg = `✅ AI识别完成！\n\n` +
                `📊 识别统计：\n` +
                `  · 扫描页数：${pageCount} 页\n` +
...
const applyOffset = () => {
  ...
  // 第三步：重新计算 end（allNodes 就是原节点的引用，修改会同步回树）
...
const showChapterAnalysisModal = computed({
  get: () => templateStore.showChapterAnalysis,
  set: (val) => {
    if (!val) templateStore.closeChapterAnalysis();
  }
});
...
const handlePreview = (data) => {
  ...
  // ✅ 弹出 PDF 页码提示
  const book = templateStore.templates.find(b => b.imagesDir === data.imagesDir);
...
onMounted(async () => { 
  ...
  window.addEventListener(APP_EVENTS.PROCESS_DRAFT, (e) => {
    ...
    selectedFilePath.value = draft.path;
    showUploadModal.value = true;
    showCatalogPageModal.value = false;
  });
});
...
const loadExistingOutline = async () => {
  ...
  if (!templateStore || !templateStore.templates) {
    await showAlertDialogFn('模板数据尚未加载，请稍后再试');
    return;
  }

  const existing = templateStore.templates.find(t => t.name === name);
  if (!existing) {
    await showAlertDialogFn('未找到同名模板，请先上传');
    return;
  }

  if (!existing.outline || existing.outline.length === 0) {
    await showAlertDialogFn('该模板没有目录数据');
    return;
  }
...
const saveTemplate = async () => {
  ...
  try {
    ...
    if (ext === 'pdf') {
      ...
      finalFileName = `${templateId}_带书签.pdf`;
    } else {
      const targetPath = `${storagePath}/模板库/${templateId}.${ext}`;
      await moveFile(tempFilePath.value, targetPath);
      totalPages.value = 1;
      finalFileName = `${templateId}.${ext}`;
    ...
    if (hasExistingCover) {
      ...
      try {
        await createDirectory(`${storagePath}/模板库/缩略图`);
    ...
    const existingIndex = templateStore.templates.findIndex(t => t.name === rawName);
    if (existingIndex !== -1) {
      // 保留原有的勾选状态和分析数据
      const oldTpl = templateStore.templates[existingIndex];
      newTemplate.selected = oldTpl.selected;
      if (oldTpl.analysis) {
        newTemplate.analysis = oldTpl.analysis;
      }
      // 同步章节勾选状态
      ...
      syncSelection(newTemplate.outline, oldTpl.outline || []);
  
      templateStore.updateTemplate(templateStore.templates[existingIndex].id, newTemplate);
    } else {
    ...
    await showAlertDialogFn(`✅ 模板「${rawName}」已成功入库！\n\n📁 存储位置：${storagePath}/模板库/\n📄 文件：${finalFileName}\n🖼️ 图片目录：${templateId}/`);
...
<style scoped>
.template-module { height: 100%; display: flex; }
.library-panel {
...
/* 📚 分组块：为吸顶组头提供**按组限定**的包含块——sticky 的作用域是它的包含块，
   若组头直接铺在 .template-list 下，第一组的组头会一直悬在第二组内容上面。
...
.template-item { border: 1px solid var(--border-light); border-radius: 8px; padding: 8px; background: white; }
...
/* 学科组头默认贴顶；只有同时显示学段组头时才让位到它下方 */
.template-list.with-stage .group-subject { top: var(--group-header-h); }
...
.template-cover { width: 32px; height: 40px; object-fit: cover; border-radius: 4px; }
...
@media (max-width: 767px) {
  ...
  /* === 通用弹窗 === */
  .modal:not(.draggable-modal):not(.outline-editor-modal) {
  ...
  .modal:not(.draggable-modal):not(.outline-editor-modal) h3 {
  ...
  .panel-header h3 { font-size: 13px; margin: 0; }
  ...
  .template-item { border-radius: 8px; padding: 6px 8px; margin-bottom: 6px; }
  .item-header {
    flex-wrap: wrap;
    gap: 6px;
  }
