<template>
  <div class="generate-module">
          ...
          >
            ...
            <div class="result-actions">
              ...
              <button
                ...
                @click.stop="openListeningForDoc(doc)"
              >
                🎧 听力稿
              </button>
    ...
    <!-- 🎧 听力工作台（2026-09-20 抽成公共组件 components/listening/ListeningWorkbench.vue）：
         记录入口与独立功能页共用同一实现，避免两套口径。 -->
    <ListeningWorkbench ref="listeningWorkbenchRef" />
...
//   按钮直接显示"类型默认（自动）"而非泛称——修复"组织风格不默认"的初始化缺口
watch(genTypes, () => {
  const type = genTypes.value[0];
...
const loadInstructionFromLibrary = async (genTypeOverride = '', booksOverride = null) => {
  ...
  try {
    ...
    if (scopeSource?.outline) {
      scopeInfo = resolveScopeName({
        book: scopeSource,
        scopeType: scopeType.value || '',
        scopeOverride: scopeOverride.value || '',
        pickScope: pickScopeFromPool,
      });
      unit = scopeInfo.name || '';
    }
  } catch { /* 范围推断失败不影响 */ }
...
// ═══════════════════════════════════════════════════════════════
// 🎧 听力配音（2026-09-20 起为**独立功能**）
// ============================================================
// 用户裁定："粘贴配音是一个功能哦，不是附属在结果列表中的，结果列表中的结果不会一直存在，
//   是20条上限滚动的" → 听力工作台已抽成 components/listening/ListeningWorkbench.vue，
//   并从左侧导航「工具 → 🎧 听力配音」独立进入（不依赖任何生成记录）。
// 生成页这里**只留一件事**：把「从本记录进入」转发给组件——解析/渲染/合成/参数面板全在组件内，
//   两条入口共用同一实现，不存在第二套口径。
// ═══════════════════════════════════════════════════════════════
/** 🎧 听力工作台引用：「从本记录进入」转发给组件（组件自行取原文、解析、渲染与合成） */
const listeningWorkbenchRef = ref(null);
const openListeningForDoc = (doc) => listeningWorkbenchRef.value?.openFromRecord(doc);
...
const downloadDoc = async (doc, format) => {
  ...
  // 🔧 裸书写空 → u.blank-N 补包（旧记录/源码直出里模型 em 空格形态 &emsp;/U+2003 先转填空横线，
  //    与全角空格 \u3000 同口径；否则导出无下划线横线——2026-09 实证回归），再打"单独空行"延伸标
  content = markSoloBlankLines(wrapBareBlankRuns(content));
...
/* 🔧 转文本后的项目符号字形(仅空心圆)：0.4em 对齐浏览器原生 circle 标记真实尺寸（需与编辑器/排版/全局同步） */
