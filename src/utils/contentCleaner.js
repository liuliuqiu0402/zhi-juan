 ...
 * 围绕…核心知识…命制同步练习。现在编写正文。"当正文首段输出——声明≠覆盖，罗列清单不算内容）。
...
export function stripPlanningPreamble(raw = '') {
  if (!raw) return raw;
  let out = String(raw);
  const PLAN_RE = /^(?:我已|已取到|已获取|已拿到|已检索到|现在|接下来|以下(?:将|是)?|根据|依据|现依据|围绕|请根据|本次)[^<\n]{0,60}?(?:教材原文|知识点|核心知识|课标|命制|编写|设计|同步练习|课时练习|课时练|课堂练习|易错题本|错题本|试卷|正文|大纲|素材)/;
 ...
 *   该段正文连同题号一起消失（实测：英语同步练习题 1~18 完整，归一化后 2、3、4 整题被吞，整卷判失败）。
 ...
 * 2026-09-12 实测（英语同步练习）：模型原始输出题号 1~14 完整无缺，归一化后 3、4、5 整题消失
