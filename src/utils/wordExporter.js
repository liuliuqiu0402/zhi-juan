...
/**
 * 将田字格字符渲染为 PNG Data URL（含外框+十字虚线+居中汉字）
 * @param {string} char - 汉字字符
 * @param {number} [fontSize=16] - 字号(px)，渲染 2x 用于 Retina
 * @returns {string} data:image/png;base64,...
 */
export const renderTianZiGeToDataUrl = (char, fontSize = 16) => {
  const scale = 2;
  const size = Math.round(fontSize * 1.8 * scale);
  ...
  // 居中汉字
  ctx.font = `${fontSize * scale}px 'KaiTi', 'SimSun', serif`;
...
/**
 * 将四线三格字符渲染为 PNG Data URL（含 4 条水平线 + 居中字母）
 * 对应 CSS: 0.1em 灰 / 0.55em 灰 / 1.0em 深灰 / 1.45em 红
 * @param {string} letter - 字母
 * @param {number} [fontSize=16] - 字号(px)，渲染 2x 用于 Retina
 * @returns {string} data:image/png;base64,...
 */
export const renderFourLineToDataUrl = (letter, fontSize = 16) => {
  const scale = 2;
  const width = Math.round(fontSize * 2.0 * scale);   // 宽裕一些容纳字母
  const height = Math.round(fontSize * 1.5 * scale);   // 对应 CSS height: 1.5em
  const canvas = document.createElement('canvas');
  canvas.width = width;
  ...
  // 4 条水平线
  const linePositions = [0.1, 0.55, 1.0, 1.45];       // em 位置
  const lineColors = ['#999', '#999', '#666', '#e74c3c'];
  const lineWidths = [1, 1, 1, 1.5];                    // scale units

  for (let i = 0; i < 4; i++) {
    const y = Math.round(linePositions[i] * fontSize * scale);
    ctx.strokeStyle = lineColors[i];
    ctx.lineWidth = lineWidths[i] * scale;
    ctx.beginPath();
    ctx.moveTo(0, y);
    ctx.lineTo(width, y);
    ctx.stroke();
  ...
  // 居中字母
  ctx.font = `${fontSize * scale}px 'Times New Roman', serif`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillStyle = '#000';
