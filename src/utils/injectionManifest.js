 ...
 * ============================================================
 */
import { PAPER_OUTPUT_CONVENTIONS, AUTONOMOUS_ITEM_TYPES, TYPED_PRACTICE_ITEM_TYPES } from '../config/promptLibrary.js';
// 🔴 题型自拟的题类（AUTONOMOUS_ITEM_TYPES）与"仅注入题型多样"的题类（TYPED_PRACTICE_ITEM_TYPES）：
//    单源定义在 config/promptLibrary.js（文本层），此处反向引用——避免两处各写一份（本模块本就 import 该文件，无循环依赖）。
//    语义与理由见该处注释（后者 = 前者 + 易错题本：易错题本只注入"题型须多样"，不注入"题组按题型分组"）。
export { AUTONOMOUS_ITEM_TYPES, TYPED_PRACTICE_ITEM_TYPES };
...
export const buildMaterialUsageBlock = ({ genType = '', materialChannel = 'auto' } = {}) => {
  ...
  //    根因：这三类题类（同步练习/专项/阅读）的**题型由模型自拟**，全链路没有任何题型来源 → 题型随内容漂移，
  ...
  //    适用范围 = TYPED_PRACTICE_ITEM_TYPES：题型自拟的题类（AUTONOMOUS_ITEM_TYPES）+ 易错题本（2026-09-15 用户定版，
  //      其题目由本资料自行命制，故同样需要题型多样）；考卷有【卷面结构】题型序列、知识归纳型不命题，均不注入。
  //    ⚠️ 易错题本**只进本句**，不进下方 buildOrganizeBlock 的"题组按题型分组"句——其分组按知识点/错因。
  if (TYPED_PRACTICE_ITEM_TYPES.includes(genType)) {
    parts.push('· 题型须多样：同一份资料内各类作答方式按本学科本学段日常练习的常态分布，不停留在单一形态（作业类型比例应合理安排、给学生留出可选余地）；具体用哪些题型，按本学科本学段常规自选（不指定、不列清单）。（本条只约束作答方式的多样，不改变要练到的范围与题量口径，不得为凑题型而漏项或加无关题。）\n');
  }
...
export const buildOrganizeBlock = (genType = '') => {
  ...
  // 🔴 分组依据（2026-09-14 用户定版）：题型自拟的题类（同步练习/专项/阅读）——分组与命名按本学科常规
