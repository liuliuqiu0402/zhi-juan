...
资料内一切**声明**（写在题干、大题标题或栏目标题里的都算）所点到、且本题作答所必需的一切内容（不论其形态），都必须在正文中真实、足量、形式吻合地存在——使本题**仅凭正文自身即可完成**；凡声明点到而正文未给出、或给出但不完整、不足以支撑该设问、或与正文不符，以及内容漏错、自相矛盾的，均属无效内容，须于定稿前修正。**声明与实给一致**：声明里写到的提示方式与作答方式，正文须真按它给出；正文实际给出的提示与作答方式，也须在对应声明里如实写到——不得声明一样、给出另一样。**写法自洽**：同类声明与同类作答位的写法前后一致（题号与答案区逐题对应、同类作答位的位置与形态统一、术语与标注同一口径）。素材依课标可取自教材之外的真实情境、不限所选教材，但题面引用或呈现的素材必须与本题实际给出的内容完全一致；正文不得先现待作答结论；正文对知识、要点、示例、数据、结论的归纳与转述须准确、完整、不遗漏、条理清晰，各题、各要点、各结论及其答案之间前后一致、互不矛盾。`;
...
export const buildOrganizeBlock = (genType = '') => {
  ...
  return `【组织方式】输出一律以委托书【卷面结构】的大题序列组织（大题名、顺序、题量以委托书为准）；开头【锚点清单】只声明要练到的范围，不是组织方式，不得据此替代委托书结构。\n\n`;
};
