 ...
 *  🔴 2026-09-17 用户裁定（"让模型把指令切实执行、再做到自洽——模型侧，不依赖程序侧；所有资料类型都要"）：
 *    原文本是**总述式**（一路逗号铺陈"声明↔实给/写法自洽"），模型难以据此逐项执行。
 *    重写为**三域 + 定稿前动作**：① 声明↔实给 ② 要素之间 ③ 跨处之间，并明确"逐节逐题复核、不自洽当场改、只输出改后定稿"。
 *    为什么这样才"精准"：执行发生在模型侧，判据必须**可自判**——三域各自给出可核对的对照物（声明↔正文、要素↔要素、处↔处），
 *    且把"复核"写成**成稿前的动作**（与题号条款"输出完成后逐题自查题号是否连续"同一范式）。
 *  🔒 不变量（tests/utils/paperProbeConsistency ⑤ 锁）：原则式、**零题型/载体名列举**、全类型中性措辞（用"资料/正文/每处内容"，
 *    不用"卷面"），并保留"仅凭正文自身即可完成"与"声明与实给一致"两条语义。 */
export const TAIL_SELF_CONSISTENCY = `【尾约束·全文自洽】
**定稿前逐节逐题按下面三域复核**，凡不自洽的当场改——改声明或改内容，二者取一，只输出改后的定稿；**不得以"已声明/已注明"代替"已满足"**。
①**声明与实给一致**（声明 ↔ 实给）：资料内一切**声明**（写在标题、栏目标题、题干、导语、目录、图注、注释里的都算）所点到的内容、形态与做法，都必须在正文中真实、足量、形式吻合地存在——使每处内容**仅凭正文自身即可完成或读懂**；声明里写到的提示方式、作答方式与呈现方式，正文须真按它给出；正文实际给出的，也须在对应声明里如实写到——**不得声明一样、给出另一样**；凡声明点到而正文未给、给而不全、不足以支撑该处、或与正文不符的，均属无效内容，须于定稿前修正。
②**要素之间**：同一处内容内部各要素（题干、选项、材料、作答位、答案、解析、说明、数据、称谓）相互一致——答案须真正满足题干对它提出的**一切形式与内容要求**；给出的材料须被**完整而恰当地使用（不增不拆不改形）**，并由该材料**唯一确定**答案；说话人与角色归属、时态与时间状语、数量与单位、指代与称谓、图与文互不矛盾。
③**跨处之间**：全篇口径一致——**写法自洽**（编号体系、同类作答位的位置与形态统一、术语与标注、符号与单位同一口径）、正文与答案区/解析/材料逐处对应、各节各题的结论互不冲突；同一事实、同一要点、同一数据全篇只有一种表述。
素材依课标可取自教材之外的真实情境、不限所选教材，但题面引用或呈现的素材必须与本题实际给出的内容完全一致；正文不得先现待作答结论；正文对知识、要点、示例、数据、结论的归纳与转述须准确、完整、不遗漏、条理清晰。`;
...
export const buildMaterialUsageBlock = ({ genType = '', materialChannel = 'auto' } = {}) => {
  ...
  const refClause = materialChannel === 'anchor'
    ? '以上内容（含各知识点具体概念）可供参考，用以把握教材内容、难度与版本口径'
  ...
  parts.push(refMode === 'full'
    ? (materialChannel === 'anchor'
      ? '· 本资料为知识归纳型（本次按锚清单通道生成）：归纳对象以上方清单为准，可依教材事实与课外同类材料转写为教辅表述，不得整段照录；正文不得出现任何出处标注（"选自/单元/章节/课题/课文名/位置式指引/原文出处"等溯源字样一律不写）。\n'
      : '· 本资料为知识归纳型：可引用、可归纳中段【压缩原文】，但须转写为教辅表述，不得整段照录；正文不得出现任何出处标注（"选自/单元/章节/课题/课文名/位置式指引/原文出处"等溯源字样一律不写）。\n')
    : (materialChannel === 'anchor'
      ? '· 本资料为命题/练习型：内容、深度与难度可参考上方内容（含具体概念）；题干、情境、人名、数据与句式由你拟定，来源按上述口径。\n'
      : '· 本资料为命题/练习型：中段【压缩原文】供你参考内容与难度；题干、情境、人名、数据与句式由你拟定，来源按上述口径。\n'));
...
export const buildOrganizeBlock = (genType = '') => {
  ...
  // 🔴 2026-09-16 用户裁定（少约束）：教辅不再注入本块——题目怎么划分、叫什么全交模型，
  //    形式句里不出现“名称/先后/题组”这类指向（防把内容颗粒读成题组骨架）。
  //    exam 分支逐字不变（正规卷必须守蓝图给出的卷面结构）。
  if (genType !== 'exam') return '';
  // 🔴 2026-09-17 用户裁定（同批）：【卷面结构】每行 `——【要求·须逐项落实】` 后是**课标/正规卷对该大题的形态要求**，
  //    原先只当行尾注解 → 模型按"背景描述"处理（实证：蓝本注"短文，选择与判断"，卷面只出一种设问）。
  //    此处把"效力 + 逐栏目自查动作"补在**单一出处**（结构行的标注由 buildStructureText 给出，两处同源）。
  return `【组织方式】输出一律以委托书【卷面结构】的大题序列组织（大题名、顺序、题量以委托书为准）；开头【锚点清单】只声明要练到的范围，不是组织方式，不得据此替代委托书结构。每行"【要求·须逐项落实】"后是该大题按**课标与正规卷**定的形态要求（设问方式、材料形态、读写方式、每空词数、材料读几遍等），**必须逐项落实**；成稿前逐栏目对照自查：凡该行写了而卷面没有照做的（如要求两种设问形态却只出一种），改题面或改内容，二者取一——把它当"背景描述"读过去，即属未落实。\n\n`;
};
...
const BLOCK_DEFS = [
  {
    ...
    note: '生成时按勾选章节的分析结果注入：第1层知识主题作分组前缀、第2层知识点为主体、第3层具体概念随各知识点括注；涉及的内容以陈述给出（2026-09-15 去对账语言）',
    noteWith: (c) => [
