...
/**
 * 认知层级标签（章级 `competency`）：小学（含未标年级的小学教材）→ '识记与理解'；初中/高中 → '应用与分析'。
 * ============================================================
 * 🔴 单一事实源，且**只判学段、不看年级数字**。旧写法
 *    `extractGradeNum(grade) <= 6 ? '识记与理解' : '应用与分析'` 有两处反例：
 *    ① grade 为空（初中/高中教材本就不标年级）→ extractGradeNum('') 返回 0 → `0<=6` 成立
 *       → **高中卷被误标成"识记与理解"**（小学低段口径）；
 *    ② 高中若把册次写进 grade（"选择性必修2"）→ 不含中文数词、parseInt 亦失败 → 同样返回 0 → 同样误判。
 *    学段才是唯一稳定依据（resolveStageKey 对初中/高中直接返回粗学段键，不依赖年级）。
 *    消费端禁止再各自写 `extractGradeNum(...) <= 6` 判认知层级。
 * ============================================================
 */
export const resolveCompetency = (stage = '', grade = '', name = '') =>
  String(resolveStageKey(stage, grade, name) || '').startsWith('primary') ? '识记与理解' : '应用与分析';

/**
 * 提示文案用的「年级 / 册次」标签（**只用于拼提示词与界面文案，不参与任何判定**）。
 * ============================================================
 * 高中 → 册次（"必修1""选择性必修2""必修上"；没填则空串）；其他学段 → 年级。
 * 🔴 为什么高中给册次而不是年级：高中教材按必修／选择性必修**分册**、本身不绑定年级
 *    （各省教学用书的「册次」与「使用年级」是两栏并列，使用年级写的是区间；
 *     "必修＝高一"各省不成立），提示词里写"高一"既不准确也会误导模型；
 *     写册次才能让模型知道是"必修1"还是"选择性必修2"这一册。见 config/highVolumes。
 * 🔴 使用注意：本函数输出**不得**回灌给 extractGradeNum / resolveStageKey 之类的判定函数
 *    （"必修1"不是年级）；判定一律用 stage/五档键。
 * 🔴 高中**优先册次、未填册次时退回年级**：不为了"口径纯洁"把没填册次的老数据变成空白
 *    （老数据里高中 grade 曾是"高一/高二/高三"，虽然与册次无官方绑定，但退回去至少不比改版前更空）。
 * ============================================================
 */
export const gradeDisplayLabel = (stage = '', grade = '', volume = '') => {
  if (resolveStageKey(stage, grade) !== 'high') return String(grade || '');
  return String(volume || grade || '');
};