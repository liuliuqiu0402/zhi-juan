...
export const MATERIAL_LINE = '◇ 语言材料（用于把握难度与理解语境，不必单独设题）：';
...
const THIRD_LAYER_NOTE =
  '知识点名后括号内为该知识点的**具体概念**（第3层）：细化"该知识点含哪些概念/词条/数值"，供把握深度与范围。';
...
export const anchorListRoleNote = ({ withConcepts = true, splitMaterial = false } = {}) =>
  '说明：下面的内容按「章 → 知识主题 → 知识点」组织，即本次资料要练到的内容范围（分散在多题中体现、或多点合成一题亦可）；'
  + '知识主题（第1层）标示知识点的归属，知识点（第2层）是内容的最小单位。'
  + (withConcepts ? THIRD_LAYER_NOTE : '')
  + '不带「主题：」前缀的章 = 该章知识点未再分主题。'
  + '其中无法通过书面作答直接体现或检验的内容，以**可写、可判**的书面形态呈现。'
  + '范围之外能否补充或整合，按资料类型见委托书【素材使用约定】。'
  + (splitMaterial ? '标◇的**语言材料**用于把握难度与理解语境，不必为其单独设题。' : '');
