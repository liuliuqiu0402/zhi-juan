...
/** 作答空位过宽统一（2026-09 实证：同步练习 35 处填空横线全部 blank-8 字位档——模型未按各空答案
...
/**
 * 资料内"同词音标冲突"检测（2026-09，只报不改）
 * ============================================================
 * 抽取 词 /音标/ 对；同一词（忽略大小写）在同一份资料内出现 ≥2 套不同音标 → 资料内自相矛盾，
 * 提示复核。属"资料内一致性"探测——单处音标是否与词典一致（如 Chinese 末 s 实为 /z/）
 * 超出文本比对能力，由生成端"音标须与实际发音一致且资料内自洽"条款约束。
 */
export const detectPhonemeConflicts = (html = '') => {
  ...
  for (const [w, set] of map) {
    ...
    out.push(`同一单词 "${w}" 在同一份资料内出现多套音标（${[...set].join(' / ')}），资料内不自洽，请按词典实际发音统一`);
  }
  return out;
};
...
/**
 * 资料内"引文复现不一致"检测（2026-09，只报不改）
 * ============================================================
 * 同一句引文（含引号的书名号内文、诗句、名句、材料引文等）在同一份资料内出现 ≥2 处，若字符高度近似
 * 却不完全一致（编辑距离 ≤2 且长度 ≥6），即疑似同一引文被复述为两种写法 → 资料内不自洽，提示复核。
 * 属"资料内一致性"探测（与 detectPhonemeConflicts 同型）：不判断引文对错，只报同份资料内写法冲突；
