...
const LEGACY = [
  ...
  { name: '旧资料类型名"听写/默写"（已统一为默写积累）', re: /听写\/默写|听写默写/g, exempt: [/留空|专用生成|知识型\/错题\/听写|听写格式|听写条目|听写推荐|听写/] },
  { name: '旧注释"8 个子库"（当前 5 库均 ready）', re: /8 个子库/g, exempt: [] },

  // ── C2. 旧资料类型名（2026-09-15 改名：课时练→同步练习 / 课前预习→预习导学 / 错题本→易错题本）──
  //   豁免：兼容用户旧输入的匹配词、历史说明注释（含"同步练习/易错题本/兼容/旧名"等字样的行）
  { name: '旧资料类型名"课时练"（已改名 同步练习）', re: /课时练/g, exempt: [/同步练习|兼容|旧名|已改名|用户输入|历史/] },
  { name: '旧资料类型名"课前预习"（已改名 预习导学）', re: /课前预习/g, exempt: [/预习导学|兼容|旧名|已改名|用户输入|历史/] },
  { name: '旧资料类型名"错题本"（已改名 易错题本）', re: /(?<!易)错题本/g, exempt: [/易错题本|兼容|旧名|已改名|用户输入|历史/] },
  { name: '旧栏目名"原题重现"（已改名 题目呈现）', re: /原题重现/g, exempt: [/兼容|旧名|已改名/] },
